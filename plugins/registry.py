...
logger = get_logger("yim.plugins")

_ENTRY_POINT_GROUP = "yim.plugins"


class PluginRegistry:
    """Discovers, loads, and manages lifecycle of yim plugins.

    Discovery order (later overrides earlier):
    1. Bundled plugins (yim/plugins/bundled/)
    2. pip-installed plugins (entry point: yim.plugins)
    3. Project-local plugins (./.yim/plugins/)
    4. User-local plugins (~/.dunimd/yim/plugins/)
    """

    def __init__(self) -> None:
        self._plugins: dict[str, YmiPlugin] = {}
    ...
    def register(self, plugin: YmiPlugin) -> None:
    ...
    def get(self, name: str) -> YmiPlugin | None:
    ...
    def discover_all(self) -> int:
        ...
        self._discover_directory("./.yim/plugins/", PluginSource.PROJECT)
        self._discover_directory("~/.dunimd/yim/plugins/", PluginSource.USER)
    ...
    def _discover_entry_points(self) -> None:
        ...
        for ep in entry_points:
            try:
                ...
                if not isinstance(plugin, YmiPlugin):
                    logger.warning(f"Entry point '{ep.name}' did not return a YmiPlugin instance")
                    continue
    ...
    def _extract_plugin(self, module: Any, name: str, source: PluginSource) -> None:
        # Look for a class that extends YmiPlugin
        for attr_name in dir(module):
            attr = getattr(module, attr_name)
            if isinstance(attr, type) and issubclass(attr, YmiPlugin) and attr is not YmiPlugin:
                plugin = attr()
        ...
        if hasattr(module, "create_plugin"):
            try:
                ...
                if isinstance(plugin, YmiPlugin):
                    plugin.manifest.source = source
                    self.register(plugin)
                    return
            except Exception:
