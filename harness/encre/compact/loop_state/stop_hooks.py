...
async def execute_stop_hooks(
    ...
    Returns:
        ...
        Empty list means no blocking 鈥?the turn can end normally.
    """
    ...
    for result in results:
        ...
        if result.get("action") == "block":
            ...
            if message:
                ...
                logger.info(
                    "[stop_hook] blocking 鈥?injecting hook output turn=%d",
                    turn_count,
                )
...
def should_block_stop(
    ...
    *max_blocks*, the block is overridden to prevent infinite loops.
    ...
    if block_count >= max_blocks:
        logger.warning(
            "[stop_hook] max blocks (%d) exceeded 鈥?forcing exit", max_blocks
        )
        return False
    return any(
