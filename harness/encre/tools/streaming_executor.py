...
class AbortController:
    ...
    aborted 鈥?no separate per-tool signalling needed.
    """
...
class TrackedTool:
    ...
    States: queued 鈫?executing 鈫?completed 鈫?yielded
    """
...
class StreamingToolExecutor:
    ...
    def _propagate_error(self, errored_tool: TrackedTool) -> None:
        ...
        ``agent``) are never cancelled 鈥?they are always isolated.
        """
    ...
    def get_completed_results(self) -> Generator[AgentEvent, None, None]:
        ...
        Non-blocking 鈥?use this to drain ready results between iterations.
        """
        if self._discarded:
            return
    ...
    def _default_emit_results(tool: TrackedTool) -> Generator[AgentEvent, None, None]:
        """Default result emission 鈥?basic tool_result + tool_call_end."""
