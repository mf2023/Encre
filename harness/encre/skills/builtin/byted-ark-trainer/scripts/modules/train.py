...
class EvalConfig:
    ...
    娉ㄦ剰锛氳瘎浼扮殑妯″瀷鍚?绔偣ID/鑷畾涔夋ā鍨婭D **涓嶅湪杩欓噷**閰嶇疆锛岃€屾槸鐢?rollout 鍑芥暟鍐呴儴
    璋冪敤 `chat.completions.create(model=...)` 鏃朵紶鍏ョ殑閭ｄ釜瀛楃涓插喅瀹氥€傝繍琛?evaluate
    鍓嶈鍏堟妸 rollout.py 涓殑 model 瀛楁鏀规垚鐩爣妯″瀷銆?
    """
...
def evaluate_command(args):
    ...
    try:
        ...
        print(f"馃摑 璇︾粏杩愯鏃ュ織宸蹭繚瀛樺埌: {log_file.absolute()}")
...
def _clean_rft_message(message: Dict[str, Any]) -> Dict[str, Any]:
    """
    娓呯悊璇勪及杞ㄨ抗涓殑杩愯鏃跺厓鏁版嵁锛屼繚鐣欒缁冮渶瑕佺殑瀵硅瘽瀛楁銆?
    """
    allowed_keys = {
        "role",
        "content",
...
def _normalize_tools_config(raw_config: Any, source: str) -> Dict[str, Any]:
    ...
    鏀寔鐩存帴浼?tools 鏁扮粍锛屾垨浼犲寘鍚?tools 瀛楁鐨勫璞°€?
    """
    ...
    if not isinstance(tools, list):
        raise ValueError(f"{source} 鐨則ools瀛楁蹇呴』鏄暟缁?)
...
def _load_tools_from_rollout(rollout_path: str) -> Dict[str, Any]:
    """
    浠巖ollout鎻掍欢涓寜绾﹀畾璇诲彇宸ュ叿瀹氫箟锛屾敮鎸佸彉閲忓悕锛?
...
def _set_thinking_field(sample: Dict[str, Any]) -> None:
    """
    鍙傝€冩暟鎹泦Thinking瀛楁澶勭悊宸ュ叿锛氭牴鎹牱鏈槸鍚﹀惈reasoning_content璁剧疆thinking銆?
    杩欓噷浼氭寜鎷嗗垎鍚庣殑鍗曟潯鏍锋湰閲嶆柊璁＄畻锛岄伩鍏嶄粠鍘熷鏍锋湰缁ф壙鍑?
    鈥渢hinking enabled 浣嗘牱鏈唴娌℃湁reasoning_content鈥濈殑涓嶄竴鑷存牸寮忋€?
    """
...
def _split_multi_turn_reasoning_sample(sample: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    鍙傝€冣€滃杞畆easoning_content鐨勬牱鏈枃浠舵媶鍒嗏€濓細
    灏嗕竴鏉℃惡甯﹀杞畆easoning_content鐨勬牱鏈媶鎴愬鏉′粎鏈€鍚庝竴杞產ssistant鎼哄甫
    reasoning_content鐨勬牱鏈€傜户缁笂涓嬫枃涓殑鍘嗗彶assistant浼氬幓鎺塺easoning_content
    骞惰缃甽oss_weight=0锛岄伩鍏嶅悓涓€鍥炲琚噸澶嶈缁冦€?
    """
...
def rft_data_collect_command(args):
    ...
    RFT鏁版嵁鏀堕泦锛氫粠璇勪及缁撴灉涓瓫閫塺eward=1.0鐨勪紭璐ㄨ建杩癸紝鐢熸垚RFT璁粌鏁版嵁
    """
    try:
        print("\n=== 鏀堕泦RFT璁粌鏁版嵁 ===")
        ...
        if not detailed_results:
            logger.error("璇勪及缁撴灉涓病鏈塪etailed_results鏁版嵁")
            return None
        ...
        print(f"鎬绘牱鏈暟: {len(detailed_results)}")
        ...
        if missing_tools_count:
            print(
                f"鈿狅笍  鏈墈missing_tools_count}鏉″惈tool_calls/tool娑堟伅鐨勮建杩圭己灏戦《灞倀ools銆傝閫氳繃--rollout鎴?-tools-file琛ュ厖宸ュ叿瀹氫箟銆?
            )
        print(f"RFT璁粌鏁版嵁宸蹭繚瀛樺埌: {output_path}")
