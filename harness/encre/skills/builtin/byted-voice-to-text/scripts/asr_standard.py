...
"""
asr_standard.py 鈥?鐏北寮曟搸 BigModel ASR 鏍囧噯鐗堬紙褰曢煶鏂囦欢璇嗗埆锛夈€?

寮傛 submit + poll 妯″紡锛屾敮鎸?鈮? 灏忔椂闊抽銆?
...
try:
    ...
    fail_json("MISSING_DEPENDENCY", "requests 搴撴湭瀹夎锛岃鎵ц pip install requests")
...
def submit_task(
    ...
    if audio_url:
        ...
        if not os.path.isfile(file_path):
            fail_json("FILE_NOT_FOUND", f"闊抽鏂囦欢涓嶅瓨鍦? {file_path}")
        if os.path.getsize(file_path) == 0:
            fail_json("FILE_EMPTY", f"闊抽鏂囦欢涓虹┖: {file_path}")
    ...
    log(f"姝ｅ湪鎻愪氦鏍囧噯鐗堣瘑鍒换鍔? task_id={task_id}")
    try:
        resp = requests.post(
    ...
    if resp_status != "20000000":
        ...
        fail_json(
            "SUBMIT_FAILED",
            f"鏍囧噯鐗堜换鍔℃彁浜ゅけ璐? code={resp_status}, msg={msg}",
            task_id=task_id,
            logid=x_tt_logid,
        )
...
def poll_until_done(
    ...
    while True:
        ...
        if elapsed > poll_max_time:
            fail_json(
                "POLL_TIMEOUT",
                f"鏍囧噯鐗堣瘑鍒换鍔¤秴鏃? task_id={task_id}, "
        ...
        if code == "20000000":
            ...
            fail_json(
                "TASK_FAILED",
                f"鏍囧噯鐗堣瘑鍒け璐? code={code}, msg={qr['message']}",
                task_id=task_id,
                logid=qr["logid"],
            )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="鐏北寮曟搸 BigModel ASR 鏍囧噯鐗堬紙褰曢煶鏂囦欢璇嗗埆锛屽紓姝?submit+poll锛?,
...
绀轰緥:
  ...
  python3 asr_standard.py --query-task-id <ID> --query-logid <LOGID>
    ...
    input_group.add_argument("--url", help="闊抽鏂囦欢鐨?URL 鍦板潃")
    input_group.add_argument("--file", help="鏈湴闊抽鏂囦欢璺緞")
    ...
    parser.add_argument(
        "--no-poll",
        action="store_true",
        help="浠呮彁浜や换鍔★紝涓嶈疆璇㈢粨鏋滐紙杩斿洖 task_id 渚涘悗缁煡璇級",
    )
    parser.add_argument(
        "--poll-interval",
        type=int,
        default=DEFAULT_POLL_INTERVAL,
        help=f"杞闂撮殧绉掓暟 (榛樿: {DEFAULT_POLL_INTERVAL})",
    )
    parser.add_argument(
        "--poll-max-time",
        type=int,
        default=DEFAULT_POLL_MAX_TIME,
        help=f"鏈€澶ц疆璇㈡椂闂寸鏁?(榛樿: {DEFAULT_POLL_MAX_TIME})",
    )

    # 
    parser.add_argument(
        "--query-task-id",
        help="鏌ヨ宸叉湁浠诲姟鐨?task_id锛堜笌 --query-logid 閰嶅悎浣跨敤锛?,
    )
    parser.add_argument(
        "--query-logid",
        default="",
        help="鏌ヨ鏃朵紶鍏ョ殑 X-Tt-Logid锛堝彲閫夛級",
    )

    args = parser.parse_args()

    #  妯″紡 1: Query existing task

    if args.query_task_id:
        log(f"鏌ヨ浠诲姟: task_id={args.query_task_id}")
    ...
    if not args.url and not args.file:
        fail_json("NO_INPUT", "蹇呴』鎻愪緵 --url 鎴?--file锛堟垨 --query-task-id 鏌ヨ宸叉湁浠诲姟锛?)
    ...
    if args.no_poll:
        print(json.dumps({
            ...
            "status": "submitted",
            "message": "浠诲姟宸叉彁浜わ紝浣跨敤浠ヤ笅鍛戒护鏌ヨ缁撴灉:",
            "query_command": (
