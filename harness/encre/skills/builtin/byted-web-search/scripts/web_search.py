...
"""鐏北寮曟搸鑱旂綉鎼滅储 API 瀹㈡埛绔€?

瀹樻柟鏂囨。锛歨ttps://www.volcengine.com/docs/85508/1650263
绛惧悕鍙傝€冿細https://github.com/volcengine/volc-openapi-demos/blob/main/signature/python/sign.py

鍑瘉锛圕law 涓紭鍏堬級锛氭嬁 Key 鍚庣洿鎺ュ湪鑱婂ぉ妗嗗彂缁欐垜鍗冲彲锛屾棤闇€缂栬緫閰嶇疆銆?
璁よ瘉浼樺厛绾э細1) WEB_SEARCH_API_KEY 鎴?--api-key  2) VOLCENGINE_ACCESS_KEY+SECRET_KEY  3) VeFaaS IAM

绀轰緥锛?
    python web_search.py "鍖椾含澶╂皵"
    python web_search.py "OpenAI 鏈€鏂板彂甯? --time-range OneWeek
    python web_search.py "鏁呭鍗氱墿闄? --type image --count 3
"""
...
ERROR_HINTS = {
    "10400": "鎻愮ず锛氬弬鏁伴敊璇€傝妫€鏌?Query銆丆ount銆乀imeRange 绛夊弬鏁版牸寮忔槸鍚︽纭€?,
    "10402": "鎻愮ず锛氭悳绱㈢被鍨嬮潪娉曘€傚綋鍓嶄粎鏀寔 web 鎴?image銆?,
    "10403": "鎻愮ず锛氳处鍙锋垨鏉冮檺寮傚父銆傝纭 API Key 鏉ヨ嚜鑱旂綉鎼滅储鎺у埗鍙帮紝鎴栨鏌ヨ处鍙锋潈闄愩€?,
    "10406": "鎻愮ず锛氬厤璐归搴﹀凡鑰楀敖銆傝妫€鏌ヨ处鎴烽搴︽垨鑱旂郴鏀寔銆?,
    "10407": "鎻愮ず锛氬綋鍓嶆棤鍙敤鍏嶈垂绛栫暐銆傝妫€鏌ヨ处鎴风姸鎬佹垨鑱旂郴鏀寔銆?,
    "10500": "鎻愮ず锛氭湇鍔″唴閮ㄩ敊璇€傚缓璁◢鍚庨噸璇曪紝鎴栬仈绯绘敮鎸併€?,
    "700429": "鎻愮ず锛氬厤璐归摼璺Е鍙戦檺娴併€傝闄嶉鍚庨噸璇曘€?,
    "100013": "鎻愮ず锛氬瓙璐﹀彿鏈巿鏉?TorchlightApiFullAccess銆?,
}
...
def _validate_time_range(time_range: Optional[str]) -> Optional[str]:
    ...
    if not match:
        raise ValueError(
            "--time-range 闇€涓?OneDay/OneWeek/OneMonth/OneYear锛屾垨鏃ユ湡鍖洪棿 YYYY-MM-DD..YYYY-MM-DD銆?
        )
    ...
    try:
        ...
        raise ValueError("--time-range 涓殑鏃ユ湡闇€涓烘湁鏁堢殑 YYYY-MM-DD銆?) from exc

    if start_date > end_date:
        raise ValueError("--time-range 鐨勫紑濮嬫棩鏈熶笉鑳芥櫄浜庣粨鏉熸棩鏈熴€?)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="鐏北寮曟搸鑱旂綉鎼滅储 API\nhttps://www.volcengine.com/docs/85508/1650263\n"
                    "鍑瘉锛欳law 涓洿鎺ュ湪鑱婂ぉ妗嗗彂 Key 鍗冲彲锛涙垨 WEB_SEARCH_API_KEY / --api-key"
    )
    ...
    parser.add_argument("--query-rewrite", action="store_true", help="寮€鍚?Query 鏀瑰啓")
    parser.add_argument("--api-key", help="API Key锛堜紭鍏堜簬鐜鍙橀噺 WEB_SEARCH_API_KEY锛?)
    ...
    if len(args.query) > 100:
        print("Error: 鎼滅储璇嶈秴杩?100 瀛楃锛孉PI 鍙兘鎴柇銆傚缓璁簿绠€鍚庨噸璇曘€?, file=sys.stderr)
        sys.exit(1)
    ...
    if args.type == "image" and args.count > 5:
        print("Error: image 绫诲瀷鏈€澶氳繑鍥?5 鏉★紝璇疯皟鏁?--count銆?, file=sys.stderr)
        sys.exit(1)
    if args.type == "web" and args.count > 50:
        print("Error: web 绫诲瀷鏈€澶氳繑鍥?50 鏉★紝璇疯皟鏁?--count銆?, file=sys.stderr)
        sys.exit(1)
    ...
    if not api_key:
        ...
        if not ak or not sk:
            print(
                "Error: 鏈壘鍒板嚟璇併€傝閰嶇疆浠ヤ笅浠讳竴鏂瑰紡锛歕n"
                "1) 銆愭帹鑽愩€戣嫢鍦?Claw 涓娇鐢細鎷?Key 鍚庣洿鎺ュ湪鑱婂ぉ妗嗗彂缁欐垜鍗冲彲锛屾棤闇€缂栬緫閰嶇疆\n"
                "2) API Key锛氳缃?WEB_SEARCH_API_KEY 鎴栦紶鍏?--api-key\n"
                "3) AK/SK锛氳缃?VOLCENGINE_ACCESS_KEY 鍜?VOLCENGINE_SECRET_KEY\n"
                "寮€閫氭寚鍗楋細references/setup-guide.md 鎴?SKILL.md",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    try:
        ...
        if exc.response is not None:
            ...
            if status == 429:
                print(
                    "鎻愮ず锛氳姹傞鐜囪繃楂樿Е鍙戦檺娴侊紝寤鸿闄嶉鍚庨噸璇曘€?
                    "璇﹁ references/setup-guide.md",
                    file=sys.stderr,
                )
            elif status == 401 and ("InvalidAccessKey" in body or "invalid" in body.lower()):
                print(
                    "鎻愮ず锛欰K/SK 鏃犳晥鎴栧凡澶辨晥銆傝妫€鏌?VOLCENGINE_ACCESS_KEY / VOLCENGINE_SECRET_KEY锛?
                    "鎴栨敼鐢?API Key锛圕law 涓彲鐩存帴鍦ㄨ亰澶╂鍙戠粰鎴戯級銆傝瑙?references/setup-guide.md",
                    file=sys.stderr,
                )
            else:
    ...
    if error:
        ...
        if str(code).lower() == "invalid_api_key" or "10403" in str(code):
            print(
                "鎻愮ず锛氳纭 API Key 鏉ヨ嚜鑱旂綉鎼滅储鎺у埗鍙?https://console.volcengine.com/search-infinity/api-key 锛?
                "鑰岄潪鐏北鏂硅垷(Ark)銆傝嫢鍦?Claw 涓紝鍙噸鏂板湪鑱婂ぉ妗嗗彂姝ｇ‘鐨?Key 缁欐垜銆傝瑙?references/setup-guide.md",
                file=sys.stderr,
            )
