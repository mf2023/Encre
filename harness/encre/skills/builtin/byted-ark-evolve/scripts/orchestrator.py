...
def run_mock_worker(data_dir, review_id, signals, genes):
    ...
    if signals:
        ...
        summary = "鏍规嵁杩戞湡鍙嶉鏂板鎵ц鍓嶆鏌ヤ笌help浼樺厛瑙勫垯"
        for sig in signals[:2]:
            selected.append(
                {
                    "signal_id": sig["id"],
                    "gene_id": (gene or {}).get("id", "fallback-no-gene"),
                    "reason": "璇ユ柟妗堝彲鍑忓皯閲嶅閿欒锛屼紭鍏堢害鏉熸墽琛屽墠妫€鏌ュ拰甯姪浼樺厛銆?,
                ...
                "mutations": [
                    {
                        ...
                        "mutation_type": "add",
                        "layer": "protocol",
                        "description": "鏂板鎵ц鍓嶆鏌ヤ笌 help 浼樺厛瑙勫垯",
                        "before_text": None,
                        "after_text": "## 鎵ц鍓嶆鏌n- 鍙傛暟鎶ラ敊鍏堢湅 --help\n- 鎵ц python 鍓嶅厛妫€鏌?.venv\n",
                        "signal_ids": [s["id"] for s in signals[:2]],
                        "status": "proposed",
                        "gene_id": (gene or {}).get("id", "fallback-no-gene"),
                        "gene_reason": "濡傛灉鎻愬墠搴旂敤锛屽彲鍑忓皯褰撳墠閿欒鍐嶆鍙戠敓",
                    }
                ],
            }
        )
...
def ingest_outputs(c, data_dir, review_id):
    ...
    for proposal in prop.get("proposals", []):
        ...
        summary = proposal.get("proposal_summary") or "鏈懡鍚嶆彁妗?
...
def summarize_session_start(c):
    ...
    c.commit()
    if not rows:
        return {"pending_count": 0, "message": "褰撳墠娌℃湁鏈鐞嗙殑杩涘寲鎻愭銆?}
    lines = [f"褰撳墠鏈?{len(rows)} 缁勬湭澶勭悊鐨勮繘鍖栨彁妗堬細"]
    for idx, row in enumerate(rows, 1):
        lines.append(
            f"{idx}. {row['proposal_summary']}锛坽row['mutation_count']} 鏉″彉鏇达紝鐘舵€侊細{row['proposal_status']}锛?
        )
    lines.append("浣犲彲浠ユ帴鍙椼€佹嫆缁濓紝鎴栬€呰鎴戝睍寮€璇︽儏銆?)
