...
def classify_file(rel_path: str, full_path: Path) -> dict:
    ...
    for pattern, meta in KNOWN_PATTERNS.items():
        if rel_path.startswith(pattern):
            ...
            # Known pattern but no status override 鈥?fall through to check
            known_meta = meta
            break
    else:
        known_meta = None
...
杩欎釜绯荤粺璁?Agent 鑳戒粠浣犵殑鍙嶉涓涔狅細
  - 浣犳棩甯哥殑绾犳銆佸缓璁細琚嚜鍔ㄨ褰?  - 鏀掑涓€瀹氭暟閲忓悗锛岀郴缁熶細鍒嗘瀽骞舵彁鍑烘敼杩涙柟妗?  - 鎵€鏈夋敼鍔ㄥ繀椤荤粡浣犵‘璁ゆ墠浼氭墽琛?
鍙敤鍛戒护锛?  /evolve           鎵嬪姩瑙﹀彂涓€娆¤繘鍖栧垎鏋?  /evolve scan      鎵弿鍘嗗彶瀵硅瘽鎻愬彇鍙嶉
  /evolve dashboard 鐢熸垚杩涘寲 Dashboard
  /evolve help      鏌ョ湅瀹屾暣寮曞
""")
