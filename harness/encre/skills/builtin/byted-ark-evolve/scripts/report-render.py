...
<details style="margin-bottom:16px;background:#f8f9fa;border:1px solid var(--border);border-radius:6px;padding:10px 14px">
  <summary style="font-size:13px;cursor:pointer">馃摉 鏈閫熸煡锛堥娆￠槄璇诲缓璁睍寮€锛?/summary>
  <table style="margin-top:8px;font-size:12px">
    <tbody>
      <tr><td style="font-weight:600;width:80px">鍙嶉</td><td>浣犳棩甯哥籂姝ｃ€佸缓璁腑璇嗗埆鍑虹殑鏀硅繘绾跨储</td></tr>
      <tr><td style="font-weight:600">鍙樻洿</td><td>鍩轰簬鍙嶉璁捐鐨勫叿浣撴枃浠舵敼鍔?/td></tr>
      <tr><td style="font-weight:600">鍗忚灞?/td><td>Agent 鐨勮涓鸿鍒欏拰鍐崇瓥娴佺▼</td></tr>
      <tr><td style="font-weight:600">鑳藉姏灞?/td><td>Agent 鐨勬妧鑳藉拰鏂规硶璁?/td></tr>
      <tr><td style="font-weight:600">甯曠疮鎵樻鏌?/td><td>纭鏀瑰姩涓嶄細璁╁叾浠栨柟闈㈠彉宸?/td></tr>
      <tr><td style="font-weight:600">楗卞拰</td><td>杩炵画澶氭杩涘寲浜у嚭鏋佸皯鏂版柟妗堬紝褰撳墠鏂瑰悜宸插厖鍒嗕紭鍖?/td></tr>
      <tr><td style="font-weight:600">杞ㄨ抗</td><td>Golden = 鍋氬浜嗙殑璁板綍锛汣orrection = 鍋氶敊鍚庝慨姝ｇ殑璁板綍</td></tr>
    </tbody>
  </table>
</details>
""")
    ...
    h.append(
        f'  <div class="stat-pill"><span class="num">{signals}</span><span class="label">鏉″弽棣?/span></div>\n'
    )
    h.append(
        f'  <div class="stat-pill"><span class="num">{changes_count}</span><span class="label">鏉″彉鏇?/span></div>\n'
    )
    if rejected > 0:
        h.append(
            f'  <div class="stat-pill"><span class="num" style="color:var(--red)">{rejected}</span><span class="label">鏉℃嫆缁?/span></div>\n'
        )
    else:
        h.append(
            '  <div class="stat-pill"><span class="num" style="color:var(--green)">0</span><span class="label">鏉℃嫆缁?/span></div>\n'
        )
    h.append("</div>\n")
    ...
    if changes:
        ...
        for c in changes:
            ...
            if c.get("reason"):
                h.append(f'  <div class="reason">鍘熷洜锛歿esc(c["reason"])}</div>\n')
            # Folded diff
            before = c.get("before")
            after = c.get("after")
            if before or after:
                ...
                h.append(
                    f'      <div class="ba-box ba-before"><div class="ba-label" style="color:var(--red)">鍙樻洿鍓?/div>{esc(before or "锛堟棤锛?)}</div>\n'
                )
                h.append(
                    f'      <div class="ba-box ba-after"><div class="ba-label" style="color:var(--green)">鍙樻洿鍚?/div>{esc(after or "锛堟棤锛?)}</div>\n'
                )
                h.append("    </div>\n  </details>\n")
            h.append("</div>\n")
        ...
        if rejected_changes:
            h.append('<h2 style="color:var(--red)">琚嫆缁濈殑鍙樻洿</h2>\n')
            for c in rejected_changes:
                ...
                if c.get("reject_reason"):
                    h.append(
                        f'  <div class="reason">鎷掔粷鍘熷洜锛歿esc(c["reject_reason"])}</div>\n'
                    )
                h.append("</div>\n")

        h.append(
            f'<div class="anchor">浠ヤ笂 {len(changes)} 鏉″彉鏇磠"锛堝惈 " + str(rejected) + " 鏉¤鎷掔粷锛? if rejected > 0 else "宸插啓鍏?workspace"}</div>\n'
        )
    else:
        h.append(
            '<h2>鍙樻洿娓呭崟</h2>\n<p style="color:var(--muted)">鏈鍒嗘瀽鏈骇鐢熷彉鏇存柟妗堛€?/p>\n'
        )

    # === SIGNAL SOURCE ===
    if signals_by_layer:
        h.append("<h2>鍙嶉鏉ユ簮</h2>\n")
    ...
    if high_signals:
        h.append(
            '<p style="font-size:13px;color:var(--muted);margin-top:10px;margin-bottom:4px">楂樹紭鍏堢骇鍙嶉锛?/p>\n'
        )
    ...
    if all_signals:
        h.append(
            f'<details style="margin-top:8px"><summary>鏌ョ湅鍏ㄩ儴 {len(all_signals)} 鏉″弽棣?/summary>\n'
        )
        h.append(
    ...
    if signals_by_layer or high_signals or all_signals:
        h.append('<div class="anchor">浠ヤ笂鏄湰娆″垎鏋愮殑鍙嶉鏉ユ簮</div>\n')

    # === NEXT STEPS ===
    if next_steps:
        h.append(
            '<h2>涓嬩竴姝?/h2>\n<div class="action-box">\n<div class="action-title">楠岃瘉璁″垝</div>\n<ul style="padding-left:18px">\n'
        )
    ...
    if has_traj or has_sat:
        ...
        if has_sat:
            if sat.get("is_saturated"):
                ...
                sat_text = f'<span style="color:var(--green)">鏈ケ鍜?/span> 路 {added} 鏂板 / {modified} 淇敼'
            h.append(
    ...
    if user_direct > 0:
        cost_parts.append(f"鐢ㄦ埛鐩存帴鍙樻洿 {user_direct} 鏉?)
    if cost_parts:
        h.append(
