...
class VePhoneClient:
    ...
    def __init__(
        ...
        Args:
            access_key: 璁块棶瀵嗛挜
            secret_key: 绉樺瘑瀵嗛挜
            region: 鍖哄煙锛岄粯璁?cn-north-1
            service: 鏈嶅姟鍚嶏紝榛樿 ACEP
            version: API 鐗堟湰瑕嗙洊鍊硷紙鍙€夛紝榛樿鎸?Action 閫夋嫨锛?
            product_id: 浜у搧 ID锛堝彲閫夛紝鍙湪璋冪敤鏃舵寚瀹氾級
            dc_id: 鏈烘埧 ID锛堝彲閫夛紝鍙湪璋冪敤鏃舵寚瀹氾級
            resource_type: 涓氬姟璧勬簮绫诲瀷锛堜簯鐩?100锛屾湰鍦板瓨鍌?200锛?
            tos_bucket: TOS Bucket锛堝彲閫夛紝鐢ㄤ簬 PullFile 绛夋帴鍙ｏ級
            tos_region: TOS Region锛堝彲閫夛級
            tos_endpoint: TOS Endpoint锛堝彲閫夛級
            tos_prefix: TOS 瀵硅薄璺緞鍓嶇紑锛堝彲閫夛級
        """
    ...
    def _request(
        ...
        Args:
            action: API 鍔ㄤ綔鍚嶇О
            version: API 鐗堟湰锛堝彲閫夛紝榛樿浣跨敤鍒濆鍖栨椂鐨勭増鏈級
            body: 璇锋眰浣?
            method: HTTP 鏂规硶
    ...
    def create_pod(
        ...
        **kwargs,
        ...
        Args:
            name: 瀹炰緥鍚嶇О
            template_id: 鏈哄瀷妯℃澘 ID
            configuration_code: 濂楅浠ｇ爜
            count: 鍒涘缓鏁伴噺锛岄粯璁?1
            product_id: 浜у搧 ID锛堝彲閫夛紝榛樿浣跨敤鍒濆鍖栨椂鐨勫€硷級
            dc_id: 鏈烘埧 ID锛堝彲閫夛紝璋冪敤鏃舵寜闇€瑕佹垨璧勬簮浣欓噺鎸囧畾锛?
            **kwargs: 鍏朵粬鍙傛暟
    ...
    def start_screenshot(self, pod_id: str, **kwargs) -> Dict[str, Any]:
        """鍏煎鏃ц皟鐢紝鍐呴儴澶嶇敤 BatchScreenShot銆?""
    ...
    def list_dcs(
        ...
        **kwargs,
        ...
        Args:
            product_id: 浜у搧 ID锛堝彲閫夛紝榛樿浣跨敤鍒濆鍖栨椂鐨勫€硷級
            volc_region: 鏈烘埧鎵€鍦ㄧ墿鐞嗗尯鍩?
            region: 鏈烘埧鎵€鍦ㄥぇ鍖?ID
            isp: 缃戠粶杩愯惀鍟?ID
            server_type_code: 浜戞満瑙勬牸
            offset: 鍒嗛〉鍋忕Щ閲?
            count: 鍗曟杩斿洖鏉℃暟
