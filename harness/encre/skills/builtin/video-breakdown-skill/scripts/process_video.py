...
"""
瑙嗛棰勫鐞嗚剼鏈紙鐙珛鍙墽琛岋級銆?鑷寘鍚棰戦澶勭悊锛屾棤闇€澶栭儴鍚庣鏈嶅姟銆?闇€瑕佹湰鏈哄畨瑁?FFmpeg銆?
Usage:
...
Env:
    ...
    DATABASE_TOS_BUCKET (鎴?TOS_BUCKET), DATABASE_TOS_REGION (鎴?TOS_REGION)
"""
...
async def main(video_url: str):
    ...
    try:
        # Download
        print("[1/4] 涓嬭浇瑙嗛...", file=sys.stderr)
        ...
        # Metadata
        print("[2/4] 鎻愬彇鍏冩暟鎹?..", file=sys.stderr)
        meta = _probe_video(ffprobe_bin, local_video)
        dur = meta["duration"]
        if dur <= 0:
            print(json.dumps({"error": "鏃犳硶鑾峰彇瑙嗛鏃堕暱"}))
            return

        # Segments
        print(f"[3/4] 鏋勫缓鍒嗛暅 ({dur:.1f}s)...", file=sys.stderr)
        segments = _build_segments(dur)

        # Frames
        print("[4/4] 鎻愬彇鍏抽敭甯?..", file=sys.stderr)
