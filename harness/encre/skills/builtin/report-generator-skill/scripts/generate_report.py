...
"""
鏁村悎鍒嗛暅鎷嗚В鏁版嵁鍜岄挬瀛愬垎鏋愮粨鏋滐紝鐢熸垚 Markdown 瑙嗛鍒嗘瀽鎶ュ憡銆?
...
def generate_video_report(breakdown_data: dict, hook_analysis: dict = None) -> str:
    """
    鏁村悎鍒嗛暅鎷嗚В缁撴灉鍜岄挬瀛愬垎鏋愮粨鏋滐紝鐢熸垚 Markdown 瑙嗛鍒嗘瀽鎶ュ憡銆?

    Args:
        breakdown_data: 鍒嗛暅鎷嗚В鐨勫畬鏁寸粨鏋滄暟鎹?
        hook_analysis: 鍓嶄笁绉掗挬瀛愬垎鏋愮粨鏋滐紙鍙€夛級
...
- **闊充箰椋庢牸**: {music_style}
- **鎯呯华鍩鸿皟**: {emotion}
- **鑺傛媿**: {tempo} BPM锛坽tempo_pace}鑺傚锛?

---

# #
- **涓昏鍦烘櫙**: {primary_scene}
- **瑙嗛椋庢牸**: {video_style}
- **鐩爣鍙椾紬**: {target_audience}
...
|------|------|------|
| 瑙嗚鍐插嚮鍔?| {hook_analysis.get("visual_impact", 0)}/10 | {hook_analysis.get("visual_comment", "")} |
...
| 淇℃伅瀵嗗害 | {hook_analysis.get("information_density", 0)}/10 | {hook_analysis.get("info_comment", "")} |
| 鑺傚鎺屾帶 | {hook_analysis.get("rhythm_control", 0)}/10 | {hook_analysis.get("rhythm_comment", "")} |"""
...
def _build_platform_section(scene: dict) -> str:
    ...
    for rec in recommendations:
        ...
        lines.append(f"- **{platform}**锛堥€傚悎搴? {suitability}锛? {reason}")
    return "\n".join(lines)
...
def _build_segments_overview(segments: list) -> str:
    ...
    if len(segments) > 10:
        lines.append(f"\n*锛堜粎灞曠ず鍓?0涓垎闀滐紝鍏眥len(segments)}涓級*")
