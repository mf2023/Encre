...
def _get_sub_agent_enforcement() -> str:
    ...
    that all sub-agents 鈥?regardless of their role .prompt file 鈥?receive:
    ...
    async def run(
        ...
        * ``event_callback(event)`` is awaited on every raw AgentEvent
        ...
        def _stamp_error_on_session(err_detail: str, err_code: str = "", err_category: str = "") -> None:
            ...
            these AFTER its ``yield create_finish(...)`` 鈥?which never runs for
            sub-agents because SubAgentRunner stops consuming at Finish 鈥?so
            ...
            async for event in sub_agent.run(prompt=prompt, system_prompt=system_prompt or None):
                ...
                if isinstance(event, TextDelta):
                    ...
                    if event.reason == "error":
                        ...
                        return {
                            "content": (
                                f"Error: Sub-agent failed{(' 鈥?' + err_detail) if err_detail else ''}"
                            ),
                            "error": True,
