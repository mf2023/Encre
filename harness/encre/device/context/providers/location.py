...
"""Location info provider 鈥?timezone + native OS geolocation.
...
from encre.device.context.base import DeviceProvider
...
class LocationInfoProvider(DeviceProvider):
    ...
    def collect(self) -> dict[str, Any] | None:
        """Synchronous wrapper 鈥?runs the async collection on a fresh loop."""
        try:
