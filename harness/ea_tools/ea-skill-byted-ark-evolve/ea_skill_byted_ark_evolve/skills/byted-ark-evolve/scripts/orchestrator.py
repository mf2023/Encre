...
def run_mock_worker(data_dir, review_id, signals, genes):
    ...
    if signals:
        ...
        summary = "鏍规嵁杩戞湡鍙嶉鏂板鎵ц鍓嶆鏌ヤ笌help浼樺厛瑙勫垯"
        for sig in signals[:2]:
            selected.append(
                {
                    "signal_id": sig["id"],
                    "gene_id": (gene or {}).get("id", "fallback-no-gene"),
                    "reason": "璇ユ柟妗堝彲鍑忓皯閲嶅閿欒锛屼紭鍏堢害鏉熸墽琛屽墠妫鏌ュ拰甯姪浼樺厛銆?",
                ...
                "mutations": [
                    {
                        ...
                        "mutation_type": "add",
                        "layer": "protocol",
                        "description": "鏂板鎵ц鍓嶆鏌ヤ笌 help 浼樺厛瑙勫垯",
                        "before_text": None,
                        "after_text": "## 鎵ц鍓嶆鏌n- 鍙傛暟鎶ラ敊鍏堢湅 --help\n- 鎵ц python 鍓嶅厛妫鏌?.venv\n",
                        "signal_ids": [s["id"] for s in signals[:2]],
                        "status": "proposed",
                        "gene_id": (gene or {}).get("id", "fallback-no-gene"),
                        "gene_reason": "濡傛灉鎻愬墠搴旂敤锛屽彲鍑忓皯褰撳墠閿欒鍐嶆鍙戠敓",
                    }
                ],
            }
        )
...
def ingest_outputs(c, data_dir, review_id):
    ...
    for proposal in prop.get("proposals", []):
        ...
        summary = proposal.get("proposal_summary") or "鏈懡鍚嶆彁妗?"
...
def summarize_session_start(c):
    ...
    c.commit()
    if not rows:
        return {"pending_count": 0, "message": "褰撳墠娌湁鏈鐞嗙殑杩涘寲鎻愭銆?"}
    lines = [f"褰撳墠鏈?{len(rows)} 缁勬湭澶勭悊鐨勮繘鍖栨彁妗堬細"]
    for idx, row in enumerate(rows, 1):
        lines.append(
            f"{idx}. {row['proposal_summary']}（{row['mutation_count']} 鏉彉鏇达紝鐘舵侊細{row['proposal_status']}（"
        )
    lines.append("浣犲彲浠ユ帴鍙椼佹嫆缁濓紝鎴栬呰鎴戝睍寮璇︽儏銆?")
