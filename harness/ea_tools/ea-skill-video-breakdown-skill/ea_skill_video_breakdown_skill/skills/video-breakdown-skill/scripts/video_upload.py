...
"""
涓婁紶鏈湴瑙嗛鏂囦欢鍒扮伀灞卞紩鎿?TOS 瀵硅薄瀛樺偍锛岃繑鍥炵鍚?URL銆?
...
def video_upload_to_tos(file_path: str, bucket_name: str = None) -> dict:
    """
    灏嗘湰鍦拌棰戞枃浠朵笂浼犲埌 TOS锛岃繑鍥炵鍚?URL銆?

    Args:
        file_path: 鏈湴瑙嗛鏂囦欢璺緞
        bucket_name: TOS 瀛樺偍妗跺悕绉帮紙鍙夛級
    ...
    if not os.path.isfile(file_path):
        return {"error": f"璺緞涓嶆槸鏂囦欢: {file_path}"}

    file_size = os.path.getsize(file_path)
    max_size = 2 * 1024 * 1024 * 1024  # 2GB
    if file_size > max_size:
        return {"error": f"File too large ({file_size / 1024 / 1024:.0f}MB), max supported 2GB"}
    ...
    if not access_key or not secret_key:
        return {
            "error": "缂哄皯 TOS 璁块棶鍑瘉锛岃璁剧疆 VOLCENGINE_ACCESS_KEY 鍜?VOLCENGINE_SECRET_KEY"
        }
    ...
    try:
        ...
        try:
            ...
            if e.status_code == 404:
                return {"error": f"TOS bucket {bucket_name} does not exist"}
            raise
        ...
        return {
            ...
            "message": "涓婁紶鎴愬姛锛佷娇鐢?video_url 璋冪敤 process_video.py 杩涜瑙嗛鍒嗛暅鍒嗘瀽",
        }

    except tos.exceptions.TosClientError as e:
        return {"error": f"TOS 瀹埛绔敊璇? {e}"}
    except tos.exceptions.TosServerError as e:
        return {"error": f"TOS 鏈嶅姟绔敊璇? {e.message}"}
