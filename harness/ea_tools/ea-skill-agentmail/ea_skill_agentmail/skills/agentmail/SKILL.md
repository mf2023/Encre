...
3. **Install Python SDK**: `bash pip install agentmail python-dotenv`
...
See [WEBHOOKS.md](external documentation) for complete webhook setup guide including ngrok for local development.
...
1. **Create allowlist filter** at `~/.encre/hooks/email-allowlist.ts`:
...
2. **Update Clawdbot config** (`~/.encre/clawdbot.json`):

```json
{
  "hooks": {
    "transformsDir": "~/.encre/hooks",
    "mappings": [
      {
        "id": "agentmail",
...
- **the tool** - Send emails with rich content and attachments
- **the tool** - Poll inbox for new messages
- **the tool** - Configure webhook endpoints for real-time processing

## References

- **[API.md](external documentation)** - Complete API reference and endpoints
- **[WEBHOOKS.md](external documentation)** - Webhook setup and event handling
- **[EXAMPLES.md](external documentation)** - Common patterns and use cases
