---
name: gmail-cleaner
description: Clean and organize Gmail accounts in bulk.
metadata:
  source: encre
  tags: 
user_invocable: true
...
- Default token path: `~/.Encre/workspace/gmail_token.pkl`
- Default creds path: `~/.Encre/workspace/gmail_credentials.json`
...
```bash
python scripts/scan.py    --token ~/.Encre/workspace/gmail_token_personal.pkl
python scripts/scan.py    --token ~/.Encre/workspace/gmail_token_work.pkl
python scripts/deep_clean.py --token ~/.Encre/workspace/gmail_token_work.pkl
```
...
- a script samples N messages; large inboxes may need `--sample 2000` for accuracy
