...
"""
鍗曞厓娴嬭瘯鑷姩鐢熸垚 Skill 鈥斺?鏍稿績鐢熸垚鍣?
绉戝ぇ璁 AI 鏁版嵁鏅鸿兘鍒嗘瀽涓庡簲鐢?Skill 寮鍙戞寫鎴樿禌 路 鏂瑰悜浜屻屾祴璇曚笌璐ㄩ噺淇濋殰銆?

杈撳叆锛歅ython 婧愮爜鏂囦欢锛堣矾寰勶級
杈撳嚭锛歱ytest 娴嬭瘯鏂囦欢 + Markdown 鎶ュ憡 +锛堝彲閫夛級pytest 杩愯缁撴灉

璁捐鍘熷垯锛氶珮椴佹鎬?
- 鍩轰簬 ast 闈欐佽В鏋愶紝澶嶆潅/娈嬬己浠ｇ爜涓嬪缁堜骇鍑恒愯娉曞悎娉曘戠殑娴嬭瘯鏂囦欢
- 鐩爣妯潡瀵煎叆澶辫触 / 璇硶閿欒 鈫?鐢熸垚"鍙繍琛屼絾鍏ㄩ儴 skip"鐨勬祴璇?+ 娓呮櫚鎶ュ憡锛岀粷涓嶅穿婧?
- 鐢熸垚鐨勬祴璇曞浠舵案杩滆兘璺戣捣鏉ワ紙鏃?collection error锛夛紝绗悎璧涗簨"鏃犺緭鍑哄穿婧?纭寚鏍?
"""
import ast
import argparse
import os
...
MAX_FUNCS = 60  # 鍗曟枃浠跺嚱鏁?鏂规硶涓婇檺锛岃秴鍑烘埅鏂苟鍦ㄦ姤鍛婁腑璇存槑
...
def param_value(name, ann, default_node):
    """happy-path 鍙栧硷細浼樺厛榛樿鍊硷紝鍏舵鎸夋敞瑙?鍚嶅瓧鎺ㄦ柇瀹夊叏 dummy銆"""
    ...
    for suffix, use_edge in (("happy", False), ("boundary", True)):
        ...
        if suffix == "happy":
            body = (
                ...
                "        pytest.skip(f\"鐩爣妯潡瀵煎叆澶辫触: {_IMPORT_ERROR}\")\n"
                "    %s\n"
                "    # TODO: 琛ュ厖閽堝杩斿洖鍊肩殑涓氬姟鏂█锛堝綋鍓嶄负鍐掔儫娴嬭瘯锛岄獙璇佷笉鎶涘紓甯革級\n"
                "    assert True\n" % call
            )
        else:
            body = (
                ...
                "        %s\n"
                "    except Exception:\n"
                "        pytest.skip(\"杈圭晫杈撳叆瑙彂寮傚父锛岄渶浜哄伐纭鏄惁搴斾负棰勬湡琛屼负\")\n"
                "    # TODO: 琛ュ厖杈圭晫鏂█\n"
                "    assert True\n" % call
            )
...
def gen_class_tests(t, mod_name):
    ...
    out.append(
        ...
        "    inst = _make_%s()\n"
        "    if inst is None:\n"
        "        pytest.skip(\"%s could not be instantiated (check constructor params)\")\n"
    ...
    for m in t["methods"]:
        ...
        body = (
            "    inst = _make_%s()\n"
            "    if inst is None:\n"
            "        pytest.skip(\"%s instantiation failed; add constructor params\")\n"
            "    %s\n"
            "    # TODO: 琛ュ厖閽堝杩斿洖鍊肩殑涓氬姟鏂█\n"
            "    assert True\n" % (cname, cname, call)
        )
...
def build_test_file(targets, target_path, test_path):
    ...
    parts = []
    parts.append('"""Automatically generated by pytest-forge (绉戝ぇ璁娴嬭瘯涓庤川閲忎繚闅滆禌閬?."""')
...
def main():
    ap = argparse.ArgumentParser(description="Unit test auto generator")
    ap.add_argument("-i", "--input", required=True, help="Python source file path")
    ap.add_argument("-o", "--out", default="./ut_output", help="Output dir (default ./ut_output)")
    ap.add_argument("--no-run", action="store_true", help="Generate only; do not run pytest")
    args = ap.parse_args()
    ...
    try:
        ...
        placeholder = (
            ...
            "    pytest.skip(\"鐩爣婧愮爜瀛樺湪璇硶閿欒锛屾棤娉曡В鏋? %s\")\n" % e
        )
        with open(test_path, "w", encoding="utf-8") as f:
            f.write(placeholder + "\n")
        stats["parse_ok"] = False
        write_report(report_path, stats,
                     "Target source syntax error (line %s): %s. Placeholder tests were generated and all skip; the source stays broken." % (e.lineno, e.msg), enc=enc)
        print("[OK] 宸查檷绾х敓鎴愬崰浣嶆祴璇曪紙婧愮爜璇硶閿欒锛? %s" % test_path)
    ...
    print("[OK] 鎶ュ憡: %s" % report_path)
    print("[INFO] funcs %d / classes %d / methods %d / generated cases %d (parse %s, run %s)"
...
def write_report(path, stats, run_note, enc):
    ...
    lines.append("")
    lines.append("> 绉戝ぇ璁 AI 鏁版嵁鏅鸿兘鍒嗘瀽涓庡簲鐢?Skill 寮鍙戞寫鎴樿禌 路 鏂瑰悜浜屻屾祴璇曚笌璐ㄩ噺淇濋殰銆?")
    lines.append("")
    lines.append("## 姒傝")
    lines.append("")
    lines.append("- **杈撳叆鏂囦欢**锛歚%s`" % stats["input"])
    lines.append("- **婧愮爜缂栫爜**锛?s" % (enc or "鏈煡"))
    ...
    lines.append("- **Truncated**: %s" % ("yes (over %d units, took first %d)" % (MAX_FUNCS, MAX_FUNCS) if stats["truncated"] else "no"))
    lines.append("- **pytest run**: %s" % (stats["run"] or "not executed"))
    lines.append("")
    lines.append("## 鐢熸垚鐨勬祴璇曟枃浠?")
    lines.append("")
    lines.append("`test_<妯潡鍚?.py` 宸茬敓鎴愪簬杈撳嚭鐩綍锛屽寘鍚細")
    lines.append("- 姣忎釜鍑芥暟锛歚test_<func>_happy`锛堜富璺緞鍐掔儫锛? `test_<func>_boundary`锛堣竟鐣屽啋鐑燂級")
    lines.append("- 姣忎釜绫伙細`test_<Class>_init`锛堝疄渚嬪寲锛? 姣忎釜鏂规硶 `test_<Class>_<method>_happy`")
    lines.append("- 鐩爣妯潡閫氳繃 `importlib` 鎸夎矾寰勫姞杞斤紝瀵煎叆澶辫触鍒欏叏閮?`skip`锛?*濂椾欢姘歌繙鍙繍琛屻佷笉宕╂簝**")
    lines.append("")
    lines.append("## 椴佹鎬ц鏄?")
    lines.append("")
    lines.append("- 澶嶆潅/娈嬬己浠ｇ爜涓嬩粛浜у嚭璇硶鍚堟硶鐨勬祴璇曟枃浠?")
    lines.append("- 鐩爣妯潡璇硶閿欒鎴栧鍏ュけ璐?鈫?鐢熸垚銆屽叏閮?skip銆嶅崰浣嶆祴璇曪紝涓嶆姤閿欓鍑?")
    lines.append("- 鐢熸垚鐨勭敤渚嬩负**鍐掔儫娴嬭瘯楠ㄦ灦**锛屾柇瑷澶勬爣娉?`TODO`锛岄渶浜哄伐琛ュ厖涓氬姟鏂█")
    lines.append("- 鍗曟枃浠跺嚱鏁颁笂闄?%d锛岃秴鍑烘埅鏂苟鍦ㄦ湰鎶ュ憡涓彁绀? % MAX_FUNCS")
    lines.append("")
    if run_note:
        lines.append("## pytest 杩愯缁撴灉")
        lines.append("")
        lines.append("```")
