...
绉戝ぇ璁 AI 鏁版嵁鏅鸿兘鍒嗘瀽涓庡簲鐢?Skill 寮鍙戞寫鎴樿禌 路 鏂瑰悜浜屻屾祴璇曚笌璐ㄩ噺淇濋殰銆?

杈撳叆锛歅ython 婧愮爜鏂囦欢 + 锛堝彲閫夌殑锛夌幇鏈夋祴璇曟枃浠?鐩綍
杈撳嚭锛氱洸鍖哄垎鏋愭姤鍛?report.md + 鍙洿鎺ヨ繍琛岀殑缂哄彛娴嬭瘯楠ㄦ灦 test_<妯潡>_gaps.py

涓?generate_tests.py 褰垚銆岀粍鍚堟嫵銆嶏細
  1) generate_tests.py 浠庨浂閾洪噺锛堜负鎵鏈夊嚱鏁扮敓鎴愬啋鐑熸祴璇曪級
  2) analyze_coverage.py 鏌ユ紡琛ョ己锛堟壘鍑虹幇鏈夋祴璇曚腑鏈鐩栫殑鍑芥暟/鏂规硶锛屾寜椋庨櫓鎺掑簭锛屽苟鑷姩涓虹己鍙ｈˉ楠ㄦ灦锛?

璁捐鍘熷垯锛氶珮椴佹鎬?
- 涓昏矾寰勭敤 coverage.py 瀹炴祴琛岃鐩栵紱coverage 涓嶅彲鐢?娴嬭瘯杩愯澶辫触鏃惰嚜鍔ㄩ檷绾т负闈欐佸悕绉板尮閰?
- 鏈彁渚涗换浣曟祴璇?鈫?瑙嗕负鍏ㄩ儴鐩插尯锛堢瓑浠蜂簬鎻愮ず鐢?generate_tests.py 浠庨浂鐢熸垚锛夛紝涓嶅穿婧?
- 澶嶇敤 generate_tests.build_test_file 鐢熸垚缂哄彛楠ㄦ灦锛屼繚璇侀鏋舵案杩滆娉曞悎娉曘佸彲杩愯
- 鎵鏈夊紓甯歌鍚炴帀骞剁粰鍑哄彲璇绘彁绀猴紝宸ュ叿鑷韩闆跺穿婧?
"""
import ast
import json
import os
...
def find_test_files(tests_path):
    """杩斿洖娴嬭瘯鏂囦欢璺緞鍒楄锛泃ests_path 鍙负鏂囦欢鎴栫洰褰曘"""
...
def coverage_run(src_abs, tests_path, out_dir):
    """鐢?coverage.py锛圥ython API锛夌簿纭祴閲忓崟涓簮鐮佹枃浠剁殑琛岃鐩栥

    Key points:
    - Use `include=[src_abs]` to lock onto the file under test and avoid pulling
      sibling files (possibly containing syntax errors) into coverage.
    - Run `cov.start()` -> `pytest.main()` -> `cov.stop()` in-process, avoiding
      the subprocess-discovery semantics of `coverage run -m pytest`; most accurate.

    Returns (executed_set, missing_set) or None (downgraded by caller on failure).
    """
    try:
        import coverage
...
def reason_text(u, ratio):
    ...
    if u["branches"] >= 3:
        ...
        parts.append("%d branches" % u["branches"])
    else:
        parts.append("no branches")
    parts.append("鍏紑API" if u["exposure"] >= 1.0 else "鍐呴儴鏂规硶")
    return ", ".join(parts)
...
def write_report(path, stats, units, covered_map, ratio_map, blind_sorted,
    ...
    L = []
    L.append("# Test Coverage Blind-Spot Analysis Report (pytest-forge)")
    L.append("")
    L.append("> Placeholder: coverage analysis summary follows.")
    L.append("")
    L.append("## 姒傝")
    L.append("")
    L.append("- **婧愮爜鏂囦欢**锛歚%s`" % stats["input"])
    L.append("- **Existing tests**: %s" % ("`%s`" % tests_path if tests_path else "none provided (all blind spots)"))
    L.append("- **鍒ゅ畾妯紡**锛?s" % mode_used)
    L.append("- **Testable units**: %d (functions %d / methods %d)"
             % (stats["units"], stats["funcs"], stats["methods"]))
    cov_n = sum(1 for u in units if covered_map[u["qual"]])
    L.append("- **宸茶鐩?*锛?d銆**鏈鐩栵紙鐩插尯锛?*锛?d銆**鍗曞厓瑕嗙洊鐜?*锛?.0f%%"
             % (cov_n, len(units) - cov_n, (cov_n / max(1, len(units)) * 100)))
    L.append("- **Truncated**: %s"""
             % ("yes (over %d units, took first %d)" % (gen.MAX_FUNCS, gen.MAX_FUNCS) if stats["truncated"] else "no"))
    L.append("")
    ...
    for u in units:
        ...
        st = "covered" if covered_map[u["qual"]] else "blind-spot"
        L.append("| `%s` | %s | %s | %d | %.0f%% | %s |"
                 % (u["qual"], typ, rng, u["branches"], ratio_map[u["qual"]] * 100, st))
    L.append("")

    # 
    L.append("## 鐩插尯琛ユ祴浼樺厛绾э紙鎸夐闄╂帓搴忥級")
    L.append("")
    if not blind_sorted:
        L.append("馃帀 鏈彂鐜扮洸鍖猴紝鐜版湁娴嬭瘯宸茶鐩栧叏閮ㄥ彲娴嬭瘯鍗曞厓銆?")
    else:
        ...
        L.append("")
        L.append("> 璇勫垎 = (1 鈭?瑕嗙洊鐜? 脳 (1 + 0.15脳鍒嗘敮鏁? 脳 鍏紑搴︽潈閲嶏紙鍏紑 1.0 / 鍐呴儴 0.4锛夈?"
                 "鍒嗗艰秺楂樿秺搴斾紭鍏堣ˉ娴嬨?")
    L.append("")
    ...
    L.append("")
    if gap_test_rel:
        L.append("- 宸茶嚜鍔ㄧ敓鎴愬彲鐩存帴杩愯鐨勭己鍙ｉ鏋讹細`%s`" % gap_test_rel)
        L.append("- 璇ユ枃浠跺鐢ㄤ笌 `generate_tests.py` 鐩稿悓鐨勯鏋舵ā鏉匡紙happy + boundary 鍐掔儫娴嬭瘯锛夛紝"
                 "寮鍙戣呭彧闇鍦?`TODO` 澶勮ˉ鍏呬笟鍔柇瑷鍗冲彲銆?")
    else:
    ...
    L.append("## 椴佹鎬ц鏄?")
    L.append("")
    L.append("- 瑕嗙洊鍒ゅ畾浼樺厛閲囩敤 `coverage.py` 瀹炴祴琛岃鐩栵紱涓嶅彲鐢ㄦ椂鑷姩闄嶇骇涓洪潤鎬佸悕绉板尮閰嶏紝缁濅笉宕╂簝")
    L.append("- 鏈彁渚涙祴璇曟枃浠舵椂锛屽叏閮ㄥ崟鍏冨垽涓虹洸鍖哄苟鎻愮ず鍙敤 `generate_tests.py` 浠庨浂鐢熸垚")
    L.append("- 缂哄彛楠ㄦ灦澶嶇敤 `generate_tests.build_test_file`锛屼繚璇佽娉曞悎娉曘佸彲杩愯銆佸鍏ュけ璐ュ垯 skip")
    L.append("- 鍗曟枃浠跺崟鍏冧笂闄?%d锛岃秴鍑烘埅鏂苟鍦ㄦ湰鎶ュ憡鎻愮ず" % gen.MAX_FUNCS)
    L.append("")
    L.append("---")
...
def main():
    ...
    ap.add_argument("-i", "--input", required=True, help="Python 婧愮爜鏂囦欢璺緞")
    ap.add_argument("-t", "--tests", default=None,
                    help="鐜版湁娴嬭瘯鏂囦欢鎴栫洰褰曪紙鍙夛紱鐪佺暐鍒欏叏閮ㄨ涓虹洸鍖猴級")
    ap.add_argument("-o", "--out", default="./coverage_report", help="杈撳嚭鐩綍锛堥粯璁?./coverage_report锛?")
    ap.add_argument("--mode", choices=["auto", "coverage", "static"], default="auto",
                    help="瑕嗙洊鍒ゅ畾妯紡锛堥粯璁?auto锛氫紭鍏?coverage锛屽け璐ラ檷绾?static锛?")
    args = ap.parse_args()
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            f.write("# 娴嬭瘯瑕嗙洊鐜囩洸鍖哄垎鏋愭姤鍛奬n\n"
                    "> 婧愮爜瀛樺湪璇硶閿欒锛堣 %s锛夛細%s銆傛棤娉曡В鏋愭彁鍙栧崟鍏冿紝璇峰厛淇婧愮爜銆俓n"
                    % (e.lineno, e.msg))
        print("[ERROR] 婧愮爜璇硶閿欒锛屾棤娉曞垎鏋? %s" % e, file=sys.stderr)
        return 2
    ...
    print("[INFO] 鍗曞厓 %d锛堝嚱鏁?%d / 鏂规硶 %d锛夛綔宸茶鐩?%d锝滅洸鍖?%d锝滄ā寮?%s"
          % (stats["units"], stats["funcs"], stats["methods"],
