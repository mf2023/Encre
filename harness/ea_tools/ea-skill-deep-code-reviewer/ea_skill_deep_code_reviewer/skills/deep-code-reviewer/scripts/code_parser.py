...
class CodeParser:
    """Multi-language code parser that segments source into functions/classes.

    Engineering design: Uses regex-based heuristics to identify language
    boundaries, then splits code into semantic segments (functions, classes,
    statements) for downstream analysis by security/performance/semantic scanners.
    """
    ...
    def parse(self, code: str, filename: Optional[str] = None) -> Dict:
        ...
        Args:
            ...
            filename: Optional filename for language detection via extension.

        Returns:
            Dict with status, language, segments, metadata, and warnings.
        """
        if not code or not code.strip():
            return {"status": "failed", "error": "鏈娴嬪埌鏈夋晥浠ｇ爜", "language": None}
    ...
    def _detect_language(self, code: str, filename: Optional[str]) -> str:
        """Detect programming language from filename extension or code signatures."""
        # 
        if filename:
            ext = filename.lower()
    ...
    def _split_segments(self, code: str, language: str) -> List[Dict]:
        """Split code into semantic segments based on language-specific rules."""
        segments = []
        lines = code.split("\n")
    ...
    def _extract_metadata(self, code: str, language: str) -> Dict:
        """Extract code metadata: imports, function/class counts, comment ratio."""
        metadata = {
            "imports": [],
            "functions_count": 0,
            "classes_count": 0,
...
# Public factory function for code parsing
