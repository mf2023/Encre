        ...
        for i, line in enumerate(lines):
            ...
            if len(loop_stack) >= 3:
                issues.append({
                    ...
                    "description": f"妫娴嬪埌{len(loop_stack)}灞傚祵濂楀惊鐜紝鏃堕棿澶嶆潅搴彲鑳戒负O(n^{len(loop_stack)})",
                    "fix": "鑰冭檻浣跨敤鍝堝笇琛ㄣ佹帓搴?鍙屾寚閽堛佸垎娌荤瓑绠楁硶浼樺寲锛屾垨鎻愬彇寰幆鍐呬笉鍙橀噺",
            ...
            for match in re.finditer(r"def\s+(\w+)\s*\([^)]*\).*?:.*?\n(?:.*\n)*?\s+\1\s*\(", code):
                ...
                if "lru_cache" not in func_block and "memo" not in func_block:
                    ...
                    issues.append({
                        ...
                        "description": "妫娴嬪埌閫掑綊鍑芥暟浣嗘湭浣跨敤璁板繂鍖栵紝鍙兘瀵艰嚧鎸囨暟绾ф椂闂村鏉傚害",
                        "fix": "浣跨敤functools.lru_cache鎴栨墜鍔ㄥ疄鐜拌蹇嗗寲瀛楀吀",
            ...
            for pattern in patterns:
                for match in re.finditer(pattern, code, re.DOTALL):
                    ...
                    issues.append({
                        ...
                        "description": "妫娴嬪埌寰幆鍐呮墽琛屾暟鎹簱鏌ヨ锛屼骇鐢烴+1鏌ヨ闂",
            ...
            for pattern in patterns:
                for match in re.finditer(pattern, code, re.DOTALL):
                    ...
                    issues.append({
                        ...
                        "line": line_num,
                        "code": match.group(0).split("\n")[0][:60],
                        "description": "妫娴嬪埌寰幆涓寔缁疮绉暟鎹埌鍒楄/瀛楀吀锛屽彲鑳藉鑷村唴瀛樻孩鍑?",
                        "fix": "浣跨敤鐢熸垚鍣▂ield浠ｆ浛鍒楄銆佸垎鎵瑰鐞嗐佹垨瀹氭湡娓呯悊缂撳瓨",
        ...
        if language in ["javascript", "typescript"]:
            for match in re.finditer(r"setInterval\s*\(\s*function", code):
                ...
                issues.append({
                    ...
                    "description": "妫娴嬪埌setInterval/setTimeout鍙兘鏈竻鐞嗭紝瀵艰嚧鍐呭瓨娉勬紡",
        ...
        if language == "python":
            ...
            for match in re.finditer(r"for\s+\w+\s+in\s+[^:]+:.*?\n(?:.*\n)*?\s+\w+\s*\+\s*=", code, re.DOTALL):
                ...
                issues.append({
                    ...
                    "line": line_num,
                    "code": match.group(0).split("\n")[-1].strip()[:60],
                    "description": "寰幆涓娇鐢?=鎷兼帴瀛楃涓诧紝鏃堕棿澶嶆潅搴(n^2)",
                    "fix": "浣跨敤鍒楄join鎴朣tringIO",
        ...
        for func in funcs:
            for match in re.finditer(rf"\b{re.escape(func)}\s*\(", code):
                ...
                issues.append({
                    ...
                    "line": line_num,
                    "code": match.group(0)[:60],
                    "description": f"妫娴嬪埌鍚屾闃诲璋冪敤{func}锛屽湪楂樺苟鍙戝満鏅細褰卞搷鎬ц兘",
                    "fix": "浣跨敤寮傛鏇夸唬鏂规锛坅syncio/aiohttp銆丳romise/async-await銆丯IO锛?",
        ...
        for pattern in patterns:
            for match in re.finditer(pattern, code, re.DOTALL):
                ...
                issues.append({
                    ...
                    "line": line_num,
                    "code": match.group(0).split("\n")[-1].strip()[:60],
                    "description": "寰幆涓噸澶嶈绠椾笉鍙橀噺锛堝len()锛夛紝鍙彁鍙栧埌寰幆澶?",
                    "fix": "灏嗕笉鍙橀噺鎻愬彇鍒板惊鐜",
                    "example": "n = len(arr)\nfor i in range(n): ..."
                })
        ...
        for pattern, desc in slow_patterns:
            for match in re.finditer(pattern, code, re.IGNORECASE):
                ...
                issues.append({
                    ...
                    "line": line_num,
                    "code": match.group(0)[:60],
                    "description": f"妫娴嬪埌{desc}锛屽彲鑳藉鑷村叏琛ㄦ壂鎻忔垨鎬ц兘涓嬮檷",
                    "fix": "鎸囧畾鎵闇瀛楁銆佷娇鐢ㄨ鐩栫储寮曘佹敼鐢ㄦ父鏍囧垎椤垫垨ES鎼滅储",
