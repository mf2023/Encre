...
def ark_api_request(action, method="POST", body=None, params=None):
    """
    閫氱敤ARK API璇锋眰鍑芥暟锛岃嚜鍔ㄥ鐞嗙鍚?
    :param action: API鎿嶄綔鍚嶇О锛屼緥濡?ListFoundationModels
    :param method: HTTP鏂规硶锛岄粯璁OST
    :param body: 璇锋眰浣撳瓧鍏?
    :param params: URL鏌ヨ鍙傛暟瀛楀吀
    :return: 鍝嶅簲JSON瀛楀吀
    """
    #  浠庣幆澧冨彉閲廏et credentials淇伅
    ...
    if not ak or not sk:
        raise ValueError("璇烽厤缃甐OLCENGINE_ACCESS_KEY鍜孷OLCENGINE_SECRET_KEY鐜鍙橀噺")
...
def list_foundation_models(args):
    ...
    try:
        ...
        print(f"鏌ヨ鍒?{total_count} 涓熀纭妯瀷锛堝綋鍓嶉〉{len(items)}涓級:")
        print("-" * 150)
        print(f"{'Model Name':<30} {'Version':<15} {'Description'}")
        print("-" * 150)

        for item in items:
            model_name = item.get("Name", "鏈煡")
            primary_version = item.get("PrimaryVersion", "鏈煡")
...
def list_foundation_model_versions(args):
    ...
    try:
        ...
        print(f"鍩虹妯瀷: {args.model_name}")
        # SDK
        items = getattr(resp, "Items", getattr(resp, "items", []))
        total = len(items)
        print(f"鎬诲叡鏈?{total} 涓増鏈?")
        print("-" * 100)
        print(f"{'Version':<25} {'Status':<10} {'Created':<25} {'Description'}")
        print("-" * 100)
        ...
        for item in items:
            ...
            model_version = item_dict.get(
                "ModelVersion", item_dict.get("model_version", "鏈煡")
            )
            status = item_dict.get("Status", item_dict.get("status", "鏈煡"))
            create_time = item_dict.get(
                "CreateTime", item_dict.get("create_time", "鏈煡")
            )
