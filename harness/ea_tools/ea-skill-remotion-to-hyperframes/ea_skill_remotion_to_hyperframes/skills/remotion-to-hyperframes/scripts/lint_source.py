...
Warnings (translate but flag 鈥?drop the construct, keep the rest):
  - r2hf/lambda-import        @remotion/lambda configuration 鈥?drop, HF is single-machine
  - r2hf/delay-render         delayRender() 鈥?HF handles asset loading differently
  - r2hf/use-callback         useCallback 鈥?usually decorative, drop
  - r2hf/use-memo             useMemo 鈥?usually decorative, drop
  - r2hf/custom-hook          Custom hook (use*) defined locally; may need manual rewrite

Info (translate and document):
  - r2hf/static-file          staticFile("x") 鈥?convert to relative path
  - r2hf/interpolate-colors   interpolateColors 鈥?translate to GSAP color tween
"""
...
def _custom_hook(src: str) -> Iterable[tuple[int, str | None]]:
    for m in _CUSTOM_HOOK_DECL.finditer(src):
        ...
        if name in _REMOTION_BUILTIN_HOOKS:
            continue
        yield m.start(), f"Custom hook `{name}` defined locally 鈥?may need manual rewrite"
...
def _third_party_react_ui(src: str) -> Iterable[tuple[int, str | None]]:
    for m in _IMPORT_FROM.finditer(src):
        ...
        if any(pkg.startswith(p) for p in THIRD_PARTY_UI_PACKAGES):
            yield m.start(), f"Imports `{pkg}` 鈥?third-party React UI library has no HF equivalent"
...
RULES: list[Rule] = [
    Rule(
        ...
        "useState detected 鈥?Remotion compositions that drive animation via React state are not deterministic frame-capture targets in HyperFrames",
    ...
    Rule(
        ...
        "useReducer detected 鈥?same issue as useState",
    ...
    Rule(
        ...
        "useEffect/useLayoutEffect with non-empty deps 鈥?side effects don't translate to HF's seek-driven model",
    ...
    Rule(
        ...
        "calculateMetadata returns a Promise 鈥?HF needs composition metadata up front",
    ...
    Rule(
        ...
        "Imports a third-party React UI library 鈥?no HF equivalent",
    ...
    Rule(
        ...
        "@remotion/lambda is Remotion-specific distributed rendering 鈥?no HF equivalent today",
    ...
    Rule(
        ...
        "delayRender() 鈥?HF waits on asset readiness via the Frame Adapter pattern",
    ...
    Rule(
        ...
        "useCallback 鈥?typically decorative for render performance, no HF equivalent needed",
    ...
    Rule(
        ...
        "useMemo 鈥?typically decorative, no HF equivalent needed",
    ...
    Rule(
        ...
        WARNING,
        _custom_hook,
        "Custom hook defined locally 鈥?may need manual rewrite",
    ...
    Rule(
        ...
        "staticFile() reference 鈥?convert to a relative path in the HF composition",
    ...
    Rule(
        ...
        "interpolateColors() 鈥?translate to a GSAP color tween",
...
def _find_matching_paren(src: str, open_idx: int) -> int | None:
    ...
    template-literal interpolations `${...}` recursively or comments 鈥?both
...
def main() -> int:
    ...
    if args.json:
        ...
        print()
        print(f"{len(files)} files scanned 路 {blockers} blocker 路 {warnings} warning 路 {infos} info")
        if blockers:
