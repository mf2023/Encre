...
async def seedream_generate(
    ...
    if version != "5.0":
        version_note += " 銆怤ote: This version does not support tools and output_format parameters銆?"
...
def list_versions():
    ...
    for version, desc in VERSION_DESCRIPTIONS.items():
        ...
        if version != "5.0":
            print(f"     鈿狅笍  Not supported: tools, output_format")
        else:
            print(f"     猸?Only supported: tools, output_format")
        print("")
...
def main():
    ...
    if args.version == "5.0":
        ...
        if args.output_format != "jpeg":
            print(f"鈿狅笍  Warning: {args.version} version does not support output_format parameter, will be ignored")
        if args.web_search:
            print(f"鈿狅笍  Warning: {args.version} version does not support tools/web_search parameter, will be ignored")
    ...
    if args.optimize_prompt_mode:
        ...
        if args.version in ["4.5", "5.0"] and args.optimize_prompt_mode == "fast":
            print(f"鈿狅笍  Warning: {args.version} version does not support fast optimization mode, will use standard mode")
    ...
    if args.version == "5.0":
        print(f"馃柤锔? Output format: {args.output_format}")
