...
"""CLI shared helpers for the ACEP API command line."""
...
def get_client() -> VePhoneClient:
    ...
    if not access_key or not secret_key:
        print(
            "閿欒: 璇疯缃幆澧冨彉閲?VOLC_ACCESS_KEY 鍜?VOLC_SECRET_KEY锛屾垨閰嶇疆 config.json 鏂囦欢"
        )
        sys.exit(1)
...
def parse_string_params(items: Optional[list], option_name: str = "--payload") -> dict:
    ...
    for item in items or []:
        if "=" not in item:
            raise SystemExit(f"{option_name} 蹇呴』鏄?Key=value 鏍煎紡")
        key, value = item.split("=", 1)
        if not key:
            raise SystemExit(f"{option_name} key 涓嶈兘涓虹┖")
        params[key] = value
    return params
...
def parse_bool_flag(value: str) -> bool:
    ...
    if lowered in {"false", "0", "no", "n"}:
        return False
    raise argparse.ArgumentTypeError("甯冨皵鍊煎繀椤绘槸 true/false")
...
def parse_json_option(
    ...
    try:
        ...
        raise SystemExit(f"{option_name} 蹇呴』鏄悎娉?JSON: {exc.msg}") from exc
    if expected_type is not None and not isinstance(parsed, expected_type):
        raise SystemExit(f"{option_name} 蹇呴』鏄?{expected_type.__name__} JSON")
    return parsed
...
def parse_app_list(value: str) -> list[dict]:
    ...
    if stripped.startswith("["):
        ...
        for item in parsed:
            if (
                ...
                raise SystemExit(
                    "--app-list JSON 鏁扮粍涓殑姣忛」閮藉繀椤诲寘鍚?AppId 鍜?VersionId"
                )
        return parsed
    ...
    for item in value.split(","):
        ...
        if ":" not in item:
            raise SystemExit(
                "--app-list 蹇呴』鏄?AppId:VersionId,AppId2:VersionId2 鎴?JSON 鏁扮粍"
            )
        app_id, version_id = item.split(":", 1)
        if not app_id or not version_id:
            raise SystemExit("--app-list 涓殑 AppId 鍜?VersionId 涓嶈兘涓虹┖")
        app_list.append({"AppId": app_id, "VersionId": version_id})
    if not app_list:
        raise SystemExit("--app-list 涓嶈兘涓虹┖")
    return app_list
