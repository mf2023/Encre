...
Memory lives in `~/yelp/`. If `~/yelp/` does not exist, run the guide. See the guide for structure.
...
| Topic | File |
|-------|------|
| Setup and activation behavior | the guide |
| Memory schema and status model | the guide |
| Official API request patterns | the guide |
| Search, compare, and shortlist workflow | the guide |
| Review interpretation and signal weighting | the guide |
| Listing QA and owner-side audit workflow | the guide |
| Access limits and safe operating boundaries | the guide |
