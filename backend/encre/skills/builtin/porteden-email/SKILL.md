---
name: porteden-email
description: Email Management - Secure Gmail, Outlook & Exchange - multi account support.
metadata:
  source: encre
  tags: 
user_invocable: true
...
If `porteden` is not installed: ``bash brew install` porteden/tap/porteden` (or `go install github.com/porteden/cli/cmd/porteden@latest`).
