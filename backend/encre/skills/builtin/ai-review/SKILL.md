---
name: ai-review
description: Reads content from URLs or files, classifies it, and generates structured summaries and comments in a specific, analytical style. Use when you need to
metadata:
  source: clawhub
  tags: ai-review
user_invocable: true
...
| **PDF Files (URL/Local)** | Use `shell` with `rest_client` to download and `pdftotext` to extract text (e.g., `curl -o t.pdf <URL> && pdftotext t.pdf -`). |
