...
_INFERENCE_PROMPT = """Analyze the conversation above and extract user traits. Return ONLY valid JSON with this exact structure:  # noqa: E501
{
  "inferred": {
...
- testing_preference: how they approach testing (e.g. "writes tests first", "tests after", "manual testing")  # noqa: E501
    ...
    async def infer(self, messages: list[dict[str, Any]], backend: Any) -> tuple[dict[str, Any], dict[str, float]] | None:
        ...
        try:
            ...
            async for event in backend.chat(messages=infer_messages, max_tokens=1024, stream=False):
                from encre.utils.types import BackendFinish, BackendText
