...
class FeishuAdapter(BaseAdapter):
    ...
    def __init__(
        ...
        if webhook_url or port:
            caps.append("webhook")
        super().__init__(gateway_url=gateway_url, capabilities=caps)
        self._webhook_url = webhook_url
        self._app_id = app_id or os.getenv("FEISHU_APP_ID")
        self._app_secret = app_secret or os.getenv("FEISHU_APP_SECRET")
        self._verify_token = verify_token or os.getenv("FEISHU_VERIFY_TOKEN")
        self._port = port or int(os.getenv("FEISHU_PORT", "18794"))
    ...
    async def _refresh_token(self) -> bool:
        ...
        Uses the internal app authentication endpoint. The token is cached
    ...
    async def send(
        ...
        *,
        ...
        If ``metadata`` contains ``"msg_type": "interactive"``, the content
        is sent as a Feishu Card (interactive message). Otherwise it is sent
        as plain text.
        """
    ...
    async def send_image(
        ...
        *,
        ...
        Uses the Feishu image upload API, then sends the resulting
        image_key as an image message.
        """
        if not await self._ensure_token():
            return SendResult(
                success=False,
    ...
    async def send_card(
        ...
        *,
        ...
        ``card`` can be a dict (card JSON structure) or a JSON string.
        """
    ...
    async def handle_webhook(
        ...
        Performs signature verification when ``verify_token`` is configured,
        handles URL verification (challenge) requests, and dispatches
        received messages to the registered handler.
