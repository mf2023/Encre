...
YUANBAO_WS_URL = "ws://127.0.0.1:18792/gateway"
...
class YuanbaoAdapter(BaseAdapter):
    ...
    from the Yuanbao platform, and the Yuanbao REST API to send outbound  # noqa: E402
    ...
    Args:
        app_key: Yuanbao application AppKey.
        app_secret: Yuanbao application AppSecret.
        bot_id: Optional bot account ID. Auto-discovered on connect if empty.
        gateway_url: Encre gateway WebSocket URL for agent processing.
        ws_url: Yuanbao WebSocket gateway URL.
        api_domain: Yuanbao REST API base domain.
    """
    ...
    async def connect(self) -> bool:
        """Connect to Yuanbao WebSocket gateway and Encre gateway."""
    ...
    async def disconnect(self) -> None:
        """Disconnect from Yuanbao and Encre gateway."""
        self._running = False
        self._connected_event.clear()
    ...
    async def _handle_inbound_message(self, parsed: dict[str, Any]) -> None:
        ...
        try:
            timestamp = time.time() if not ts else ts / 1000.0
    ...
    async def send(
        ...
        *,
        ...
        The chat_id format is ``direct:<account>`` for C2C or
        ``group:<group_code>`` for group chats. Long messages are
        automatically chunked to fit platform limits.
        """
    ...
    async def send_image(
        ...
        *,
        ...
        Uses the Yuanbao media upload REST endpoint, then sends the
        resulting media ID as an image message.
        """
        if not self._http_client:
            return SendResult(success=False, error="HTTP client not initialized")
    ...
    async def _upload_media(self, file_path: str, media_type: str) -> tuple[str | None, str | None]:
        ...
        Returns (media_id, None) on success or (None, error_message) on failure.
        """
        if not self._http_client:
