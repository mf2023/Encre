...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Encre handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Encre session — matching the per-thread behavior channels already
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        # "Running /cmd…" ack shown by handle_encre_command).
