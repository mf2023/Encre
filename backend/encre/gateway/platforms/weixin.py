...
# weixin.py
#
# Platform adapter for WeChat (iLink Bot) in the Encre gateway framework.
# Provides the WeixinAdapter class that connects via long-polling to the
# iLink Bot API and dispatches normalized MessageEvents.
#
# Exported classes:
#   - WeixinAdapter
#
...
logger = logging.getLogger("encre.gateway.platforms.weixin")
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    framework (https://ilinkai.weixin.qq.com).

    Example standalone usage::

        import asyncio  # noqa: E402
        from encre.gateway.platforms.weixin import WeixinAdapter  # noqa: E402
    ...
    def __init__(
        ...
        Args:
            config: Platform configuration containing app_id and token.
        ...
        self._api_base = config.extra.get("api_url", "").rstrip("/") or ILINK_BASE_URL
    ...
    async def _poll_loop(self) -> None:
        ...
        while self._running:
            if not self._http_client:
                ...
                return
            if not self._token:
                logger.warning("[%s] Token is empty, stopping poll loop", self.name)
                return
    ...
    async def get_chat_info(self, chat_id: str) -> dict[str, Any]:
        """Get information about a WeChat chat."""
