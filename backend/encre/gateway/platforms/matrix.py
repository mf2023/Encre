...
# matrix.py
#
# Matrix platform adapter for the Encre gateway.
# Provides Matrix Client-Server API integration.
#
# Exported classes:
#   - MatrixAdapter
#
...
logger = logging.getLogger("encre.gateway.platforms.matrix")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform = Platform.MATRIX) -> None:
        super().__init__(config=config, platform=platform)
        self._homeserver = config.extra.get("homeserver", "https://matrix-client.matrix.org").rstrip("/")
        self._user_id = config.extra.get("user_id", "")
        self._password = config.token
        self._device_id = config.extra.get("device_id", "")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._sync_task = self._spawn_task(self._sync_loop(), name="matrix-sync")
