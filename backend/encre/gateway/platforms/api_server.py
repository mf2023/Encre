...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Encre-Session-Id header; opt-in long-term memory scoping via X-Encre-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Encre-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists encre-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Encre sessions
- POST /api/sessions               — create an empty Encre session
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _ensure_session_db(self):
        ...
        redirects ``get_data_dir()``, so each profile gets its own DB —
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list encre-agent and any configured model_routes aliases.
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Encre sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
