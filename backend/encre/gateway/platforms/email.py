...
# Provides SMTP (send) + IMAP (receive) integration.
#
# Exported classes:
#   - EmailAdapter
#
...
logger = logging.getLogger("encre.gateway.platforms.email")
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform = Platform.EMAIL) -> None:
        ...
        self._smtp_host = config.extra.get("smtp_host", "")
        self._smtp_port = int(config.extra.get("smtp_port", 587))
        self._smtp_user = config.extra.get("smtp_user", "")
        self._smtp_pass = config.token
        self._imap_host = config.extra.get("imap_host", "")
        self._imap_port = int(config.extra.get("imap_port", 993))
        self._imap_user = config.extra.get("imap_user", "")
        self._imap_pass = config.extra.get("imap_pass", "") or config.token
        self._poll_interval = int(config.extra.get("poll_interval", 30))
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        self._poll_task = self._spawn_task(self._poll_inbox(), name="email-poll")
        logger.info(
...
def _check_requirements() -> bool:
    return AIOIMAPLIB_AVAILABLE and AIOSMTPLIB_AVAILABLE
...
platform_registry.register(PlatformEntry(
    ...
    required_env=["EMAIL_SMTP_HOST"],
    install_hint="pip install aioimaplib aiosmtplib",
))
