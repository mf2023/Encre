...
# msgraph.py
#
# Platform adapter for Microsoft Graph (Teams / Outlook) integration.
# Provides webhook-based notification reception and REST API messaging.
#
# Exported classes:
#   - MSGraphAdapter
#
...
logger = logging.getLogger("encre.gateway.platforms.msgraph")
...
class MSGraphAdapter(BasePlatformAdapter):
    """Microsoft Graph adapter for Teams and Outlook messaging.

    Connects to Microsoft Graph API for sending messages via Teams channels,
    chats, and email.  Receives inbound notifications via a webhook server.
    """
    ...
    def __init__(
        ...
        platform: Platform = Platform.MSGRAPH,
    ) -> None:
        super().__init__(config=config, platform=platform)
        self._tenant_id = config.extra.get("tenant_id", "") or os.getenv("MSGRAPH_TENANT_ID", "")
        self._client_id = config.extra.get("client_id", "") or os.getenv("MSGRAPH_CLIENT_ID", "")
        self._client_secret = config.token or os.getenv("MSGRAPH_CLIENT_SECRET", "")
        self._host = config.extra.get("host", "") or os.getenv("MSGRAPH_WEBHOOK_HOST", "127.0.0.1")
        self._port = int(config.extra.get("port", 0) or os.getenv("MSGRAPH_WEBHOOK_PORT", "8646"))
    ...
    async def disconnect(self) -> None:
        ...
        if self._http is not None:
            ...
            self._http = None
        logger.info("[msgraph] Disconnected")
...
platform_registry.register(PlatformEntry(
    ...
    platform=Platform.MSGRAPH,
    adapter_factory=lambda cfg: MSGraphAdapter(cfg),
    check_fn=_check_requirements,
    required_env=["MSGRAPH_CLIENT_SECRET", "MSGRAPH_TENANT_ID", "MSGRAPH_CLIENT_ID"],
))
