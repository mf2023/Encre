...
# qqbot.py
#
# Platform adapter for QQ Bot in the Encre gateway framework.
# Provides the QQBotAdapter class that connects to the QQ Bot WebSocket
# gateway and dispatches normalized MessageEvents.
#
# Exported classes:
#   - QQCloseError
#   - QQBotAdapter
...
logger = logging.getLogger("encre.gateway.platforms.qqbot")
...
class QQCloseError(Exception):
    """Raised when the QQ Bot WebSocket connection closes with a code."""
...
class QQBotAdapter(BasePlatformAdapter):
    ...
    def __init__(
        ...
        """Initialize the QQ Bot adapter.

        Args:
            config: Platform configuration containing app_id and client_secret.
        ...
        self._client_secret = config.token or config.extra.get("client_secret", "")
        ...
        # Heartbeat ACK tracking for zombie connection detection
        self._heartbeat_pending: bool = False
        self._missed_heartbeats: int = 0
        self._max_missed_heartbeats: int = 2
        # READY gate: only mark connected after READY received
        self._ready_event: asyncio.Event = asyncio.Event()
        self._ws_authenticated: bool = False

    @property
    def running(self) -> bool:
        """Override: only report running after READY confirmed."""
        return self._running and self._ws_authenticated

    def _mark_disconnected(self) -> None:
        """Override: also clear authentication state."""
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        # Step 5: Wait for READY event from QQ gateway (proves bot is online)
        try:
            await asyncio.wait_for(self._ready_event.wait(), timeout=15.0)
        except asyncio.TimeoutError:
            msg = "Step 5 FAILED -- READY not received within 15s (QQ may show bot as offline)"
            logger.error("[qqbot] %s", msg)
            self._set_fatal_error("ready_timeout", msg)
            await self._cleanup()
            return False

        self._mark_connected()
        logger.info("[qqbot] QQ Bot adapter fully connected (READY confirmed)")
        return True
    ...
    async def _reconnect(self, backoff_idx: int) -> bool:
        ...
        try:
            ...
            gateway_url = await self._get_gateway_url()
            await self._open_ws(gateway_url)
            # Don't mark connected yet — wait for READY in _read_events
            logger.info("[qqbot] Reconnected (awaiting READY)")
            return True
        except Exception as exc:
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        Tracks heartbeat ACK responses to detect zombie connections.
        If we miss too many ACKs, forcibly close the WS to trigger reconnect.
        """
        try:
            while self._running:
    ...
    async def get_chat_info(self, chat_id: str) -> dict[str, Any]:
        ...
        return {"id": chat_id, "platform": self.name, "type": self._guess_chat_type(chat_id)}
...
platform_registry.register(PlatformEntry(
    ...
    adapter_factory=lambda cfg: QQBotAdapter(cfg),
