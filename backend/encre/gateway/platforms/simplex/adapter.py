...
``gateway/run.py`` and ``tools/send_message_tool`` through the registry — no
...
# Imported at module top because they're stdlib-only inside Encre — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    Responsibilities:
        ...
        * Translate inbound daemon events (``newChatItems``, ``rcvFile*``,
          ``contactRequest`` …) into Encre ``MessageEvent`` objects and feed
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``encre setup gateway`` → SimpleX.
