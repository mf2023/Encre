...
# auth.json helpers — share the file with the rest of encre.
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    ...
    try:
        ...
        logger.warning("photon: encre.config unavailable — skipping .env write")
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `encre photon setup`)"
    ...
    labels["phone_number"] = (
        ...
        phone if phone else "✗ missing (run `encre photon setup --phone ...`)"

    )

    labels["assigned_phone_number"] = (

        assigned if assigned else "✗ missing (run `encre photon setup`)"
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `encre photon setup --phone ...`)"
    ...
    def _present_assigned_phone() -> str:
        ...
        return assigned or "✗ missing (run `encre photon setup`)"
