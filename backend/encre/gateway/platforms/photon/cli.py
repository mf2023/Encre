...
"""
``encre photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Encre
...
from encre.colors import Colors, color
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    """Wire up `encre photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         ...
                         help="Project name (default: 'Encre Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``encre photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Encre Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> data dir/.env,
    ...
    print("  Start the gateway:  encre gateway start")
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `encre photon install-sidecar`'}")

    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`encre photon telemetry on|off`)")
...
def _telemetry_enabled() -> bool:
    ...
    """Read PHOTON_TELEMETRY from the env / data dir/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `encre photon telemetry on` / `encre photon telemetry off`.")
    ...
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in data dir)")

    print("  Restart the gateway for the sidecar to pick it up:  encre gateway restart")
...
# `encre gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``encre photon setup`` with interactive
...
def gateway_setup() -> None:
    ...
    """Run Photon first-time setup from the `encre gateway setup` wizard."""
