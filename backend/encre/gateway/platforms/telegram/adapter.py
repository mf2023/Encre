...
import os
def atomic_replace(path, data, *args, **kwargs):
    import json
...
def _probe_voice_duration_seconds(path: str) -> Optional[int]:
    ...
    Mirrors ``encre.gateway.run._probe_audio_duration``: stdlib ``wave`` for WAV,
...
# The shared convert_table_to_bullets() in encre.gateway.platforms.helpers handles
# the full conversion (detection + rendering); Telegram just calls it.

from encre.gateway.platforms.helpers import (
...
# trigger its own recovery path. Refs: encre#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: encre#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("ENCRE_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "ENCRE_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "ENCRE_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Encre-created private-chat topic lanes are marked with
    ...
    def _prune_stale_dm_topic_binding(
        ...
        ``encre.gateway.run._recover_telegram_topic_thread_id`` keeps
    ...
    # older PTB/clients. Streaming edits stay on Encre's existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Encre must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        if attempt > MAX_NETWORK_RETRIES:
            message = (
                ...
                "Restarting encre.gateway." % MAX_NETWORK_RETRIES
            )
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `encre update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Encre or OpenClaw instance is running "
            "with this token, then restart the gateway with 'encre gateway restart'."
    ...
    def _persist_dm_topic_thread_id(
        ...
        try:
            ...
            if changed:
                def atomic_config_write(*args, **kwargs):
                    pass
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                telegram_menu_commands = lambda max_commands=100: ([], 0)
                telegram_menu_max_commands = lambda: 100
                if not self._bot:
                    return
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  Encre defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Encre process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("ENCRE_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("ENCRE_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("ENCRE_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("ENCRE_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("ENCRE_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            disable_fallback = (os.getenv("ENCRE_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("ENCRE_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set ENCRE_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``encre model`` picker via the shared ``group_providers`` fold,
    ...
    async def _handle_callback_query(
        ...
        if data.startswith("ea:"):
            ...
            if len(parts) == 3:
                ...
                logger.info(
                    "Telegram button approval stub for session %s (choice=%s, user=%s)",
        ...
        if data.startswith("sc:"):
            ...
            if len(parts) == 3:
                ...
                try:
                    async def _resolve_slash(*a, **kw): return None
                    result_text = await _resolve_slash(session_key, confirm_id, choice)
        ...
        if data.startswith("cl:"):
            ...
            if len(parts) == 3:
                ...
                if choice_token == "other":
                    ...
                    flipped = False
                    logger.warning("[%s] mark_awaiting_text stub (not available)", self.name)
                ...
                self._clarify_state.pop(clarify_id, None)
                logger.error("[%s] resolve_gateway_clarify stub (not available)", self.name)
                resolved = False
    ...
    # Scripts live in data dir/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        from encre.config import get_data_dir
        script_path = get_data_dir() / "scripts" / "gmail-triage" / script_name
    ...
    async def send_image(
        ...
        if not EncreSSRFGuard().validate_url(image_url):
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@encre_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Encre bot profiles. A message like
    ...
    async def _cache_observed_media(self, msg: Message, event: MessageEvent) -> None:
        ...
        """
        from encre.gateway.platforms.base import cache_media_bytes
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Encre inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Encre inbox (#52363).
        if self._is_own_message(message):
            return False
    ...
    async def _handle_sticker(self, msg: Message, event: "MessageEvent") -> None:
        ...
        """
        from encre.gateway.sticker_cache import (
        ...
        try:
            ...
            logger.info("[Telegram] Analyzing sticker stub (vision_analyze_tool not available)")
...
# encre/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("ENCRE_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
            from encre.gateway.config import load_gateway_config
            from encre.gateway.run import cfg_get
...
async def _standalone_send(
    ...
    *,
    ...
    async def _send_telegram(*a, **kw): return None
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in encre/encre.gateway.py.
    """
    logger.info("[Telegram] interactive_setup stub (encre not available)")
