...
# group mention gating without custom aliases, use conservative wake words so
# `require_mention: true` is a one-line enablement path.
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Encre defaults).
        """
        if raw is None:
...
platform_registry.register(PlatformEntry(
    ...
    required_env=["BLUEBUBBLES_URL", "BLUEBUBBLES_PASSWORD"],
))