...
"""
Email platform adapter for the Encre gateway.

This adapter lets users interact with Encre by sending and receiving emails.
It uses IMAP to receive mail and SMTP to send replies. The module also contains
the sender-authentication checks (SPF/DKIM/DMARC) that protect the allowlist
from a spoofable ``From:`` header, plus the standalone delivery helper and the
platform-registry glue used when the adapter runs as a bundled plugin.
...
Key collaborators: :class:`encre.gateway.platforms.base.BasePlatformAdapter`
(which provides ``build_source``/``handle_message``), the platform registry
that exposes this adapter to the gateway, and the base send helpers in
...
from encre.gateway.platforms.base import env_int, env_bool
...
def _create_ipv4_connection(
    ...
    Args:
        host: Target hostname or IPv4 literal.
        port: Target TCP port.
        timeout: Per-socket connect timeout in seconds.
        source_address: Optional local ``(host, port)`` to bind before connect.

    Returns:
        socket.socket: A connected IPv4 TCP socket.

    Raises:
        OSError: If no resolvable IPv4 address connects.
    """
...
class _IPv4SMTP(smtplib.SMTP):
    """SMTP subclass that connects over an IPv4-only socket path."""
...
class _IPv4SMTP_SSL(smtplib.SMTP_SSL):
    """SMTP_SSL subclass that connects over an IPv4-only socket path."""
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    Args:
        imap: An authenticated IMAP4 connection.
    """
    _encre_version = "0"
    try:
        imap.xatom(
            "ID",
            f'("name" "encre-agent" "version" "{_encre_version}" '
            '"vendor" "Dunimd" '
            '"support-email" "dunimd@outlook.com")',
        )
...
def _is_automated_sender(address: str, headers: dict) -> bool:
    ...
    Args:
        address: The sender's bare email address (already lower-cased works).
        headers: Mapping of header name to raw header value for the message.

    Returns:
        bool: True when the address matches a known noreply pattern or carries
        a header indicating automated/bulk mail.
    """
    addr = address.lower()
...
def check_email_requirements() -> bool:
    ...
    Returns:
        bool: True only when address, password, IMAP host and SMTP host are set.
    """
...
def _decode_header_value(raw: str) -> str:
    ...
    Args:
        raw: The raw header value, possibly with ``=?...?=`` encoded words.

    Returns:
        str: The decoded header text, with unknown charsets replaced.
    """
...
def _extract_text_body(msg: email_lib.message.Message) -> str:
    ...
    Args:
        msg: A parsed email message object.

    Returns:
        str: The extracted body text, or an empty string when none is found.
    """
    if msg.is_multipart():
...
def _strip_html(html: str) -> str:
    ...
    Converts the most common block/line elements to newlines, decodes a handful
    of HTML entities, and collapses runs of blank lines.

    Args:
        html: Raw HTML markup.

    Returns:
        str: Plain-text approximation with normalized whitespace.
    """
...
def _extract_email_address(raw: str) -> str:
    ...
    Args:
        raw: A raw ``From``/``To`` header value.

    Returns:
        str: The bare address, lower-cased (or the trimmed input if no angle
...
def _domain_of(address: str) -> str:
    ...
    Args:
        address: A bare email address.

    Returns:
        str: The substring after the last ``@``, or ``''`` when absent.
    """
...
def _domains_aligned(a: str, b: str) -> bool:
    ...
    Args:
        a: First domain string.
        b: Second domain string.

    Returns:
        bool: True when the domains are equal or one is a subdomain of the other.
    """
...
def _verify_sender_authentication(
    ...
    *,
    ...
    Args:
        msg: The parsed email message (used to read Authentication-Results).
        from_addr: The bare ``From:`` address whose domain we authenticate.
        authserv_id: Optional authserv-id to pin the trusted header to the
            operator's own receiving server.

    Returns:
        Tuple[bool, str]: ``(authenticated, reason)``. ``authenticated`` is
        True when a DMARC pass is recorded for the From domain, or an SPF/DKIM
        pass aligned with the From domain. When no ``Authentication-Results``
        header is present at all we fail-closed with reason
...
def _extract_attachments(
    ...
    (useful for malware protection or bandwidth savings). Image parts are
    cached through ``cache_image_from_bytes`` (which validates magic bytes),
    while other parts go through ``cache_document_from_bytes``.

    Args:
        msg: A parsed multipart email message.
        skip_attachments: When True, ignore every attachment/inline part.

    Returns:
        List[Dict[str, Any]]: Each entry carries ``path``, ``filename``,
        ``type`` (``"image"`` or ``"document"``) and ``media_type``.
    """
    attachments = []
...
class EmailAdapter(BasePlatformAdapter):
    ...
    The adapter connects to an IMAP server to poll an INBOX for unseen mail and
    to an SMTP server to deliver replies. It enforces sender authentication and
    an optional allowlist, threads replies using stored subject/message-id
    context, and caches attachments locally before handing them to the gateway
    core via :class:`BasePlatformAdapter` hooks.

    Attributes:
        _address: The agent's own email address (login identity).
        _password: The login secret (env-sourced; never logged).
        _imap_host/_imap_port: IMAP endpoint used for receiving.
        _smtp_host/_smtp_port: SMTP endpoint used for sending.
        _poll_interval: Seconds between INBOX checks.
        _seen_uids: Deduplication set of already-processed IMAP UIDs.
        _thread_context: Per-sender reply-threading context.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # encre.gateway.config populates and that the "connected" check, the
        # send-helper, and `encre config show` already read. Without the
    ...
    def _connect_smtp(self) -> smtplib.SMTP:
        ...
        Returns:
            smtplib.SMTP: A connected SMTP object with TLS established — callers
            can proceed directly to ``login()``.

        Raises:
            OSError: Re-raised from the IPv4 fallback when the primary also fails.
        """
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        Validates configuration up front, runs a connection test against both
        IMAP and SMTP, marks existing messages as seen, then launches the poll
        loop. On missing configuration it records a fatal, non-retryable error so
        the gateway does not loop forever against an empty host.

        Args:
            is_reconnect: Hint that this is a reconnect attempt (currently unused
                by the adapter's validation logic).

        Returns:
            bool: True when both connection tests pass and polling starts.
        """
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `encre gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    async def disconnect(self) -> None:
        ...
        Cancels the poll task (ignoring the resulting ``CancelledError``) and
        clears the cached task handle.
        """
        self._running = False
        if self._poll_task:
    ...
    async def _poll_loop(self) -> None:
        ...
        Loops until ``_running`` is cleared, checking the INBOX and then sleeping
        for ``_poll_interval`` seconds. Surfaces per-iteration errors as logs so a
        single failure does not stop future polling.
        """
        while self._running:
            try:
    ...
    async def _check_inbox(self) -> None:
        ...
        Runs the blocking IMAP fetch in an executor thread and dispatches each
        parsed result to :meth:`_dispatch_message`.
        """
    ...
    def _fetch_new_messages(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: One dict per fetched message carrying sender,
            subject, body, attachments, threading headers and the
            ``sender_authenticated`` verdict.
        """
        results = []
        try:
            imap = imaplib.IMAP4_SSL(self._imap_host, self._imap_port, timeout=30)
    ...
    def _allow_all_senders() -> bool:
        ...
        Returns:
            bool: True when allow-all is enabled via either env flag.
        """
        truthy = {"true", "1", "yes"}
        return (
    ...
    def _allowlist_in_effect() -> bool:
        ...
        Returns:
            bool: True when an allowlist is configured via either env flag.
        """
        return bool(
    ...
    async def _dispatch_message(self, msg_data: Dict[str, Any]) -> None:
        ...
        Applies the full authorization pipeline before building a
        :class:`MessageEvent`: drops self-messages and automated senders, honors
        the sender allowlist and the allow-all opt-out, and rejects senders whose
        ``From:`` domain failed authentication when an allowlist is in effect.
        Then it derives the message type from attachments, records thread context
        and hands the event to ``handle_message``.

        Args:
            msg_data: A parsed-message dict from :meth:`_fetch_new_messages`.
        """
    ...
    async def send(
        ...
        Runs the blocking SMTP send in an executor thread and wraps the outcome
        in a :class:`SendResult`.

        Args:
            chat_id: Recipient email address.
            content: Plain-text body of the reply.
            reply_to: Optional original Message-ID to set as In-Reply-To.
            metadata: Optional metadata (accepted for the base contract).

        Returns:
            SendResult: Success flag plus the generated Message-ID, or an error
            string on failure.
        """
        try:
            loop = asyncio.get_running_loop()
    ...
    def _message_id_domain(self) -> str:
        ...
        Returns:
            str: The domain after the last ``@``, or ``"localhost"``.
        """
        if "@" in self._address:
    ...
    def _send_email(
        ...
        Builds a threaded reply (subject ``Re: ...``, In-Reply-To/References set
        from stored thread context) and delivers it over a TLS SMTP connection.

        Args:
            to_addr: Recipient address.
            body: Plain-text body.
            reply_to_msg_id: Optional Message-ID to thread under.

        Returns:
            str: The generated Message-ID of the sent mail.
        """
        msg = MIMEMultipart()
        ...
        subject = ctx.get("subject", "Encre Agent")
        ...
        msg_id = f"<encre-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        Args:
            chat_id: Recipient address (unused).
    ...
    async def send_image(
        ...
        Args:
            ...
            image_url: URL of the image to embed as a link.
            caption: Optional caption prepended to the body.
            reply_to: Optional Message-ID to thread under.
            metadata: Optional metadata (unused).

        Returns:
            SendResult: Result of the underlying :meth:`send` call.
        """
        text = caption or ""
        text += f"\n\nImage: {image_url}"
    ...
    async def send_multiple_images(
        ...
        Args:
            ...
            metadata: Optional metadata (accepted for the base contract).
            human_delay: Delay between items (unused; single email is sent).
        """
        if not images:
            return
    ...
    def _send_email_with_attachments(
        ...
        Args:
            ...
            file_paths: Local filesystem paths to attach.
    ...
    async def send_document(
        ...
        **kwargs,
        ...
        Args:
            ...
            file_name: Optional override for the attachment filename.
            reply_to: Optional Message-ID to thread under.
            **kwargs: Extra base-class keyword arguments (ignored).

        Returns:
            SendResult: Success flag plus generated Message-ID, or error string.
        """
        try:
            loop = asyncio.get_running_loop()
            message_id = await loop.run_in_executor(
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        Args:
            chat_id: Sender email address used as the chat identifier.

        Returns:
            Dict[str, Any]: A small dict with name, type, chat_id and subject.
        """
...
# encre/encre.gateway.py, and the _send_email dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    Args:
        pconfig: The platform configuration object (read for address/host).
        chat_id: Recipient address.
        message: Plain-text body to send.
        thread_id: Optional thread identifier (unused for email).
        media_files: Optional media file list (unused for plain text send).
        force_document: Optional flag (unused for plain text send).

    Returns:
        dict: ``{"success": True, "platform": "email", "chat_id": ...}`` on
        success, or ``{"error": ...}`` when misconfigured or sending fails.
    """
    import smtplib
    import ssl as _ssl
    from email.mime.text import MIMEText
...
def _is_connected(config) -> bool:
    ...
    Args:
        config: A platform configuration object exposing ``extra``.

    Returns:
        bool: True when an address is present in config or EMAIL_ADDRESS.
    """
    extra = getattr(config, "extra", {}) or {}
    if extra.get("address"):
        return True
    return bool((os.environ.get("EMAIL_ADDRESS") or "").strip())
...
def _check_requirements() -> bool:
    """Return whether the email platform is available to load.

    Returns:
        bool: True when email support is importable/available.
    """
...
def _build_adapter(config):
    ...
    Returns:
        EmailAdapter: A freshly constructed adapter instance.
    """
