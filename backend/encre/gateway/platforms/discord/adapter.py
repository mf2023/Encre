...
_consume_background_task_result = lambda task: None
...
from encre.ssrf import EncreSSRFGuard as _G
def is_safe_url(url):
    try:
        return _G().validate_url(url)
    except Exception:
        return False
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("ENCRE_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid ENCRE_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
class VoiceReceiver:
    ...
    def pcm_to_wav(pcm_data: bytes, output_path: str,
        ...
        try:
            ...
            windows_hide_flags = getattr(_sp, "CREATE_NO_WINDOW", 0)
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``data dir env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("ENCRE_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("ENCRE_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="ENCRE_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="ENCRE_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        from encre.config import get_data_dir
        from .recovery import DiscordRecoveryStore
        self._discord_recovery_store = DiscordRecoveryStore(get_data_dir())
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Encre gateway
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            from encre.gateway.platforms.base import resolve_proxy_url, proxy_kwargs_for_bot
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``ENCRE_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("ENCRE_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_data_dir() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Encre response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Encre work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Encre/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:encre|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Encre manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "encre_voice",
    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        is_whisper_hallucination = lambda x: False
        ...
        try:
            ...
            async def transcribe_audio(*a, **kw): return ""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Encre pairing grant."""
    ...
    def _is_allowed_user(
        ...
        *,
        ...
        # (see AGENTS.md pitfall #17 — same pattern as encre.gateway.run).
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Encre session")
        ...
        @tree.command(name="status", description="Show Encre session status")
        ...
        @tree.command(name="stop", description="Stop the running Encre agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan skills directory for new or removed skills")
        ...
        @tree.command(name="update", description="Update Encre Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Encre gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start an Encre session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Encre in the thread",
        ...
        # encre/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            COMMAND_REGISTRY = {}; _is_gateway_available = False; _resolve_config_gates = lambda x: []
        ...
        try:
            _iter_plugin_command_entries = lambda: []
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run an Encre skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        discord_skill_commands_by_category = lambda: {}
    ...
    def refresh_skill_group(self) -> tuple[int, int]:
        ...
        Invoked by :meth:`encre.gateway.run.GatewayOrchestrator._handle_reload_skills_command`
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Encre session in the thread
    ...
    def _resolve_channel_skills(self, channel_id: str, parent_id: str | None = None) -> list[str] | None:
        ...
        """
        from encre.gateway.platforms.base import resolve_channel_skills
        return resolve_channel_skills(self.config.extra, channel_id, parent_id)

    def _resolve_channel_prompt(self, channel_id: str, parent_id: str | None = None) -> str | None:
        """Resolve a Discord per-channel prompt, preferring the exact channel over its parent."""
        from encre.gateway.platforms.base import resolve_channel_prompt
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Encre: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Encre has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Encre"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_encre_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Encre: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        from encre.gateway.platforms.base import utf16_len, _prefix_within_utf16_limit
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Encre semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Encre handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Encre to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Encre needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``encre update --gateway``
    ...
    async def _read_attachment_bytes(
        ...
        *,
        ...
        Oversized attachments (per ``encre.gateway.max_inbound_media_bytes``) raise
        ``ValueError`` BEFORE the bytes are pulled into memory when Discord
        reports the size up front, so a hostile upload can't OOM the encre.gateway.
        """
        ...
        *,
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Encre could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_encre_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``encre pairing approve``).
    ...
    # so users approved via ``encre pairing approve`` can interact with
...
def _resolve_exec_approval_admin_gate(
    ...
    if not require_admin:
        return (False, set())
    try:
        from encre.gateway.slash_access import _coerce_id_list
...
def _define_discord_view_classes() -> None:
    ...
    class ExecApprovalView(discord.ui.View):
        ...
        async def _resolve(
            ...
            try:
                async def resolve_gateway_approval(*a, **kw): return None
        ...
        async def _resolve(
            ...
            try:
                try:
                    ...
                    class _Fake: pass; _slash_confirm_mod = _Fake()
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``encre update`` prompts.
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _expensive_warning_for(self, model_id: str):
            try:
                expensive_model_warning = lambda x: ""
    ...
    class ClarifyChoiceView(discord.ui.View):
        ...
        async def _resolve_choice(
            ...
            try:
                async def resolve_gateway_clarify(*a, **kw): return None
        ...
        async def _on_other(self, interaction: "discord.Interaction") -> None:
            ...
            try:
                mark_awaiting_text = lambda *a, **kw: None
...
# is not in this process (e.g. ``encre cron`` running standalone) and no live
    ...
    *,
    ...
    try:
        ...
        if thread_id:
            ...
            _channel_type = None
            try:
                from encre.gateway.channel_directory import lookup_channel_type
...
def interactive_setup() -> None:
    ...
    """
    get_env_value = lambda k: os.environ.get(k, ""); save_env_value = lambda k, v: None
    prompt = lambda x="": input(x)
    prompt_yes_no = lambda x: input(x).strip().lower() in ("y", "yes")
    print_header = print
    print_info = print
    print_success = print
    ...
    print()
    print_info("📬 Home Channel: where Encre delivers cron job results,")
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "ENCRE_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "ENCRE_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
