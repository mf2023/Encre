...
# dingtalk.py
#
# Platform adapter for DingTalk in the Encre gateway framework.
# Provides the DingTalkAdapter class that connects via Stream Mode
# for real-time message reception and uses the DingTalk Open API
...
logger = logging.getLogger("encre.gateway.platforms.dingtalk")
...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    def __init__(
        ...
        platform: Platform = Platform.DINGTALK,
    ) -> None:
        """Initialize the DingTalk adapter.

        Args:
            config: Platform configuration containing client_id and client_secret.
        ...
        self._robot_code: str = self._client_id
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            logger.info("[dingtalk] Step 1/3: Initializing HTTP client...")
            proxy_url = None
            import os
            proxy_url = os.environ.get("HTTPS_PROXY") or os.environ.get("HTTP_PROXY") or None
            self._http_client = httpx.AsyncClient(timeout=30.0, follow_redirects=True, proxy=proxy_url)
            logger.info("[dingtalk] Step 1/3: HTTP client ready")

            logger.info("[dingtalk] Step 2/3: Creating DingTalk stream client...")
            ...
            logger.info("[dingtalk] Step 2/3: Stream client created")

            logger.info("[dingtalk] Step 3/3: Starting stream background task...")
            self._stream_task = asyncio.create_task(self._run_stream())
            logger.info("[dingtalk] Step 3/3: Stream task started")
    ...
    async def get_chat_info(self, chat_id: str) -> dict[str, Any]:
        """Get information about a DingTalk chat."""
    ...
    async def _send_via_webhook(
        ...
        try:
            resp = await self._http_client.post(
                webhook,
                headers={"User-Agent": "Encre/1.0.0 (DingTalk Adapter)"},
                json=payload,
                timeout=15.0,
            )
        ...
        try:
            resp = await self._http_client.post(
                ...
                headers={
                    ...
                    "User-Agent": "Encre/1.0.0 (DingTalk Adapter)",
                },
                json=payload,
                timeout=15.0,
            )
...
def _check_requirements() -> bool:
    return DINGTALK_AVAILABLE and HTTPX_AVAILABLE
