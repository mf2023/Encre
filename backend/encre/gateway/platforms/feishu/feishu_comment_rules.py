...
Config: data dir/feishu_comment_rules.json (mtime-cached, hot-reload).
Pairing store: data dir/feishu_comment_pairing.json.
"""
...
# ---------------------------------------------------------------------------
#
# Uses the canonical ``get_data_dir()`` helper (data dir-aware and
...
RULES_FILE = get_data_dir() / "feishu_comment_rules.json"
PAIRING_FILE = get_data_dir() / "feishu_comment_pairing.json"
...
def _main() -> int:
    ...
    try:
        import os
        os.environ.get("ENCRE_DOTENV", ".env")
    except Exception:
        pass

    usage = (
        "Usage: python -m encre.gateway.platforms.feishu_comment_rules <command> [args]\n"
        "\n"
        "Commands:\n"
