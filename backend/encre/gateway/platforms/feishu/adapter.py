...
In single-bot mode (what Encre currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_data_dir() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("ENCRE_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "ENCRE_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "ENCRE_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("ENCRE_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("ENCRE_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "ENCRE_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="encre-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Encre gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``encre_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"encre_action": action_name, "approval_id": approval_id},
                }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "encre_update_prompt_action": answer,
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_data_dir() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Encre does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        ...
        Delegates to :mod:`encre.gateway.platforms.feishu_comment` for parsing,
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        encre_action = action_value.get("encre_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("encre_update_prompt_action")
    ...
    def _submit_on_loop(self, loop: Any, coro: Any) -> bool:
        ...
        try:
            ...
            logger.warning("[Feishu] Failed to schedule background callback work: %s", exc)
            return False
        if future is None:
            return False
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("encre_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("encre_update_prompt_action", "") or "").strip().lower()
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                file_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; EncreAgent/1.0)",
                    "Accept": "*/*",
                },
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Encre webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Encre's SessionSource fields.
...
# Called by `encre gateway setup` via _setup_feishu() in encre/encre.gateway.py.
# =============================================================================
...
# dict in encre/encre.gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    succeed when cron runs separately from the encre.gateway. Builds a transient
    ...
    """
    if not FEISHU_AVAILABLE:
        return {"error": "Feishu dependencies not installed. Run: pip install 'encre[feishu]'"}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in encre/encre.gateway.py and the static
    _PLATFORMS["feishu"] dict. CLI helpers are lazy-imported.
    """
    get_env_value = lambda k, d=None: os.environ.get(k, d or "")
    save_env_value = lambda k, v: None
    prompt_choice = lambda x, **kw: input(x)
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `encre pairing approve`.")
    elif access_idx == 1:
