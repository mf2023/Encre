...
# homeassistant.py
#
# Platform adapter for Home Assistant integration.
# Connects to the HA WebSocket API for real-time event monitoring
# and delivers outbound messages as persistent notifications.
#
# Exported classes:
...
logger = logging.getLogger("encre.gateway.platforms.homeassistant")
...
class HomeAssistantAdapter(BasePlatformAdapter):
    ...
    def __init__(
        ...
        platform: Platform = Platform.HOMEASSISTANT,
    ) -> None:
        super().__init__(config=config, platform=platform)
        self._hass_url = config.extra.get("url", "http://homeassistant.local:8123").rstrip("/")
        self._hass_token = config.token
        ...
        watch_domains = config.extra.get("watch_domains", [])
        watch_entities = config.extra.get("watch_entities", [])
        ignore_entities = config.extra.get("ignore_entities", [])
        ...
        self._watch_all: bool = config.extra.get("watch_all", False)
        self._cooldown_seconds: int = config.extra.get("cooldown_seconds", _HA_DEFAULT_COOLDOWN)
...
platform_registry.register(PlatformEntry(
    ...
    adapter_factory=lambda cfg: HomeAssistantAdapter(cfg),
    check_fn=_check_requirements,
    required_env=["HOMEASSISTANT_TOKEN"],
))
