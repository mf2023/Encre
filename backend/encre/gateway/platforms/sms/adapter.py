...
from encre.gateway.platforms.helpers import redact_phone, strip_markdown
...
class SmsAdapter(BasePlatformAdapter):
    """
    Twilio SMS <-> Encre gateway adapter.

    Each inbound phone number gets its own Encre session (multi-tenant).
...
# static dict in encre/encre.gateway.py, and the _send_sms dispatch in
...
def _is_connected(config) -> bool:
    """SMS is connected when Twilio credentials are present."""
    return bool(os.getenv("TWILIO_ACCOUNT_SID") and os.getenv("TWILIO_AUTH_TOKEN"))
