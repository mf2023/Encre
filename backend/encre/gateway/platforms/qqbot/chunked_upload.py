...
- UploadFileTooLargeError — file exceeds the platform per-file limit.
- RuntimeError — generic upload failure (network, part PUT, complete).
"""
...
class ChunkedUploader:
    ...
    async def upload(
        ...
        Four-step process:
            1. Compute file hashes (md5, sha1, md5_10m) — runs in executor.
            2. Call ``upload_prepare`` API → get upload_id, block_size, part URLs.
            3. Upload each part concurrently (PUT to COS + notify server).
            4. Call ``complete_upload`` API → returns file_info token.
        ...
        Returns:
            Raw response dict from ``complete_upload`` — contains ``file_info``
