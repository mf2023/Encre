...
/**
 * Yim cryptographic layer (TypeScript / Web Crypto API).
 *
 * Uses the same AES‑256‑GCM algorithm and keyfile format as the Python
 * `yim/crypto.py` module.  The master key is read (and unwrapped) once
...
const HKDF_SALT = new TextEncoder().encode("yim-crypto-hkdf-v1");
...
async function getMachineId(): Promise<Uint8Array> {
  ...
  // Fallback: use navigator properties (consistent within a machine session)
  const fallback = `${navigator.platform}-${navigator.hardwareConcurrency}-${navigator.language}`;
...
async function unwrapMasterKey(keyfileBytes: Uint8Array): Promise<CryptoKey> {
  ...
  const masterKeyRaw = await crypto.subtle.decrypt(
    { name: "AES-GCM", iv: nonce.buffer as ArrayBuffer, additionalData: new TextEncoder().encode("yim-keywrap-v1").buffer as ArrayBuffer } as AesGcmParams,
    wrappingKey,
...
export async function initCrypto(): Promise<void> {
  ...
  try {
    ...
    if (!keyfileBytes || keyfileBytes.byteLength === 0) {
      ...
      console.warn("[yim/crypto] Keyfile not available — transport encryption disabled");
      _ready = true;
      return;
    }
    ...
    console.warn("[yim/crypto] Failed to initialise crypto:", err);
