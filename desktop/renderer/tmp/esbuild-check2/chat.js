...
<span style="color:var(--text-muted)">\u2026 ${deduped.length - maxLines} more lines</span>` : "";
  ...
  function saveVideoPlayback(scope) {
    const states = [];
    (scope ?? document).querySelectorAll(".info-card--media video").forEach((v) => {
      const card = v.closest(".info-card--media");
      if (card?.id) states.push({ id: card.id, t: v.currentTime, p: v.paused });
    });
    return states;
  }
  function restoreVideoPlayback(states, scope) {
    if (states.length === 0) return;
    requestAnimationFrame(() => {
      states.forEach((s) => {
        const card = scope ? scope.querySelector(`#${CSS.escape(s.id)}`) : document.getElementById(s.id);
        if (!card) return;
  ...
  var Chat = class {
    ...
    constructor() {
      ...
      w.__answerQuestion = (event, btn, cardId) => {
        ...
        const card = btn.closest('[id^="tc-"]') ?? document.getElementById(cardId);
        if (!card) return;
      ...
      w.__toggleStatusCard = (el, id) => {
        const card = el.closest('[id^="tc-"]') ?? (id ? document.getElementById(id) : null);
      ...
      w.__initInfoCardMedia = (elOrId) => {
        const card = typeof elOrId === "string" ? document.getElementById(elOrId) : elOrId.closest('[id^="tc-"]');
        if (!card) return;
      ...
      w.__initMediaCards = (scope) => {
        (scope ?? document).querySelectorAll(".info-card--media").forEach((card) => {
          w.__initInfoCardMedia(card);
        });
      };
      w.__openInfoHtmlCard = async (el, cardId) => {
        const card = el.closest('[id^="tc-"]') ?? (cardId ? document.getElementById(cardId) : null);
        if (!card) return;
      ...
      w.__openInfoHtmlCards = (scope) => {
        (scope ?? document).querySelectorAll(".strip-item[data-source]").forEach((card) => {
          const id = card.id;
