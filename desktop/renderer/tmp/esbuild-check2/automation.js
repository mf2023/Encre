...
(() => {
  ...
  var import_session = __require("./session.js");
  var import_context_menu = __require("./context-menu.js");
  var import_stream = __require("./stream.js");
  var import_icons = __require("./icons.js");
  function gatewayDisplayName(id) {
    const key = "settings.adapterName" + id.charAt(0).toUpperCase() + id.slice(1);
    const name = (0, import_i18n.t)(key);
  ...
  function formatSchedule(cron) {
    if (!cron) return (0, import_i18n.t)("automation.once");
    const parts = cron.split(" ");
    if (parts.length !== 5) return cron;
    const [min, hour, , , dow] = parts;
    if (hour === "*" && min === "*") return (0, import_i18n.t)("automation.everyMinute");
    if (hour !== "*" && min !== "*" && dow === "*")
      return `${hour.padStart(2, "0")}:${min.padStart(2, "0")} ${(0, import_i18n.t)("automation.daily")}`;
    if (dow !== "*")
      return `${hour.padStart(2, "0")}:${min.padStart(2, "0")} ${(0, import_i18n.t)("automation.onWeekdays")}`;
    return cron;
  }
  ...
  function formatDateTime(unixTs) {
    ...
    const locale = (0, import_i18n.getLocale)() === "en" ? "en-US" : "zh-CN";
    const yyyy = d.getFullYear();
    const mm = String(d.getMonth() + 1).padStart(2, "0");
    const dd = String(d.getDate()).padStart(2, "0");
    const hh = String(d.getHours()).padStart(2, "0");
    const mi = String(d.getMinutes()).padStart(2, "0");
    return locale === "en-US" ? `${mm}/${dd}/${yyyy} ${hh}:${mi}` : `${yyyy}\u5E74${Number(mm)}\u6708${Number(dd)}\u53F7 ${hh}:${mi}`;
  }
  function formatDate(unixTs) {
    ...
    const mm = String(d.getMonth() + 1).padStart(2, "0");
    const dd = String(d.getDate()).padStart(2, "0");
    return locale === "en-US" ? `${mm}/${dd}` : `${Number(mm)}\u6708${Number(dd)}\u65E5`;
  }
  var Automation = class {
    /**
    ...
    constructor() {
      ...
      this.historyStatus = "";
      // "" = all, or "SUCCESS"/"FAILED"/"RUNNING"/"PENDING"
      this.historyTaskId = "";
      // "" = all, or a specific job id
      ...
      (0, import_i18n.onLocaleChange)(() => this.updateHistoryFilterLabels());
    ...
    updateExecution(jobId, eventType, eventData) {
      ...
      if (eventType === "start") {
        ...
        const keepMsgs = Array.isArray(eventData.messages) && eventData.messages.length > 0 ? eventData.messages : this.activeExecution.messages;
        this.activeExecution = { ...this.activeExecution, state: failed ? "FAILED" : "COMPLETED", messages: keepMsgs, result: String(eventData.result || this.activeExecution.result || ""), error_code: failed ? String(eventData.error_code || "AUTOMATION_EXECUTION_FAILED") : void 0 };
      } else {
        return;
      }
    ...
    showDetail(data) {
      if (data.state === "FAILED") {
        ...
        this.detailContentEl.innerHTML = `<div class="si-empty-center"><i data-lucide="message-square" class="lucide"></i><span class="si-empty-title">${(0, import_i18n.t)("automation.noMessages") || "No messages"}</span></div>`;
      }
     ...
     */
    renderDetailBreadcrumb(name) {
      const rootLabel = (0, import_i18n.t)("automation.title") || "Automation";
      const trunc = (s, max = 18) => s.length > max ? s.slice(0, max) + "\u2026" : s;
    ...
    renderSubAgentTimeline(messages) {
      if (this.chatRenderer) {
        ...
        this.chatRenderer.renderSubAgentInto(this.detailContentEl, (0, import_state.restoreMessages)(messages), !!isRunning);
      }
    }
    /**
    ...
    renderTaskCard(prompt, state) {
      ...
      const label = state === "COMPLETED" ? (0, import_i18n.t)("automation.stateSuccess") : state === "FAILED" ? (0, import_i18n.t)("automation.stateFailed") : state === "RUNNING" ? (0, import_i18n.t)("automation.stateRunning") : (0, import_i18n.t)("automation.statePending");
      ...
      <div class="automation-detail-card">
        ...
        <div class="automation-detail-task-label">${this.escapeHtml((0, import_i18n.t)("automation.taskContent") || "Task")}</div>
    ...
    bindCallbacks() {
      (0, import_stream.onAutomationJobs)((jobs) => {
        this.jobs = jobs;
      ...
      (0, import_state.subscribe)(() => {
        ...
        const cur = (0, import_state.getState)().automationHistory;
        if (cur !== prev) {
      ...
      (0, import_stream.onAutomationJobCreated)(() => {
        this.requestJobsList();
      });
      (0, import_stream.onAutomationJobUpdated)(() => {
        this.requestJobsList();
      });
      (0, import_stream.onAutomationJobCancelled)(() => {
        this.requestJobsList();
      });
    }
    /** Requests the list of configured automation jobs from the backend. */
    requestJobsList() {
      (0, import_ws.send)({ type: "automation_list_jobs" });
    }
    /** Requests the automation run history from the backend. */
    requestHistory() {
      (0, import_ws.send)({ type: "automation_get_history" });
    }
    ...
    openDialog(template, editJob) {
      const isEdit = !!editJob;
      const defaultCron = editJob?.cron || template.defaultCron;
      const parsed = parseCronForUI(defaultCron);
      const title = isEdit ? (0, import_i18n.t)("automation.editTask") : (0, import_i18n.t)("automation.createTaskTitle");
      const btnLabel = isEdit ? (0, import_i18n.t)("automation.save") : (0, import_i18n.t)("automation.create");
      const _dlgModels = (0, import_state.getState)().modelConfigs || [];
      const initModelIndex = (() => {
        const idx = editJob?.model_index ?? (0, import_state.getState)().activeModelIndex;
      ...
      <div class="toast-dialog dialog-wide">
        ...
        <div class="dialog-body">
            <div class="model-form-row">
              <label class="model-form-label">${(0, import_i18n.t)("automation.taskName")}</label>
              <input type="text" id="auto-dlg-name" class="model-form-input" placeholder="${(0, import_i18n.t)("automation.taskNamePlaceholder")}" value="${escapeHtml(editJob?.name || (template.defaultNameKey ? (0, import_i18n.t)(`automation.${template.defaultNameKey}`) : ""))}" />
            </div>
            <div class="model-form-row">
              <label class="model-form-label">${(0, import_i18n.t)("automation.triggerTime")}</label>
              <div class="model-form-dropdown-row">
                <div class="settings-dropdown-wrap" id="auto-dlg-schedule-wrap">
                  <button class="settings-dropdown-trigger" id="auto-dlg-schedule-trigger" type="button">
                    ...
                    <span>${parsed.scheduleType === "daily" ? (0, import_i18n.t)("automation.everyDay") : parsed.scheduleType === "weekly" ? (0, import_i18n.t)("automation.everyWeek") : (0, import_i18n.t)("automation.everyHour")}</span>
                  ...
                  <div class="settings-dropdown" id="auto-dlg-schedule-dropdown">
                    <div class="settings-dropdown-item" data-value="daily"><span>${(0, import_i18n.t)("automation.everyDay")}</span></div>
                    <div class="settings-dropdown-item" data-value="weekly"><span>${(0, import_i18n.t)("automation.everyWeek")}</span></div>
                    <div class="settings-dropdown-item" data-value="hourly"><span>${(0, import_i18n.t)("automation.everyHour")}</span></div>
                  </div>
                </div>
              </div>
          ...
          <div class="model-form-row">
            <label class="model-form-label">${(0, import_i18n.t)("automation.modelLabel")}</label>
            <div class="model-form-dropdown-row">
          ...
          <div class="settings-card" style="margin-bottom:12px;margin-top:8px;overflow:hidden">
            <div class="settings-item-row">
              <div class="settings-item-info">
                <div class="settings-item-title">
                  <span>${(0, import_i18n.t)("automation.pushCardTitle")}</span>
                </div>
                <div class="settings-item-desc">${(0, import_i18n.t)("automation.enablePushHint")}</div>
              </div>
              <div class="settings-item-control">
                <label class="toggle-switch" title="${(0, import_i18n.t)("automation.enablePush")}">
            ...
            <div id="auto-dlg-push-gateways-row" style="${editJob?.push_gateways?.length ? "" : "display:none"}">
              <div class="auto-push-gateways">
                <div class="auto-push-gateways-label">${(0, import_i18n.t)("automation.pushGateways")}</div>
                <div id="auto-dlg-push-gateways"></div>
              </div>
            </div>
          </div>
          <div class="model-form-row" style="flex-direction:column;align-items:stretch">
            <label class="model-form-label">${(0, import_i18n.t)("automation.whatToDo")}</label>
            <div class="input-wrapper input-wrapper--dialog">
              <textarea id="auto-dlg-prompt" class="setting-rich-text" placeholder="${(0, import_i18n.t)("automation.whatToDo")}" style="min-height:80px">${escapeHtml(editJob?.prompt || template.defaultPrompt)}</textarea>
            </div>
          </div>
        </div>
        <div class="dialog-footer">
          <button class="btn" id="auto-dlg-cancel">${(0, import_i18n.t)("dialog.cancel")}</button>
      ...
      const timeTrigger = overlay.querySelector("#auto-dlg-time-trigger");
      ...
      const buildTimeOptions = () => {
        ...
        const renderCol = (col, count, selected, onPick) => {
          col.innerHTML = Array.from(
            { length: count },
            (_, i) => `<div class="settings-dropdown-item time-picker-cell${i === selected ? " selected" : ""}" data-value="${i}">${pad2(i)}</div>`
          ).join("");
      ...
      const buildModelOptions = () => {
        const allModels = (0, import_state.getState)().modelConfigs;
        ...
        modelDropdown.querySelectorAll(".settings-dropdown-item").forEach((item) => {
          item.addEventListener("click", (e) => {
            ...
            const models2 = (0, import_state.getState)().modelConfigs;
      ...
      const pushToggle = overlay.querySelector("#auto-dlg-push-toggle");
      const pushGatewaysRow = overlay.querySelector("#auto-dlg-push-gateways-row");
      const pushGatewaysContainer = overlay.querySelector("#auto-dlg-push-gateways");
      const buildPushGateways = () => {
        const gs = (0, import_state.getState)().gatewayStatus;
        const adapters = gs?.adapters || [];
        const available = adapters.filter((a) => a.connected).slice().sort(
          (a, b) => gatewayDisplayName(a.name).localeCompare(gatewayDisplayName(b.name), void 0, { sensitivity: "base" })
        );
        if (available.length === 0) {
          pushGatewaysContainer.innerHTML = `<span style="color:var(--text-muted);font-size:13px">${(0, import_i18n.t)("automation.pushGatewaysEmpty")}</span>`;
          return;
        }
        ...
        pushGatewaysContainer.innerHTML = available.map((a) => {
          ...
          const iconHtml = (0, import_icons.platformIconHtml)(a.name, 18);
      ...
      overlay.querySelector("#auto-dlg-ok")?.addEventListener("click", () => {
        const name = overlay.querySelector("#auto-dlg-name")?.value.trim();
        if (!name) return;
        const prompt = overlay.querySelector("#auto-dlg-prompt")?.value?.trim() || template.defaultPrompt;
        ...
        let selectedPushGateways = [];
        const pushToggleEl = overlay.querySelector("#auto-dlg-push-toggle");
        ...
        if (isEdit && editJob) {
          ...
          (0, import_ws.send)({
            ...
            prompt,
            cron,
            tag: template.titleKey ? (0, import_i18n.t)(template.titleKey) : "",
            model_index: selectedModelIndex,
            push_gateways: selectedPushGateways
          });
        }
    ...
    bindCreateButton() {
      const renderDropdown = () => {
        ...
        <span>${(0, import_i18n.t)("automation.customCreate")}</span>
      </div>`;
        ...
        <div class="settings-dropdown-item" data-template-id="${tmpl.id}">
          ...
          <span>${(0, import_i18n.t)(tmpl.titleKey)}</span>
        </div>
      `
        ).join("");
        if (typeof window.lucide !== "undefined") {
          window.lucide.createIcons({ root: this.createDropdown });
        }
        this.createDropdown.querySelectorAll(".settings-dropdown-item").forEach((item) => {
          item.addEventListener("click", (e) => {
            ...
            if (!id) {
              const blank = { id: "custom", icon: "plus", titleKey: "", descKey: "", defaultNameKey: "", defaultPrompt: "", defaultCron: "" };
              this.openDialog(blank);
            } else {
              const template = TEMPLATES.find((t2) => t2.id === id);
      ...
      document.addEventListener("click", (e) => {
        if (!this.createWrap.contains(e.target)) {
          this.createDropdown.classList.remove("open");
        }
      });
    }
    ...
    renderConfigured() {
      ...
      if (this.jobs.length === 0) {
        ...
        <div class="si-panel-empty-title">${(0, import_i18n.t)("automation.noTasks")}</div>
        <div class="si-panel-empty-sub">${(0, import_i18n.t)("automation.noTasksHint")}</div>
      </div>`;
        if (typeof window.lucide !== "undefined") {
      ...
      const stateTag = (state, suspended) => {
        if (suspended) return `<span class="model-active-tag" style="color:var(--text-muted);background:rgba(156,163,175,0.12)">${(0, import_i18n.t)("automation.statePaused")}</span>`;
        switch (state) {
          case "RUNNING":
            return `<span class="model-active-tag" style="color:var(--success);background:rgba(34,197,94,0.12)">${(0, import_i18n.t)("automation.stateRunning")}</span>`;
          case "COMPLETED":
            return `<span class="model-active-tag" style="color:var(--success);background:rgba(34,197,94,0.12)">${(0, import_i18n.t)("automation.stateSuccess")}</span>`;
          case "FAILED":
            return `<span class="model-active-tag" style="color:var(--danger);background:rgba(239,68,68,0.12)">${(0, import_i18n.t)("automation.stateFailed")}</span>`;
          case "PENDING":
            return `<span class="model-active-tag" style="color:var(--warning);background:rgba(234,179,8,0.12)">${(0, import_i18n.t)("automation.statePending")}</span>`;
          default:
            return `<span class="model-active-tag" style="color:var(--text-muted);background:rgba(156,163,175,0.12)">${escapeHtml(state)}</span>`;
        }
      };
      let rowsHtml = "";
      for (const job of this.jobs) {
        ...
        <div class="model-table-row" data-configured-id="${escapeHtml(job.id)}">
          ...
          <div class="model-table-cell model-cell-actions">
            ...
            <button class="btn-icon" data-action="edit" data-tooltip="${(0, import_i18n.t)("general.edit")}">
              <i data-lucide="pencil" class="lucide"></i>
            </button>
            <button class="btn-icon btn-icon--danger" data-action="delete" data-tooltip="${(0, import_i18n.t)("general.delete")}">
      ...
      <div class="settings-card">
        <div class="model-manage-header">
          <div class="model-manage-desc">${(0, import_i18n.t)("automation.configuredDesc")}</div>
        </div>
        <div class="model-table">
          <div class="model-table-header">
            <div class="model-table-cell model-cell-name">${(0, import_i18n.t)("automation.taskName")}</div>
            <div class="model-table-cell model-cell-provider">${(0, import_i18n.t)("automation.triggerTime")}</div>
            <div class="model-table-cell model-cell-actions">${(0, import_i18n.t)("automation.configured")}</div>
          </div>
          ${rowsHtml}
        </div>
      </div>`;
      if (typeof window.lucide !== "undefined") {
        window.lucide.createIcons({ root: this.configuredListEl });
      }
      this.configuredListEl.querySelectorAll(".model-table-row").forEach((row) => {
        const id = row.getAttribute("data-configured-id");
    ...
    bindHistoryFilters() {
      ...
      const rebindStatus = () => {
        const statusOptions = [
          { id: "", label: (0, import_i18n.t)("automation.filterAll") },
          { id: "COMPLETED", label: (0, import_i18n.t)("automation.filterSuccess") },
          { id: "FAILED", label: (0, import_i18n.t)("automation.filterFailed") },
          { id: "RUNNING", label: (0, import_i18n.t)("automation.stateRunning") },
          { id: "PENDING", label: (0, import_i18n.t)("automation.statePending") }
        ];
      ...
      const renderTaskOptions = () => {
        const opts = [{ id: "", label: (0, import_i18n.t)("automation.filterAllTasks") }];
        const seenNames = /* @__PURE__ */ new Set();
        for (const h of (0, import_state.getState)().automationHistory) {
      ...
      if (dateFromText) dateFromText.textContent = (0, import_i18n.getLocale)() === "en" ? "Start Date" : "\u5F00\u59CB\u65E5\u671F";
      if (dateToText) dateToText.textContent = (0, import_i18n.getLocale)() === "en" ? "End Date" : "\u7ED3\u675F\u65E5\u671F";
      const MONTHS_SHORT = (0, import_i18n.getLocale)() === "en" ? ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"] : ["1\u6708", "2\u6708", "3\u6708", "4\u6708", "5\u6708", "6\u6708", "7\u6708", "8\u6708", "9\u6708", "10\u6708", "11\u6708", "12\u6708"];
      const WEEKDAYS_SHORT = (0, import_i18n.getLocale)() === "en" ? ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"] : ["\u4E00", "\u4E8C", "\u4E09", "\u56DB", "\u4E94", "\u516D", "\u65E5"];
      ...
      <div class="cal-grid">`;
        ...
        container.querySelectorAll(".cal-day[data-date]").forEach((el) => {
          el.addEventListener("click", (e) => {
            e.stopPropagation();
            onSelect(el.getAttribute("data-date") || "");
      ...
      bindDateField(dateFromBtn, dateFromDD, dateFromText, dateFromHidden, (v) => {
        this.historyDateFrom = v;
        this.renderHistory();
      });
      bindDateField(dateToBtn, dateToDD, dateToText, dateToHidden, (v) => {
      ...
      if (dateClear) {
        dateClear.setAttribute("title", (0, import_i18n.t)("automation.clearDate"));
    ...
    updateHistoryFilterLabels() {
      ...
      if (this._dateFromText && !this.historyDateFrom) {
        this._dateFromText.textContent = (0, import_i18n.getLocale)() === "en" ? "Start Date" : "\u5F00\u59CB\u65E5\u671F";
      }
      if (this._dateToText && !this.historyDateTo) {
        this._dateToText.textContent = (0, import_i18n.getLocale)() === "en" ? "End Date" : "\u7ED3\u675F\u65E5\u671F";
      }
    }
    exportHistory() {
      ...
      const lines = [];
      lines.push(`# ${(0, import_i18n.t)("automation.exportHistoryTitle")}`);
      lines.push("");
      const now = formatDateTime(Math.floor(Date.now() / 1e3));
      lines.push(`*${now}*`);
      lines.push("");
      lines.push(`**${this.currentFilteredHistory.length}** ${(0, import_i18n.t)("automation.history")}`);
      lines.push("");
      ...
      for (const [taskName, entries] of taskMap) {
        ...
        for (const e of sorted) {
          ...
          const stateBadge = e.state === "SUCCESS" || e.state === "COMPLETED" ? "\u2705" : e.state === "FAILED" ? "\u274C" : e.state === "RUNNING" ? "\u{1F504}" : "\u23F3";
          lines.push(`### ${stateBadge} ${ts} \u2014 ${stateLabel}`);
          if (e.tag && e.tag !== e.name) {
            lines.push("");
            lines.push(`- **${(0, import_i18n.t)("automation.modelLabel")}**: ${e.tag}`);
          }
          if (e.fail_count && e.fail_count > 0) {
            lines.push(`- **${(0, import_i18n.t)("automation.failedCount")}**: ${e.fail_count}`);
          }
          if (e.last_result) {
            lines.push("");
      ...
      (0, import_stream.downloadMarkdownFile)(md, filename);
    }
    stripControlChars(s) {
      return s.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "");
    }
    getStateLabel(state) {
      switch (state) {
        case "SUCCESS":
        case "COMPLETED":
          return (0, import_i18n.t)("automation.stateSuccess");
        case "FAILED":
          return (0, import_i18n.t)("automation.stateFailed");
        case "RUNNING":
          return (0, import_i18n.t)("automation.stateRunning");
        case "PENDING":
          return (0, import_i18n.t)("automation.statePending");
        case "PAUSED":
          return (0, import_i18n.t)("automation.statePaused");
        default:
          return state;
      }
    }
    renderHistoryDropdown(id, options, currentId, onChange) {
    ...
    renderHistory() {
      const filtersEl = document.getElementById("history-filters");
      let displayHistory = [...(0, import_state.getState)().automationHistory];
      ...
      if (displayHistory.length === 0 && (0, import_state.getState)().automationHistory.length === 0) {
        filtersEl.style.display = "none";
        this.historyTimelineEl.innerHTML = `<div class="si-panel-empty">
        <i data-lucide="history" class="lucide"></i>
        <div class="si-panel-empty-title">${(0, import_i18n.t)("automation.noHistory")}</div>
        <div class="si-panel-empty-sub">${(0, import_i18n.t)("automation.noHistoryHint")}</div>
      </div>`;
        if (typeof window.lucide !== "undefined") {
      ...
      if (displayHistory.length === 0) {
        ...
        <div class="si-panel-empty-title">${(0, import_i18n.t)("automation.noMatchingHistory")}</div>
      </div>`;
        if (typeof window.lucide !== "undefined") {
          window.lucide.createIcons({ root: this.historyTimelineEl });
        }
          ...
          <div class="history-timeline-item" data-entry-id="${escapeHtml(record.id)}"${record.session_id ? ` data-sid="${escapeHtml(record.session_id)}"` : ""}>
            ...
            <div class="history-timeline-content">
              ...
              <div class="history-timeline-meta">
                ${metaText}
                ${record.fail_count > 0 ? `(${(0, import_i18n.t)("automation.failedCount")}: ${record.fail_count})` : ""}
              </div>
            </div>
          </div>
        `;
    ...
    `
      ...
      this.historyTimelineEl.querySelectorAll(".history-timeline-item").forEach((item) => {
        item.addEventListener("click", () => {
          ...
          const entry = (0, import_state.getState)().automationHistory.find((h) => h.id === entryId);
          if (!entry) return;
      ...
      this.historyTimelineEl.querySelectorAll(".history-timeline-item").forEach((item) => {
        item.addEventListener("contextmenu", (e) => {
          ...
          const doRename = () => {
            if (!entry) return;
            (0, import_session.showRenameDialog)(entry.name || "", (newName) => {
              if (entryId) {
                (0, import_ws.send)({ type: "automation_rename_execution", entry_id: entryId, new_name: newName });
              }
            });
          };
          const doDelete = () => {
            if (entryId) {
              import_dialog.Dialog.confirm((0, import_i18n.t)("automation.confirmDeleteRecord") || "Delete this record?", "").then((confirmed) => {
                if (confirmed) {
                  const history = (0, import_state.getState)().automationHistory.filter((h) => h.id !== entryId);
                  (0, import_state.setAutomationHistory)(history);
                  (0, import_ws.send)({ type: "automation_delete_execution", entry_id: entryId });
                }
              });
            }
          };
          if (sid && !isFailed) {
            (0, import_session.showSessionContextMenu)(sid, e.clientX, e.clientY, true, false, void 0, doRename, true);
          } else if (sid && isFailed) {
            (0, import_session.showSessionContextMenu)(sid, e.clientX, e.clientY, true, true, doDelete, doRename, true);
          } else if (entryId) {
    ...
    showAutomationHistoryContextMenu(x, y, doRename, doDelete) {
      ...
      <div class="context-menu-item" id="ctx-automation-rename">
        ...
        <span>${this.escapeHtml((0, import_i18n.t)("session.rename"))}</span>
      </div>
      ...
      <div class="context-menu-item context-menu-item-danger" id="ctx-automation-delete">
        ...
        <span>${this.escapeHtml((0, import_i18n.t)("session.delete"))}</span>
      </div>`;
      (0, import_context_menu.showContextMenu)(menuEl, x, y);
    ...
    onConfiguredAction(id, action) {
      if (action === "delete") {
        const job = this.jobs.find((j) => j.id === id);
        const name = job?.name || id;
        import_dialog.Dialog.confirm(
          (0, import_i18n.t)("automation.confirmDeleteTaskTitle") || "Delete Task",
          (0, import_i18n.t)("automation.confirmDeleteTask", { name }) || `Delete task "${name}"? This cannot be undone.`
        ).then((confirmed) => {
          if (!confirmed) return;
          (0, import_ws.send)({ type: "automation_delete_job", job_id: id });
          this.requestJobsList();
          this.requestHistory();
        });
      }
    }
    onConfiguredToggle(id) {
      const job = this.jobs.find((j) => j.id === id);
      if (!job) return;
      (0, import_ws.send)({ type: "automation_toggle_job", job_id: id });
    }
    onConfiguredEdit(job) {
      const fakeTemplate = {
        id: "edit",
        icon: "edit",
