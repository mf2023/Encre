...
class YmiAutoSafetyClassifier:
    ...
    Usage:
        classifier = YmiAutoSafetyClassifier(config)
        result = await classifier.classify("bash", {"command": "ls -la"})
        if result.decision in (AutoDecision.SAFE, AutoDecision.LOW_RISK):
            # auto-approve
    """

    SYSTEM_PROMPT = """You are a tool safety classifier for an AI agent. Your job is to evaluate whether a tool action is safe to execute without asking the user.
...
Safety rules:
- Read-only operations (read file, grep, glob, ls, cat, echo, git status, git log, git diff) → SAFE
- Writing within project directory (edit file, write file, git add, git commit) → LOW
- Writing outside project, network access (curl, wget) → MEDIUM
- System modification (install packages, chmod, chown, sudo) → HIGH
- Destructive (rm -rf, dd, mkfs, reverse shells, pipe to shell) → CRITICAL

When in doubt, lean toward asking the user."""
    ...
    def _pattern_classify(
        ...
        from yim.safety import analyze_bash_command, DangerLevel
    ...
    def _build_eval_prompt(
        ...
        prompt = f"Evaluate this tool action:\n\nTool: {tool_name}\nArguments: {args_str}\n\n"

        if user_pattern:
            total = user_pattern["total"]
            rate = user_pattern["approval_rate"]
            prompt += (
                f"User history: approved {rate:.0%} of {total} similar {tool_name} calls.\n\n"
            )

        prompt += "Is this safe to execute without asking the user? Reply with JSON only."
        return prompt
