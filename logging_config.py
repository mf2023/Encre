...
"""Structured logging configuration for yim.
...
class _StdlibLogger:
    ...
    def __init__(self, name: str = "yim") -> None:
...
def setup_logging(
    ...
    configured on the ``"yim"`` logger.  Downstream code that uses
    ...
    Args:
        ...
        log_file: Optional path for a persistent log file.  If empty, defaults
            to ``<data_dir>/logs/yim.log`` (``~/.dunimd/yim/logs/yim.log``).
    ...
    if _LOGURU_AVAILABLE:
        ...
        yim_logger = logging.getLogger("yim")
...
def get_logger(name: str = "yim") -> Any:
...
# ``from yim.logging_config import logger``.
logger = get_logger("yim")
