...
"""Tests for YmiAgent: construction, properties, run, lifecycle."""
...
class TestYmiAgentConstruction:
    """Verify YmiAgent can be constructed with various configurations."""

    def test_creation_with_no_args(self):
        agent = YmiAgent()
        assert agent is not None
        assert isinstance(agent.config, YmiConfig)

    def test_creation_with_explicit_config(self):
        config = YmiConfig(model="gpt-4o-mini", max_tokens=1000)
        agent = YmiAgent(config=config)
...
class TestYmiAgentProperties:
    """Verify YmiAgent exposes expected attributes after construction."""
...
class TestYmiAgentLifecycle:
    """Verify YmiAgent lifecycle methods exist."""
...
class TestYmiAgentGoalAndSwarm:
