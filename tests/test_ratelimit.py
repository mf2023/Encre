...
class TestRateLimitResult:
    def test_allowed_result(self):
        from yim.ratelimit import RateLimitResult
...
class TestYmiRateLimiterConstruction:
    def test_default_values(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter()
...
class TestYmiRateLimiterCheck:
    ...
    def test_multiple_checks_track_count(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(per_minute=100)
        for _ in range(10):
            result = limiter.check("bash")
            assert result.allowed is True
    ...
    def test_per_minute_limit_exceeded(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(per_minute=5, per_hour=99999, per_day=99999)
        for _ in range(5):
            result = limiter.check("bash")
            assert result.allowed is True
    ...
    def test_per_day_limit_exceeded(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(per_minute=99999, per_hour=99999, per_day=3)
        for _ in range(3):
            result = limiter.check("bash")
            assert result.allowed is True
...
class TestYmiRateLimiterSlots:
    @pytest.mark.asyncio
    async def test_acquire_slot_below_limit(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(max_concurrent=10)
    ...
    async def test_acquire_multiple_slots(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(max_concurrent=5)
        await limiter.acquire_slot()
        await limiter.acquire_slot()
        await limiter.acquire_slot()
    ...
    async def test_acquire_slot_blocks_when_at_capacity(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(max_concurrent=1)
        await limiter.acquire_slot()
        assert limiter._concurrent_count == 1
...
class TestYmiRateLimiterBackoff:
...
class TestYmiRateLimiterReset:
    def test_reset_clears_windows(self):
        from yim.ratelimit import YmiRateLimiter
        limiter = YmiRateLimiter(per_minute=5)
        for _ in range(3):
            limiter.check("bash")
...
class TestYmiRateLimiterActiveTools:
