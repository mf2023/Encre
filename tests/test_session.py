...
"""Tests for YmiSession and SessionCheckpoint."""
...
class TestYmiSessionConstruction:
    """Verify YmiSession can be constructed."""
    ...
    def test_session_config_stored(self):
        config = YmiConfig(max_tokens=9999)
        session = YmiSession(config)
        assert session.config is config
        assert session.config.max_tokens == 9999


class TestYmiSessionMessages:
...
class TestYmiSessionCheckpoints:
...
class TestYmiSessionUtility:
    ...
    def test_is_expired_false_for_new_session(self):
        config = YmiConfig(session_max_age_hours=24.0)
        session = YmiSession(config)
        assert session.is_expired() is False

    def test_is_max_turns_reached(self):
        config = YmiConfig(max_turns=10)
        session = YmiSession(config)
    ...
    def test_estimate_tokens_static(self):
        count = YmiSession.estimate_tokens("hello world")
        assert isinstance(count, int)
        assert count > 0

    def test_estimate_tokens_empty(self):
        count = YmiSession.estimate_tokens("")
        assert count == 0

    def test_count_messages_tokens_static(self):
        msgs = [{"role": "user", "content": "hello world"}]
        count = YmiSession.count_messages_tokens(msgs)
        assert isinstance(count, int)
        assert count > 0
    ...
    def test_from_dict_roundtrip(self):
        config = YmiConfig()
        session1 = YmiSession(config)
        session1.add_message("user", "hello")
        data = session1.to_dict()
        session2 = YmiSession.from_dict(data, config)
