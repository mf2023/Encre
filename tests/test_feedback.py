...
"""Tests for yim.feedback — error correction learner with Jaccard similarity."""
...
from yim.feedback import YmiFeedbackLearner, CorrectionRecord
from yim.feedback.learner import cut_str
...
class TestYmiFeedbackLearner:
    def setup_method(self):
        self.learner = YmiFeedbackLearner()
    ...
    def test_save_and_load(self, tmp_path: Path):
        ...
        learner = YmiFeedbackLearner(storage_path=storage)
        ...
        learner2 = YmiFeedbackLearner(storage_path=storage)
        loaded = learner2.load()
        assert loaded is True
        assert learner2.record_count == 2

    def test_load_nonexistent(self):
        learner = YmiFeedbackLearner(storage_path="/nonexistent/path/file.json")
        result = learner.load()
        assert result is False

    def test_load_no_path(self):
        learner = YmiFeedbackLearner()  # no storage_path
        result = learner.load()
        assert result is False
