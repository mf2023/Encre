...
"""Tests for yim.git.repo (YmiGitRepo, GitState) and yim.git.diff (YmiGitDiff, GitDiffResult)."""
...
class TestYmiGitRepo:
    """Tests for YmiGitRepo using an actual git repo."""
    ...
    def test_construction_absolute_path(self):
        ...
        repo = YmiGitRepo(workspace=abs_path)
    ...
    def test_get_state_not_in_repo_for_non_git_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = YmiGitRepo(workspace=tmpdir)
...
# ===========================================================================
# YmiGitDiff static methods
# ===========================================================================

class TestYmiGitDiff:
    """Tests for YmiGitDiff static methods."""

    def test_compute_diff_returns_str(self):
        from yim.git.diff import YmiGitDiff
        diff = YmiGitDiff.compute_diff(
            old="line1\nline2\nline3\n",
            new="line1\nline2 modified\nline3\nline4\n",
        )
    ...
    def test_compute_diff_no_changes(self):
        from yim.git.diff import YmiGitDiff
        content = "hello world\nfoo bar\n"
        diff = YmiGitDiff.compute_diff(old=content, new=content)
        assert isinstance(diff, str)

    def test_compute_diff_empty_to_content(self):
        from yim.git.diff import YmiGitDiff
        diff = YmiGitDiff.compute_diff(old="", new="line1\nline2\n")
        assert isinstance(diff, str)
        assert len(diff) > 0

    def test_compute_diff_content_to_empty(self):
        from yim.git.diff import YmiGitDiff
        diff = YmiGitDiff.compute_diff(old="line1\nline2\n", new="")
        assert isinstance(diff, str)
    ...
    def test_apply_diff_returns_str(self):
        ...
        result = YmiGitDiff.apply_diff(content=original, diff=diff)
        assert isinstance(result, str)
    ...
    def test_apply_diff_noop(self):
        ...
        content = "hello\nworld\n"
        diff = YmiGitDiff.compute_diff(old=content, new=content)
        result = YmiGitDiff.apply_diff(content=content, diff=diff)
        assert isinstance(result, str)

    def test_parse_diff_stats_returns_dict(self):
        from yim.git.diff import YmiGitDiff
        stats = YmiGitDiff.parse_diff_stats("3\t2\tfile.py\n")
        assert isinstance(stats, dict)
    ...
    def test_parse_diff_stats_empty(self):
        from yim.git.diff import YmiGitDiff
        stats = YmiGitDiff.parse_diff_stats("")
    ...
    def test_parse_diff_stats_multiple_files(self):
        ...
        stats = YmiGitDiff.parse_diff_stats(output)
    ...
    def test_is_transient_git_state_returns_bool(self):
        from yim.git.diff import YmiGitDiff
        result = YmiGitDiff.is_transient_git_state(workspace=".")
        assert isinstance(result, bool)

    def test_is_transient_git_state_non_existent_dir(self):
        from yim.git.diff import YmiGitDiff
        result = YmiGitDiff.is_transient_git_state(workspace="/nonexistent/path/xyz")
    ...
    def test_roundtrip_compute_and_apply(self):
        ...
        original = "def foo():\n    return 1\n\n\ndef bar():\n    return 2\n"
        modified = "def foo():\n    return 42\n\n\ndef bar():\n    return 2\n\ndef baz():\n    return 3\n"
        diff = YmiGitDiff.compute_diff(old=original, new=modified)
        applied = YmiGitDiff.apply_diff(content=original, diff=diff)
