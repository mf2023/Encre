...
class TestCodeIndex:
    def test_module_info(self):
        from yim.codebase.indexer import ModuleInfo
        mi = ModuleInfo(
    ...
    def test_code_index_create(self):
        from yim.codebase.indexer import YmiCodeIndex
        ci = YmiCodeIndex(workspace=".")
...
class TestLSPProtocol:
    def test_position(self):
        from yim.lsp.protocol import Position
        p = Position(line=10, character=5)
        assert p.line == 10
        assert p.character == 5

    def test_range(self):
        from yim.lsp.protocol import Position, Range
    ...
    def test_location(self):
        from yim.lsp.protocol import Position, Range, Location
    ...
    def test_diagnostic(self):
        from yim.lsp.protocol import Position, Range, Diagnostic
    ...
    def test_hover_result(self):
        from yim.lsp.protocol import HoverResult
    ...
    def test_hover_result_with_range(self):
        from yim.lsp.protocol import HoverResult, Position, Range
        r = Range(start=Position(line=1, character=0), end=Position(line=1, character=10))
        hr = HoverResult(contents="def foo()", range=r)
        assert hr.range is not None

    def test_lsp_state(self):
        from yim.lsp.protocol import LSPState
...
class TestGitTypes:
    def test_git_state_default(self):
        from yim.git.repo import GitState
    ...
    def test_git_diff_result(self):
        from yim.git.diff import GitDiffResult
    ...
    def test_git_repo_creation(self):
        from yim.git.repo import YmiGitRepo
        repo = YmiGitRepo(workspace=".")
...
class TestServerProtocol:
    def test_client_run(self):
        from yim.server.protocol import ClientRun
    ...
    def test_parse_client_message(self):
        from yim.server.protocol import parse_client_message
        import json
    ...
    def test_encode_server_message(self):
        from yim.server.protocol import encode_server_message
...
class TestSessionManager:
    def test_session_info(self):
        from yim.server.session_manager import SessionInfo
        from yim.agent import YmiAgent
        from yim.config import YmiConfig
        agent = YmiAgent(config=YmiConfig(backend_type="openai", api_key="sk-fake"))
    ...
    def test_session_manager_create_session(self):
        ...
        from yim.config import YmiConfig
        sm = SessionManager()
        info = sm.create_session(YmiConfig(backend_type="openai", api_key="sk-fake"))
    ...
    def test_session_manager_list_sessions(self):
        ...
        from yim.config import YmiConfig
        sm = SessionManager()
        sm.create_session(YmiConfig(backend_type="openai", api_key="sk-fake"))
...
class TestAgentTypes:
    def test_goal_definition(self):
        from yim.goal import GoalDefinition
    ...
    def test_goal_result(self):
        from yim.goal import GoalResult, GoalStatus
    ...
    def test_goal_status(self):
        from yim.goal import GoalStatus
    ...
    def test_goal_event(self):
        from yim.goal import GoalEvent, GoalStatus
    ...
    def test_session_checkpoint(self):
        from yim.session import SessionCheckpoint
    ...
    def test_goal_loop_create(self):
        ...
        from yim.config import YmiConfig
        agent = YmiAgent(config=YmiConfig(backend_type="openai", api_key="sk-fake"))
        loop = YmiGoalLoop(agent=agent, description="test", success_criteria="works")
        assert loop is not None
        assert loop._description == "test"

    def test_goal_runner_create(self):
        from yim.goal import YmiGoalRunner
        from yim.config import YmiConfig
        ...
        config = YmiConfig(backend_type="openai", api_key="sk-fake")
        registry = ToolRegistry()
        hooks = YmiHookSystem()
        safety = YmiSafetyEngine(config=config)
        runner = YmiGoalRunner(
            config=config,
