...
class TestGoalResult:
    ...
    def test_is_dataclass(self):
        from dataclasses import is_dataclass
        from yim.goal import GoalResult
...
class TestGoalEvent:
    ...
    def test_is_dataclass(self):
        from dataclasses import is_dataclass
        from yim.goal import GoalEvent
        assert is_dataclass(GoalEvent)


class TestYmiGoalRunnerConstruction:
    def test_construction_with_config(self):
        from yim.goal import YmiGoalRunner
        from yim.config import YmiConfig
        config = YmiConfig(model="gpt-4o", backend_type="openai")
        runner = YmiGoalRunner(config=config)
    ...
    def test_build_goal_prompt(self):
        from yim.goal import YmiGoalRunner, GoalDefinition
        from yim.config import YmiConfig
        config = YmiConfig(model="gpt-4o", backend_type="openai")
        runner = YmiGoalRunner(config=config)
        goal = GoalDefinition(
...
class TestYmiGoalLoopConstruction:
    def test_construction_basic(self):
        ...
        # Manually create a minimal mock for YmiAgent
        import tempfile
        class MockAgent:
            def __init__(self):
                self.config = config
                ...
                self.hook_system = YmiHookSystem()
                self.safety = YmiSafetyEngine(config)
                self.memory_system = YmiMemorySystem(auto_memory_path=tempfile.mkdtemp())
                self.skill_registry = YmiSkillRegistry()
                self.telemetry = YmiTelemetry(enabled=False)
