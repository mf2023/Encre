...
class TestCompactionStrategies:
    ...
    def test_token_budget_should(self):
        s = YmiTokenBudgetStrategy(budget_ratio=0.5)
    ...
    def test_budget_reduction_execute(self):
        s = YmiBudgetReductionStrategy(max_chars_per_message=100)
        msgs = [_m("user", "x" * 5000)]
        result = asyncio.run(s.compact(msgs, 128000))
    ...
    def test_snip_execute(self):
        s = YmiSnipStrategy(keep_recent_turns=3)
        msgs = _make_messages(20)
        result = asyncio.run(s.compact(msgs, 128000))
        assert len(result) <= len(msgs)

    def test_micro_compact_execute(self):
        s = YmiMicroCompactStrategy()
    ...
    def test_context_collapse_execute(self):
        s = YmiContextCollapseStrategy()
        msgs = _make_messages(20)
        result = asyncio.run(s.compact(msgs, 128000))
        assert result is not None

    def test_semantic_should(self):
        s = YmiSemanticCompactStrategy()
    ...
    def test_multi_stage_has_six_stages(self):
        pipeline = YmiMultiStagePipeline()
    ...
    def test_auto_compact_strategy(self):
        s = YmiAutoCompactStrategy(threshold_ratio=0.5)
...
class TestCompactEngine:
    def test_create(self):
        engine = YmiCompactEngine()
        assert engine is not None

    def test_with_strategy(self):
        s = YmiAlwaysCompactStrategy()
        engine = YmiCompactEngine(strategy=s)
        assert engine is not None
