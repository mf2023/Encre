...
from yim.swarm.planner import YmiTaskPlanner, TaskTree, TaskNode, _detect_pattern
...
class TestMailbox:
    def test_create(self):
        mb = YmiMailbox(owner_id="agent1")
        assert mb.owner_id == "agent1"

    async def test_send_receive(self):
        mb_a = YmiMailbox(owner_id="a")
        mb_b = YmiMailbox(owner_id="b")
    ...
    async def test_receive_timeout(self):
        mb = YmiMailbox(owner_id="test", timeout=0.1)
...
class TestTeammate:
    def test_create_teammate(self):
        tm = YmiTeammate(name="coder", task="write a function")
...
class TestTaskPlanner:
    ...
    def test_plan_from_json(self):
        ...
        tree = YmiTaskPlanner.plan_from_json("test goal", json.dumps(data))
