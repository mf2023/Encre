...
class YmiSpreadsheetTool(YmiTool):
    name: ClassVar[str] = "spreadsheet"
    description: ClassVar[str] = "Read, write, and manipulate CSV and Excel spreadsheets"
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        try:
            if ext == ".csv":
                return await self._handle_csv(action, file_path, data)
            elif ext in (".xlsx", ".xls"):
                return await self._handle_excel(action, file_path, sheet_name, data, _range)
            else:
    ...
    async def _handle_csv(self, action: str, file_path: str, data: str) -> str:
        if action == "read":
    ...
    async def _handle_excel(self, action: str, file_path: str, sheet_name: str, data: str, _range: str) -> str:
        try:
            import openpyxl
        except ImportError:
