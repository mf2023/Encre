...
class YmiLSPTool(YmiTool):
    ...
    @classmethod
    def _get_manager(cls) -> YmiLSPManager:
        if cls._manager is None:
            cls._manager = YmiLSPManager()
        return cls._manager
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        if operation == "initialize":
            ...
            await self._get_manager().initialize_for_workspace(workspace)
            return "LSP servers initialized"

        if operation == "shutdown":
            await self._get_manager().shutdown()
            self.__class__._manager = None
    ...
    def _format_symbols(self, symbols: list[dict[str, Any]], indent: int = 0) -> str:
        lines: list[str] = []
        prefix = "  " * indent
        for sym in symbols:
            name = sym.get("name", "?")
