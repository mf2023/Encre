...
class YmiDeployTool(YmiTool):
    name: ClassVar[str] = "deploy"
    description: ClassVar[str] = "Deploy applications to cloud platforms and container registries"
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        cmd = self._build_command(target, action, config_file, project_name)
    ...
    def _build_command(self, target: str, action: str, config_file: str, project_name: str) -> list[str]:
        if target == "docker":
            if action == "build":
                cmd = ["docker", "build"]
                if config_file:
