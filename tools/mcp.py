...
logger = logging.getLogger("yim.tools.mcp")
...
# ──────────────────────────────────────────────────────────────────────
# YmiTool adapter — bridge between MCP and yim tool registry
# ──────────────────────────────────────────────────────────────────────


class YmiMCPTool(YmiTool):
    """Ymi tool that proxies calls to an MCP server using JSON-RPC 2.0.

    Supports two transport modes:

    **Stdio mode** (subprocess)::

        tool = YmiMCPTool(command="npx -y @anthropic/mcp-server-filesystem /tmp")
        await tool._connect()
    ...
    **HTTP mode** (streamable HTTP)::
        ...
        tool = YmiMCPTool(server_url="http://localhost:8000/mcp")
        await tool._connect()
        result = await tool.execute(tool_name="some_tool", arguments={...})

    The ``execute`` method implements the standard ``YmiTool`` interface so the
    tool can be used via the yim tool registry like any other tool.
    """
    ...
    async def _connect(self) -> None:
        ...
        async with self._connection_lock:
            ...
            logger.info("YmiMCPTool connected to MCP server")
    ...
    async def _disconnect(self) -> None:
        ...
        if self._client is not None:
            ...
            self._client = None
            logger.info("YmiMCPTool disconnected")
    ...
    # ------------------------------------------------------------------
    # YmiTool.execute — standard tool interface
    # ------------------------------------------------------------------

    async def execute(self, **kwargs: Any) -> str:
        """Execute an MCP tool call (standard ``YmiTool`` interface).
...
class _MCPDiscoveredTool(YmiTool):
    """A single MCP-discovered tool wrapped as a YmiTool.

    This is created automatically by ``YmiMCPTool.register_with()``.
    Each instance proxies to its parent ``YmiMCPTool`` for execution.
    """

    def __init__(self, mcp_tool: YmiMCPTool, tool_name: str,
        ...
        # but YmiTool uses ClassVar… we set them as instance attrs which
