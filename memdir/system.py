...
from yim.memdir.age import memory_age_days, memory_age_text, memory_freshness_text
from yim.memdir.entrypoint import load_entrypoint_raw, write_entrypoint
from yim.memdir.manifest import format_memory_manifest
from yim.memdir.semantic import SemanticMemorySearch, WorkingMemory, MemoryConsolidator
...
class YmiMemorySystem:
                ...
                for entry in entries:
                    ...
                    try:
                        with open(entry.path, "r", encoding="utf-8") as f:
                            head = "".join(
                                f.readline() for _ in range(FRONTMATTER_MAX_LINES)
                            )
    ...
    def build_prompt(self) -> str:
        ...
        parts.append(
            ...
            "Use memories to recall project context, conventions, "
            "decisions, and patterns. Memories are organized in "
            "MEMORY.md (the entrypoint) with supporting .md files."
        )
        parts.append("")
        ...
        parts.append("")
        parts.append(
            "To use memories: read relevant .md files from the memory directory. "
            "To create a memory: write a new .md file to the memory directory."
        )
