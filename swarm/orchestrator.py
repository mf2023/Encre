...
class YmiOrchestrator:
    ...
    def __init__(
        ...
        blackboard: YmiBlackboard | None = None,
        max_concurrent: int = 10,
        enable_reviewer_gate: bool = True,
    ) -> None:
        self._roles = role_registry or RoleRegistry()
        self._blackboard = blackboard or YmiBlackboard()
    ...
    async def _run_agent(self, node: Any, role: AgentRole, context: str) -> str:
        ...
        from yim.utils.types import TextDelta, ToolResult, Finish
    ...
    async def _run_simple_agent(self, role: AgentRole, prompt: str) -> str:
        ...
        config = YmiConfig(max_turns=5, permission_mode="auto")
        agent = YmiAgent(config=config)
        parts: list[str] = []
