...
class YmiConsensus:
    ...
    def __init__(self) -> None:
        ...
        self._mailbox = YmiMailbox(owner_id="consensus")
    ...
    async def run_proposal_vote(
        ...
        """
        from yim.swarm.teammate import YmiTeammate
        from yim.agent import YmiAgent
        from yim.config import YmiConfig
        from yim.utils.types import TextDelta
        ...
        async def _voter_task(voter: YmiTeammate) -> Vote | None:
            try:
                vote_prompt = (
                ...
                config = YmiConfig(max_turns=3, permission_mode="bypass")
    ...
    async def debate_mediate(
        self,
        topic: str,
        debaters: list["YmiTeammate"],
        mediator: "YmiTeammate",
        rounds: int = 3,
    ) -> ConsensusResult:
        ...
        async def _run_agent(prompt: str, max_turns: int = 5) -> str:
            config = YmiConfig(max_turns=max_turns, permission_mode="bypass")
            agent = YmiAgent(config=config)
            parts: list[str] = []
            try:
