...
class YmiSwarmManager:
    def __init__(self, max_concurrent: int = 30):
        self.max_concurrent = max_concurrent
        self._semaphore = asyncio.Semaphore(max_concurrent)
        self._teammates: "dict[str, YmiTeammate]" = {}
        self._handles: "dict[str, TeammateHandle]" = {}

    async def spawn(self, teammate: "YmiTeammate") -> "TeammateHandle":
    ...
    async def spawn_many(self, teammates: "list[YmiTeammate]") -> "list[TeammateHandle]":
        coros = [self.spawn(t) for t in teammates]
        results = await asyncio.gather(*coros, return_exceptions=True)
        handles: "list[TeammateHandle]" = []
        for r in results:
            if isinstance(r, Exception):
                from yim.swarm.teammate import TeammateHandle
                handles.append(
                    TeammateHandle(
                        teammate_id="error",
                        name="spawn_failed",
