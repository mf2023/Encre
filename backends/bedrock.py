            ...
            if isinstance(content, str):
                ...
                for item in content:
                    if item.get("type") == "text":
                        ...
                        blocks.append({"image": {"source": {"bytes": item.get("image_url", {}).get("url", "")}}})
                    ...
                    if "contentBlockStart" in event:
                        ...
                        if tool_input_delta and tool_use_buffer is not None:
                            ...
                            yield create_backend_tool_call_delta(
                                ...
                                tool_input_delta if isinstance(tool_input_delta, str) else json.dumps(tool_input_delta),
                            )
                        ...
                        if tool_use_buffer is not None:
                            yield create_backend_tool_call(
                                ...
                                arguments=tool_use_buffer["input"] if isinstance(tool_use_buffer["input"], str) else json.dumps(tool_use_buffer["input"]),
                            )
