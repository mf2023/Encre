...
- ``data: {"choices":[{"delta":{"tool_calls":[{"function":{"name":"search","arguments":""}}]}}]}`` — tool call start
- ``data: {"choices":[{"delta":{"tool_calls":[{"function":{"arguments":"{\"q\":"}}]}}]}`` — tool call argument delta
...
from yim.backends.retry import DEFAULT_RETRY_CONFIG, RetryConfig, retry_with_backoff
...
class OpenAISSEBackend(BaseBackend):
        ...
        if self._client is None:
            self._client = httpx.AsyncClient(
                timeout=httpx.Timeout(
                    connect=10.0,   # fail fast on unreachable hosts
    ...
    def _extract_extra_stream_events(
        ...
        """Extract extra events from a streaming SSE delta.

        Subclasses (e.g. DeepSeek) override this to emit
        :class:`BackendThinking` for ``reasoning_content``.
    ...
    def _extract_extra_non_stream_events(
        ...
        """Extract extra events from a non-streaming response message.
            ...
            async with client.stream(
                ...
                **extra_kwargs,
                ...
                if response.status_code >= 400:
                    ...
                    logging.getLogger("yim.backend").error(
