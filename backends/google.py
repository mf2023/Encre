        ...
        try:
            if not stream:
                ...
                if resp.status_code != 200:
                    yield create_backend_error(f"Gemini API error {resp.status_code}: {resp.text}")
                    return
            ...
            async with self._client.stream("POST", url, json=body) as resp:
                if resp.status_code != 200:
                    ...
                    yield create_backend_error(f"Gemini API error {resp.status_code}: {error_body.decode()}")
                    return
            ...
            yield create_backend_error("Request timed out")
