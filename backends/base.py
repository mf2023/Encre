...
class BaseBackend(ABC):
    ...
    Every backend in the ``yim.backends`` package extends this class and
    ...
    async def chat(
        ...
        Args:
            ...
            tool_choice: Controls tool selection behaviour.
                ...
                ``"none"`` — no tool usage; or a specific ``{"type": "function", "function": {"name": "..."}}``.
    ...
    def supports_thinking(self) -> bool:
        """Return True if the backend supports reasoning/thinking tokens.

        Thinking tokens are emitted as :class:`BackendThinking` events and
        represent the model's internal reasoning process before it produces
        its final answer.  Supported by Anthropic (Claude Opus/Sonnet 4.6+),
        DeepSeek (V4 with thinking mode), and Google (Gemini 2.5 Pro).
        """
        return False

    def supports_prompt_caching(self) -> bool:
        """Return True if the backend supports prompt caching for cost savings.

        Prompt caching reuses previously-processed prompt prefixes across
        requests, reducing latency and cost.  Supported by:
        - OpenAI GPT-4.1 family (75% off cached input)
        - OpenAI GPT-5.x (90% off cached input)
        - Anthropic Claude (90% off cache reads)
        - DeepSeek V4 (80-92% off cache hits)
