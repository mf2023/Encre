...
"""Yim cryptographic layer — provides encrypt/decrypt for all sensitive data.
...
Master key
    ...
    stored in ``~/.yim/keyfile`` (mode ``0o600``).  The on‑disk representation
...
_KEYFILE_PATH = pathlib.Path("~/.yim/keyfile").expanduser()
...
_HKDF_SALT = b"yim-crypto-hkdf-v1"
_HKDF_SALT_SHA256 = hashlib.sha256(_HKDF_SALT).digest()[:32]

_WRAP_CTX_AAD = b"yim-keywrap-v1"
