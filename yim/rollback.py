...
Each session gets a directory under ``~/.yim/rollback/<session_id>/``::
...
A commit blob contains::

    {
        ...
        "turn_count": 5,
        "message":  "turn_5",
        "state":    { ... full YmiSession.to_dict() output ... }
    }
...
__all__ = ["YmiRollbackGit", "CommitEntry"]


# ── storage layout ──────────────────────────────────────────────────────────

_BASE = pathlib.Path("~/.yim/rollback").expanduser()
...
class YmiRollbackGit:
    ...
    def commit(
        self,
        session: Any,  # YmiSession — duck‑typed for decoupling
        message: str = "",
    ) -> str:
