...
//!
//! Landlock is an unprivileged access-control mechanism built into the Linux
//! kernel.  This module provides a safe Rust builder that constructs Landlock
//! rulesets via raw syscalls, restricting filesystem and network access for
//! the **current thread** (and its future children).
//!
//! All functions are gated behind `#[cfg(target_os = "linux")]` and produce
//! descriptive `Result<(), String>` values.  On non-Linux targets stub
//! functions return an "unsupported" error.
