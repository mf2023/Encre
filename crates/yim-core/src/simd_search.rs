...
//!
//! SIMD-accelerated pattern matching engine.
//!
//! Uses 32-byte-wide parallel comparisons for fast substring and byte-level
//! search. When the `simd` feature is enabled the `wide` crate provides
//! portable SIMD; otherwise chunked-scalar fallbacks are used.
//!
//! All functions automatically fall back to the standard library when the
//! needle is shorter than 8 bytes (the per-byte overhead of SIMD setup
//! outweighs the parallelism gain for tiny patterns).
