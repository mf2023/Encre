...
required 鈥?uses aiohttp which is already an Encre dependency.
...
# practical limit for readable messages 鈥?matching OpenClaw's choice).
...
_CHANNEL_TYPE_MAP = {
    ...
    "P": "group",   # private channel 鈫?treat as group
...
class MattermostAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            async with self._session.get(url, headers=self._headers(), timeout=aiohttp.ClientTimeout(total=30)) as resp:
                ...
                if resp.status >= 400:
                    ...
                    logger.error("MM API GET %s 鈫?%s: %s", path, resp.status, body[:200])
        ...
        try:
            ...
            async with self._session.post(
                ...
                if resp.status >= 400:
                    ...
                    logger.error("MM API POST %s 鈫?%s: %s", path, resp.status, body[:200])
    ...
    async def _post_preserving_thread(
        ...
        flat_payload["message"] = (
            ...
            "鈿狅笍 Mattermost thread delivery failed; posting final reply in channel.\n\n"
        ...
        try:
            ...
            async with self._session.put(
                ...
                if resp.status >= 400:
                    ...
                    logger.error("MM API PUT %s 鈫?%s: %s", path, resp.status, body[:200])
    ...
    async def _upload_file(
        ...
        async with self._session.post(url, headers=headers, data=form, timeout=aiohttp.ClientTimeout(total=60)) as resp:
            ...
            if resp.status >= 400:
                ...
                logger.error("MM file upload 鈫?%s: %s", resp.status, body[:200])
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not me or "id" not in me:
            ...
            logger.error("Mattermost: failed to authenticate 鈥?check MATTERMOST_TOKEN and MATTERMOST_URL")
    ...
    def format_message(self, content: str) -> str:
        ...
        """Mattermost uses standard Markdown 鈥?mostly pass through.
        ...
        # Convert ![alt](url) to just the URL 鈥?Mattermost renders
    ...
    async def _ws_loop(self) -> None:
        ...
        while not self._closing:
            ...
            try:
                ...
                # Clean disconnect 鈥?reset delay.
                ...
                # succeed on retry 鈥?stop reconnecting instead of looping forever.
                ...
                if isinstance(exc, aiohttp.WSServerHandshakeError) and exc.status in {401, 403}:
                    ...
                    logger.error("Mattermost WS auth failed (HTTP %d) 鈥?stopping reconnect", exc.status)

                    return

                if "401" in err_str or "403" in err_str or "unauthorized" in err_str:

                    logger.error("Mattermost WS permanent error: %s 鈥?stopping reconnect", exc)

                    return

                logger.warning("Mattermost WS error: %s 鈥?reconnecting in %.0fs", exc, delay)
    ...
    async def _ws_connect_and_listen(self) -> None:
        ...
        # Build WS URL: https:// 鈫?wss://, http:// 鈫?ws://
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        if channel_type_raw != "D":
            ...
            # allowed_channels check (whitelist 鈥?must pass before other gating).
            ...
            try:
                ...
                async with self._session.get(
                    ...
                    if resp.status < 400:
                        ...
                        from encre.gateway.platforms.base import cache_image_from_bytes, cache_document_from_bytes
...
async def _standalone_send(
    ...
    *,
    ...
    on the ``POST /posts`` payload 鈥?pass ``thread_id`` when threading is
    ...
    standalone senders but unused 鈥?Mattermost stores every uploaded file
...
def interactive_setup() -> None:
    ...
    ``encre/setup.py::_setup_mattermost`` function this migration
    ...
    print_info("   1. In Mattermost: Integrations 鈫?Bot Accounts 鈫?Add Bot Account")
    ...
    print_info("   To find your user ID: click your avatar 鈫?Profile")
    ...
    print_info("馃摤 Home Channel: where Encre delivers cron job results and notifications.")

    print_info("   To get a channel ID: click channel name 鈫?View Info 鈫?copy the ID")
    ...
    print_info("   Open config in your editor:  encre config edit")
...
# YAML 鈫?env config bridge (apply_yaml_config_fn, #25443)
...
def _apply_yaml_config(yaml_cfg: dict, mattermost_cfg: dict) -> dict | None:
    ...
    model and merely owns the YAML鈫抏nv translation here, next to the

    adapter that consumes it.



    Env vars take precedence over YAML 鈥?every assignment is guarded
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``encre.gateway.get_env_value`` at call time (not via
