...
Protocol hierarchy:
  WebSocket frame
    鈹斺攢鈹€ ConnMsg (protobuf: trpc.yuanbao.conn_common.ConnMsg)
          鈹溾攢鈹€ head: Head  (cmd_type, cmd, seq_no, msg_id, module, ...)
          鈹斺攢鈹€ data: bytes  (business payload, standard protobuf)
                鈹斺攢鈹€ InboundMessagePush / SendC2CMessageReq / SendGroupMessageReq / ...
                      (trpc.yuanbao.yuanbao_conn.yuanbao_openclaw_proxy.*)

Note: the conn layer (ConnMsg) is standard protobuf, not a custom binary format.
      The custom format described in conn.proto comments (magic+head_len+body_len)
      is only used for quic/tcp; WebSocket passes ConnMsg protobuf bytes directly
      (no sticky-packet issue, each ws frame = one message).

Implementation: hand-written varint / protobuf wire-format codec, no third-party
...
# conn layer message type enum (ConnMsg.Head.cmd_type)
PB_MSG_TYPES = {
...
# cmd_type enum
CMD_TYPE = {
    "Request": 0,   # upstream request
    "Response": 1,  # response to upstream request
    "Push": 2,      # downstream push
    "PushAck": 3,   # response ACK to downstream push
}
...
_seq_counter = 0
_SEQ_MAX = 2 ** 32 - 1  # uint32 upper bound
...
# ============================================================
# Protobuf wire-format primitives (hand-written, no google.protobuf dependency)
# ============================================================
...
def _encode_varint(value: int) -> bytes:
    ...
    if value < 0:
        # Handle signed negatives (int32/int64 use two's complement, 64-bit)
...
def _encode_string(s: str) -> bytes:
    """Encode the value portion of a protobuf string field (length-prefixed UTF-8)."""
    encoded = s.encode("utf-8")
    return _encode_varint(len(encoded)) + encoded


def _encode_bytes(b: bytes) -> bytes:
    """Encode the value portion of a protobuf bytes field (length-prefixed)."""
...
def _fields_to_dict(fields: list) -> dict[int, list]:
    """Convert a fields list to a {field_number: [value, ...]} dict (repeated fields have multiple entries)."""
    d: dict[int, list] = {}
...
# ============================================================
# ConnMsg layer encoding/decoding
# ============================================================
#
...
def _decode_head(data: bytes) -> dict:
    """Decode a ConnMsg.Head, returning a dict."""
...
def encode_conn_msg(msg_type: int, seq_no: int, data: bytes) -> bytes:
    """
    Encode a ConnMsg (simplified interface matching the required signature).

    Args:
        msg_type: cmd_type (value from CMD_TYPE enum)
        seq_no:   sequence number
        data:     inner payload bytes (business protobuf)

    Returns:
        Encoded ConnMsg bytes
    """
...
def decode_conn_msg(data: bytes) -> dict:
    """
    Decode a ConnMsg, returning {msg_type, seq_no, data, head}.

    Returns:
        {
          "msg_type": int,      # cmd_type
          "seq_no":   int,
          "data":     bytes,    # inner payload
          "head":     dict,     # full head fields
        }
    """
...
def encode_conn_msg_full(
    ...
    """
    Encode a full ConnMsg (with cmd/msg_id/module and other head fields).
...
# ============================================================
# BizMsg layer encoding/decoding (biz payload is also protobuf)
# ============================================================
#
# The required encode_biz_msg / decode_biz_msg are an intermediate abstraction:
# wrap the business body into a ConnMsg where head.cmd = method and head.module = service.
# This mirrors buildBusinessConnMsg() in conn-codec.ts.
...
def encode_biz_msg(service: str, method: str, req_id: str, body: bytes) -> bytes:
    ...
    Args:
        service: module name (head.module), e.g. "yuanbao_openclaw_proxy"
        method:  command word (head.cmd), e.g. "send_c2c_message"
        req_id:  message ID (head.msg_id)
        body:    already-encoded business protobuf bytes
...
def decode_biz_msg(data: bytes) -> dict:
    """
    Decode ConnMsg bytes, returning business-layer info.
...
# ============================================================
# Business protobuf message encoding/decoding (biz payload)
# ============================================================

# ---------- MsgContent encoding/decoding ----------
#   field 1: text (string)
#   field 2: uuid (string)
...
#   field 999: ext_map (map<string, string>)  鈫?extension info for WeChat chat-history forwarding
...
#       value: base64(ForwardMsgData protobuf)  鈫?NOT JSON; it is base64-encoded
...
def _encode_msg_content(content: dict) -> bytes:
    ...
    # ext_map (map<string, string>, field 999) 鈥?repeated message entries
...
def _decode_msg_content(data: bytes) -> dict:
    ...
    # ext_map (field 999) 鈥?decode repeated map entries into a plain dict
...
# ---------- MsgBodyElement encoding/decoding ----------
...
def decode_inbound_push(data: bytes) -> Optional[dict]:
    """
    Parse the biz payload of an inbound message push (InboundMessagePush proto bytes).

    Args:
        data: bytes from ConnMsg.data (i.e. the biz payload)

    Returns:
        {
          "from_account":  str,
          "to_account":    str (optional),
          "group_code":    str (optional, group messages only),
          ...
          "trace_id":      str,
          "recall_msg_seq_list": [{"msg_seq": int, "msg_id": str}, ...] or None,
        }
    ...
    try:
        ...
        # Filter out empty values (keep the API clean)
...
def encode_auth_bind(
    ...
    """
    Construct an auth-bind request ConnMsg.
    ...
    dev_buf += _encode_field(10, WT_LEN, _encode_string(str(ENCRE_INSTANCE_ID)))
    if bot_version:
...
def encode_ping(msg_id: str) -> bytes:
    """Construct a ping request ConnMsg (PingReq is an empty message)."""
    return encode_conn_msg_full(
        cmd_type=CMD_TYPE["Request"],
        cmd=CMD["Ping"],
        seq_no=next_seq_no(),
...
def encode_send_private_heartbeat(
    ...
    """
    Encode SendPrivateHeartbeatReq, returning full ConnMsg bytes.
...
def encode_send_group_heartbeat(
    ...
    """
    Encode SendGroupHeartbeatReq, returning full ConnMsg bytes.

    SendGroupHeartbeatReq fields:
      1: from_account (string)
      2: to_account   (string)  鈥?leave empty for group scenario
      3: group_code   (string)
...
def encode_query_group_info(group_code: str) -> bytes:
    """
    Encode QueryGroupInfoReq, returning full ConnMsg bytes.
...
def decode_query_group_info_rsp(data: bytes) -> Optional[dict]:
    """
    Decode QueryGroupInfoRsp biz payload.

    Proto structure (aligned with TS biz-codec / member.ts queryGroupInfo):

      message QueryGroupInfoRsp {
        int32  code       = 1;
        string message    = 2;
        GroupInfo group_info = 3;   // nested message
      }
    ...
    Returns:
        Decoded dict, or None (if parsing fails)
    """
    try:
        fdict = _fields_to_dict(_parse_fields(data))
...
def encode_get_group_member_list(
    ...
    """
    Encode GetGroupMemberListReq, returning full ConnMsg bytes.
...
def decode_get_group_member_list_rsp(data: bytes) -> Optional[dict]:
    """
    Decode GetGroupMemberListRsp biz payload.
    ...
    MemberInfo fields:
      1: user_id      (string)
      2: nickname     (string)
      3: role         (uint32)  鈥?0=member, 1=admin, 2=owner
      4: join_time    (uint32)
      5: name_card    (string)  鈥?group nickname
