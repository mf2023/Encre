...
``add_done_callback`` (never ``.result()`` 鈥?that would block the callback
...
# ``model_tools`` import time, so ``encre status``, ``encre chat``, etc.
...
# ``try`` block executes (which it does 鈥?``__init__`` calls it), the
...
def _load_google_modules() -> bool:
    ...
    googleapiclient, grpc, and friends 鈥?about 33MB RSS and 110ms wall
    on a fresh interpreter. Plugin discovery imports this module on
    every CLI invocation, even ones that never touch a encre.gateway.
    """
...
# the in-tree 鈫?plugin migration. ``__name__`` resolves to
# ``encre_plugins.platforms__google_chat.adapter`` once the plugin
# loader namespaces this module, which would silently break every
# downstream log-monitor that greps for ``encre.gateway.platforms.google_chat``.
logger = logging.getLogger("encre.gateway.platforms.google_chat")
...
# SA scopes 鈥?chat.bot is sufficient for the bot's own messaging operations
# (messages.create / patch / delete, spaces metadata, memberships,
# media.download for inbound user attachments). The bot CANNOT call
# media.upload 鈥?Google requires user OAuth for that endpoint, no scope
# adjustment changes it.
#
# Native attachment delivery (bot 鈫?user) is handled via a separate user-
...
# and 429 occasionally 鈥?without a retry wrapper, single hiccups drop
...
def _is_retryable_error(exc: BaseException) -> bool:
    ...
    and well-formed non-retryable failures are NOT retried 鈥?those
...
#   * ``send_typing`` checks for any value before posting 鈥?sentinel keeps
#     ``_keep_typing`` (running on the base-class timer) from creating a
#     fresh "Encre is thinking鈥? card during the small window between
#     ``send()`` finishing and the base-class cancelling its typing_task.
#   * ``stop_typing`` checks for the sentinel and skips the API delete 鈥?#     otherwise the safety-net cleanup at base.py:_process_message_background
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to an Encre MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "encre-card"), "card": card}
...
class _ThreadCountStore:
    ...
    - prev_count == 0 (first time we see this thread) 鈫?"main flow":
    ...
    - prev_count >= 1 (we've already seen this thread) 鈫?"side thread":
    ...
    (the bug Ram贸n reported across 4 iterations of the in-memory
    version).
    ...
    def load(self) -> None:
        ...
        Missing file 鈫?empty store. Corrupt JSON 鈫?empty store + warn.
        """
        self._loaded = True
        ...
        try:
            ...
            self._counts = {}
            return
        # Validate shape 鈥?anything off-schema gets dropped silently.
    ...
    def incr(self, chat_id: str, thread_name: str) -> int:
        ...
        PRE-increment value (the heuristic input 鈥?"have we seen this
    ...
    def _save(self) -> None:
        ...
        Failure is non-fatal 鈥?log warning and continue. The in-memory
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        # ``Platform("google_chat")`` resolves via ``_missing_()`` 鈫?pseudo-member
        # cached in ``_value2member_map_``.  We deliberately do NOT add an enum
        # attribute to ``encre.gateway.config.Platform`` 鈥?bundled platform plugins
        ...
        # ~110ms / ~33MB on every CLI invocation. Idempotent 鈥?pays the cost
        ...
        # but now hold the LEGACY single-user token (if any) 鈥?used as a
        ...
        # chat_id 鈫?most-recent inbound sender's email. Populated in
        ...
        #       stays stable across top-level messages 鈥?see
        ...
        # Persisted to ${data dir}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _data_dir = _Path.home() / "data dir"
        self._thread_count_store = _ThreadCountStore(
            _data_dir / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Encre is thinking鈥?.
    ...
    def _load_sa_credentials(self) -> Any:
        ...
        Priority:
          ...
          3. Application Default Credentials via ``google.auth.default()``
             鈥?works on Cloud Run / GCE / GKE with a workload identity
        ...
        # No explicit SA configured 鈥?try ADC. This is the Cloud Run / GCE
    ...
    async def _resolve_bot_user_id(self) -> Optional[str]:
        ...
        precise 鈥?own messages and other bots look alike).
        """
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        # First call into the heavy google-cloud stack 鈥?trigger the lazy
        # import. ``_load_google_modules()`` is idempotent and rebinds the
        # module globals (``pubsub_v1``, ``service_account``, ``HttpError``,
        # 鈥? used throughout this file. Anything that runs *before* this
        ...
        try:
            ...
            if user_creds is not None:
                ...
                logger.info(
                    "[GoogleChat] Legacy user OAuth loaded 鈥?fallback "
            ...
            if authorized:
                ...
                logger.info(
                    "[GoogleChat] No user OAuth tokens at setup 鈥?file "
    ...
    def _extract_message_payload(
        ...
        Format 1 鈥?Workspace Add-ons (canonical, ce-type-driven)::

            {"chat": {"messagePayload": {"message": {...}, "space": {...}}}}

        Format 2 鈥?Native Chat API Pub/Sub (alternative configuration where
        the Chat app publishes events directly without the Workspace
        Add-ons wrapper)::

            {"type": "MESSAGE", "message": {...}, "space": {...}}

        Format 3 鈥?Relay / flat (a custom Cloud Run relay that flattens the
        ...
        dict so downstream code (``_dispatch_message`` 鈫?        ``_build_message_event``) can consume it without branching.
        """
    ...
    def _on_pubsub_message(self, message: Any) -> None:
        """Pub/Sub callback 鈥?parse envelope and dispatch to asyncio loop.
        ...
        try:
            ...
            if "membership" in ce_type or "MEMBERSHIP" in ce_type:
                ...
                if "created" in ce_type:
                    # ADDED_TO_SPACE for this bot 鈥?resolve self user_id.
            ...
            # Dedup guard 鈥?Pub/Sub is at-least-once.
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        sees it 鈥?that's a bot-local OAuth setup flow, not a prompt.
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # key 鈥?the bot stores this user's token at
                # ${data dir}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _handle_setup_files_command(
        ...
        Subcommands:
          /setup-files                  鈫?show status + next step
          /setup-files start            鈫?print OAuth URL
          /setup-files revoke           鈫?revoke and delete stored token
          /setup-files <CODE_OR_URL>    鈫?exchange auth code for token
        ...
        if not arg:
            ...
            if creds is not None:
                ...
                await _reply(
                    "鉁?Native attachment delivery is **active** for "
                    f"`{who}`.\n"
                    f"Token: `{token_path}`\n"
            ...
            if not client_secret_present:
                await _reply(
                    "馃敡 Native attachment delivery is **not configured**.\n"
                    "**Step 1 (one-time, on the host):** create OAuth client "
                    "credentials at "
                    "https://console.cloud.google.com/apis/credentials 鈫?"
                    "*Create credentials* 鈫?*OAuth client ID* 鈫?*Desktop app*. "
                ...
                return True
            await _reply(
                "馃敡 Client credentials are stored but you haven't "
        ...
        if arg == "start":
            if not oauth_helper._client_secret_path().exists():
                await _reply(
                    "鈿狅笍 No client credentials stored for this profile. Send "
            ...
            try:
                ...
                await _reply(
                    "鉂?Couldn't generate the OAuth URL. Check the gateway "
            ...
            await _reply(
                ...
                "Tip: the URL contains your access grant 鈥?keep it private."
            )
            return True
        ...
        try:
            ...
            await _reply(
                "鉂?Token exchange failed. The code may have expired or "
        ...
        try:
            ...
            if new_creds is not None:
                ...
                await _reply(
                    "鉁?Authorized! Native attachment delivery is now "
        ...
        await _reply(
            "鈿狅笍 Token exchanged but the gateway couldn't load the new "
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into an Encre MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
        ...
        # - prev_count == 0  鈫?first message in this thread. Google Chat
        ...
        #   reply ALSO must NOT thread with the user message 鈥?if we
        ...
        # - prev_count >= 1  鈫?user explicitly engaged a thread that
        ...
        source = self.build_source(
            ...
            # resource name when sender has no email (rare 鈥?bot-to-bot
    ...
    async def _download_attachment(
        ...
        Priority for bot Service Accounts:
          ...
          1. ``attachmentDataRef.resourceName`` via ``chat.media.download`` 鈥?             the supported bot path. The Service Account bearer token has
          ...
          3. Direct HTTP fetch of ``downloadUri`` only as a last resort 鈥?             that URL is meant for user OAuth tokens (chat.google.com
        ...
        # can use 鈥?otherwise try the bot path first.
    ...
    async def send(
        ...
        ``messages.patch`` 鈥?NO delete+create. Google Chat shows a tombstone
        ...
        try:
            ...
            # and strip invisible Unicode that renders as tofu (鈻?. Runs
            ...
            # Treat any earlier sentinel as "no real card to patch" 鈥?defensive.
...
"action": "encre_clarify",
    ...
    async def edit_message(
        ...
        *,
        ...
        tool activity (no "馃攳 web_search鈥?, no progressive token edits).

        ``message_id`` is the Google Chat resource name
        ``spaces/X/messages/Y``. ``finalize`` is unused here 鈥?Google
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        """Delete a message 鈥?used sparingly (deletion creates a tombstone).
    ...
    # Outbound formatting
    # ------------------------------------------------------------------
    # Invisible Unicode codepoints that render as tofu (鈻? in Google
    ...
    def format_message(cls, content: str) -> str:
        ...
        # Headers (## Title) 鈫?*Title* (Chat has no header support).
        text = re.sub(
            r"^#{1,6}\s+(.+)$",
        ...
        # Bold: **text** 鈫?*text* (Chat uses single asterisks).
        text = re.sub(
            r"\*\*(.+?)\*\*",
        ...
        # Markdown links [text](url) 鈫?<url|text> (Slack-style angle-bracket).
        text = re.sub(
            r"\[([^\]]+)\]\(([^)]+)\)",
    ...
    def _resolve_thread_id(
        ...
        Priority:
          1. ``metadata['thread_id']`` 鈥?populated by the gateway's session
             plumbing from ``SessionSource.thread_id`` (the inbound
             ``thread.name``). Canonical path for groups.
          2. ``metadata['thread_name']`` / ``metadata['thread_ts']`` 鈥?Slack
          ...
          4. ``self._last_inbound_thread[chat_id]`` 鈥?Google Chat DMs spawn
    ...
    async def _call_with_retry(
        ...
        *,
        ...
        attempt 鈥?see :func:`_is_retryable_error`. Cancellation propagates
        ...
        # Defensive 鈥?the loop above always either returns or re-raises.
    ...
    async def _create_message(
        ...
        ``messageReplyOption=REPLY_MESSAGE_FALLBACK_TO_NEW_THREAD`` 鈥?        otherwise Google Chat silently ignores ``thread.name`` and
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Encre is thinking鈥? marker message.
        ...
        IMPORTANT 鈥?must place the typing card in the user's thread:
        ...
        top-level card in place 鈥?leaving the bot's whole response
        stranded outside the user's thread. We resolve the thread the
        same way send() does.

        IMPORTANT 鈥?cancellation safety:
        ...
        ``send_typing`` mid-flight 鈥?but the underlying ``asyncio.to_thread``
        ...
        "Encre is thinking鈥? stuck in chat forever, plus one card that
        ...
        Cancellation of THIS coroutine no longer cancels the create 鈥?        the task runs to completion and the msg_id lands in the slot
        regardless.
        """
        # Already have a card (real msg_id, sentinel, or in-flight) 鈥?bail.
        ...
        if chat_id in self._typing_card_inflight:
            ...
            # is up by the time we return". Bounded wait 鈥?if the
            ...
            try:
                ...
                if result.success and result.message_id:
                    ...
                    if chat_id not in self._typing_messages:
                        ...
                        # Slot already populated 鈥?likely send() patched
        ...
        # but the task continues in the background 鈥?so the msg_id
    ...
    async def stop_typing(self, chat_id: str) -> None:
        """Stop the typing indicator 鈥?NO-OP when a live card is tracked.

        Google Chat has no separate typing API: the "Encre is thinking鈥?
        ...
        ``stop_typing`` at three moments per turn 鈥?typically BEFORE
        ...
        Three cases:
          * Slot empty 鈫?nothing to do.
          * Slot holds SENTINEL 鈫?``send()`` already patched the card;
            pop the sentinel so the next turn starts clean.
          * Slot holds a real ``message_name`` 鈫?leave it for ``send()``
            to consume. NO-OP.

        Stranded cards on error / cancellation paths (where ``send()``
        never runs) are reaped by ``on_processing_complete`` 鈥?see that
        ...
        if current == _TYPING_CONSUMED_SENTINEL:
            ...
            return
        # Real message_name 鈥?leave it for send() to patch. Deliberate no-op.
        return
    ...
    async def on_processing_complete(
        ...
        patched it), the slot holds the SENTINEL 鈥?pop and exit.
        ...
        see "Encre is thinking鈥? stuck forever.
        """
        ...
        try:
            ...
            if current and current != _TYPING_CONSUMED_SENTINEL:
                # Real message_name still in slot 鈥?send() never ran. Patch
    ...
    async def _consume_typing_card_with_text(
        ...
        try:
            ...
            status = getattr(getattr(exc, "resp", None), "status", None)
            if status == 404:
                # Card disappeared 鈥?caller should create a new message.
                return None
            raise
    ...
    async def send_image(
        ...
        the image (caption + URL) 鈥?same anti-tombstone pattern used by
    ...
    def _is_app_auth_attachment_error(exc: HttpError) -> bool:
        ...
        still see). Used to flag a misuse 鈥?calling ``media.upload``
    ...
    async def _acquire_user_chat_api(
        ...
        Lookup order:
          1. Per-user token for ``sender_email`` 鈥?the asker's identity.
          2. Legacy single-user fallback (``self._user_chat_api``) for
             pre-multi-user installs.
          3. None 鈥?caller posts the setup-instructions text notice.
        ...
        if self._user_chat_api is not None:
            ...
            if refreshed is None:
                logger.warning(
                    "[GoogleChat] legacy user-OAuth refresh returned None 鈥?"
    ...
    def _invalidate_user_creds(self, identity: Optional[str]) -> None:
        ...
        ``identity`` comes from ``_acquire_user_chat_api`` 鈥?either the
    ...
    async def _send_file(
        ...
        BOTH calls go through a user-authed Chat API client 鈥?the
        ...
        # No user OAuth 鈫?can't upload natively. Surface clear setup
        ...
        try:
            ...
            if status in {401, 403}:
                logger.warning(
                    ...
                    "(token revoked or scope missing) 鈥?falling back to "
        ...
        # thread.name in body to actually be honored 鈥?see
        ...
        try:
            ...
            # comment for why 鈥?same reasoning applies to the
    ...
    async def _post_attachment_fallback(
        ...
        lines.extend([
            f"鈿狅笍 No he podido adjuntar **{filename}**.",
            "Google Chat s贸lo permite adjuntar archivos cuando el bot tiene "
            "permiso expl铆cito tuyo (OAuth de usuario). Es un consentimiento "
            "煤nico que se hace desde este chat.",
            "**Para activarlo:** env铆a `/setup-files` y sigue las instrucciones.",
            f"Mientras tanto el archivo est谩 en el host: `{path}`",
        ])
        ...
        return SendResult(
            success=False,
            error="google_chat: native attachment requires user OAuth 鈥?"
...
def _check_for_registry() -> bool:
    """``check_fn`` for the platform registry pass 鈥?stricter than the
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    core hook 鈥?it becomes a proper ``HomeChannel`` dataclass on the
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``encre setup``.

    The setup wizard at ``encre/encre.gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``data dir/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. encre-chat-events) and a Pull subscription.")
    ...
    print_info("  7. Google Chat API console 鈫?Configuration: connection = Cloud Pub/Sub,")
    ...
    if not project:
        print_warning("Project ID is required 鈥?skipping Google Chat setup")
        return
    ...
    if not subscription:
        print_warning("Subscription is required 鈥?skipping Google Chat setup")
        return
    ...
    if prompt_yes_no("Restrict access to specific users? (recommended)", True):
        ...
        print_warning("鈿狅笍  Open access 鈥?anyone who can DM the bot can command it.")
    ...
    print()
    print_success("Google Chat configuration saved to data dir/.env")
    print_info("Restart the gateway: encre gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``encre cron`` running as a
    separate process from ``encre gateway``).  Without this hook,
