...
def _word_split(text: str, limit: int, page_offset: int) -> list[str]:
    """Split *text* on word boundaries so each piece 鈮?*limit*.
    ...
    *page_offset* is the zero-based number of complete chunks already emitted
    ...
    """
    # 鈹€鈹€ second resort: split on word boundaries 鈹€鈹€
    words = text.split()
    parts: list[str] = []
    buf = ""
    for w in words:
        ...
        if buf:
            ...
            # Single word exceeds limit 鈫?hard-chop
...
class BasePlatformAdapter(ABC):
    ...
    def truncate_message(content: str, max_length: int) -> list[str]:
        ...
        # 鈹€鈹€ first pass: split on sentence boundaries 鈹€鈹€
        ...
        for sentence in raw:
            ...
            if buf:
                ...
                # Single sentence exceeds max_length 鈫?word-split it inline
        ...
        # 鈹€鈹€ second pass: re-chunk any oversized remainder 鈹€鈹€
        final: list[str] = []
        page = 0
        for c in chunks:
            if len(c) <= max_length:
        ...
        # 鈹€鈹€ third pass: add page indicators 鈹€鈹€
        total = len(final)
        if total > 1:
            final = [
        ...
        if self._authz is not None and not self.authorization_is_upstream:
            ...
            if source and source.user_id:
                ...
                if text.startswith("/pair") and self._pairing is not None:
                    ...
                    if not code:
                        ...
                        if result.authorized:
                            ...
                            await self.send(chat_id, "鉀?Not authorized to create pairing codes.")
                        return
                    else:
                        # Redeem a code
                        ok = self._pairing.redeem(code, source.platform, source.user_id)
                        if ok:
                            await self.send(chat_id, "鉁?Paired successfully!")
                        else:
                            await self.send(chat_id, "鉂?Invalid or expired pairing code.")
                        return
                ...
                if not result.authorized:
                    ...
                    await self.send(chat_id, "鉀?Not authorized. Use /pair <code> to pair.")
                    return
            elif source:
            ...
            if event.source and event.source.chat_id:
                try:
                    await self.send(
                        ...
                        "鈿狅笍 Gateway error: Message handler not registered. Please contact administrator."
                    )
