...
mixin's methods are called (typically in ``__init__``):
    ...
    self.config        # encre.encre.gateway.config.PlatformConfig
...
class WhatsAppBehaviorMixin:
    ...
    def _matches_whatsapp_allowlist(candidate: str, allow_from) -> bool:
        ...
        ``encre.encre.gateway.whatsapp_identity`` helper that the gateway authz and
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    mirrors the bridge source to a writable location and returns that
