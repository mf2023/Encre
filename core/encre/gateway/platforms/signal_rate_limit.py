...
Producers 鈥?``SignalAdapter.send_multiple_images`` and the ``send_message``
tool's Signal path 鈥?call :meth:`SignalAttachmentScheduler.acquire` before an
...
class SignalRateLimitError(Exception):
    ...
    reports it (signal-cli 鈮?v0.14.3). ``retry_after`` is ``None`` when the
...
# ---------------------------------------------------------------------------
# Detection helpers 鈥?used to fish a 429 out of signal-cli's various error
...
def _extract_retry_after_seconds(err: Any) -> Optional[float]:
    ...
    Two sources are tried, in order:
        1. ``error.data.response.results[*].retryAfterSeconds`` 鈥?the structured
           field signal-cli 鈮?v0.14.3 surfaces for a plain RateLimitException.
        2. ``"Retry after N seconds"`` parsed out of the message 鈥?covers
...
def _is_signal_rate_limit_error(err: Any) -> bool:
    ...
    Three layers are matched:
        * typed ``RATELIMIT_ERROR`` code (signal-cli 鈮?v0.14.3, plain
          RateLimitException);
        ...
        * libsignal-net's ``RetryLaterException`` / ``Retry after N seconds``
          ...
          hit during attachment upload 鈥?signal-cli never re-tags these as
...
def _signal_send_timeout(num_attachments: int) -> float:
    ...
    truncates large attachment batches mid-upload 鈥?which would otherwise log a
...
class SignalAttachmentScheduler:
    ...
    async def acquire(self, n: int) -> float:
        ...
        This does **not** deduct tokens 鈥?the bucket is a read-only model of
        ...
        while True:
            ...
            if first_pass:
                logger.info(
                    ...
                    "(available=%.1f, deficit=%.1f, refill=%.4f/s 鈮?%.1fs/token)",
    ...
    async def report_rpc_duration(self, rpc_duration: float, n_attachments: int) -> None:
        ...
        not refill during request processing 鈥?refill resumes only after the
        ...
        logger.log(
            ...
            "Signal scheduler: RPC for %d att took %.1fs 鈥?"
            "tokens %.1f 鈫?%.1f (deducted=%d, no upload refill credited, refill=%.4fs鈦宦?",
...
def get_scheduler() -> SignalAttachmentScheduler:
    ...
    if _scheduler is None:
        ...
        logger.info(
            "Signal scheduler: created (capacity=%d tokens, refill=%.4f/s 鈮?%.1fs/token)",
...
def _reset_scheduler() -> None:
    ...
    Test-only helper 鈥?never call from production paths, as it discards any
