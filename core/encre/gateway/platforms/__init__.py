...
# Explicit registry mapping platform name -> implementing module for adapters
# that do NOT live under encre.gateway.platforms.  The OpenAI-compatible API
# server (platform name "api_server", see encre.gateway.config.Platform) is
# part of the consolidated transport layer, so its platform entry resolves to
# encre.transport.http.openai_api.
_PLATFORM_MODULE_OVERRIDES = {
    "api_server": "encre.transport.http.openai_api",
}

_discovered = False


def _platform_module(name: str) -> str:
    """Resolve the importable module path for a platform name."""
    return _PLATFORM_MODULE_OVERRIDES.get(name, f"encre.gateway.platforms.{name}")
...
def discover_platforms() -> List[str]:
    ...
    for name in _BUILTIN_PLATFORMS:
        try:
            importlib.import_module(_platform_module(name))
            loaded.append(name)
