...
Buttons bubble 鈥?the user taps it later to receive the cached answer via a
*fresh* reply token (also free). State machine: PENDING 鈫?READY 鈫?DELIVERED,
...
binary uploads 鈥?images, audio, and video must be reachable HTTPS URLs.
...
to Encre. It deliberately ports the *strongest* idea from each into
a single plugin-form module that requires zero core edits:

* PR #18153 (leepoweii)   鈥?Template Buttons postback cache state machine,
  Markdown URL preservation, system-message bypass.
* PR #8398  (yuga-hashimoto) 鈥?media URL serving with traversal guard,
  send_voice / send_video, ``LINE_PUBLIC_URL`` env, macOS ``/tmp`` root.
* PR #16832 (jethac)      鈥?config wiring style, voice/image tests.
* PR #21023 (perng)       鈥?plugin-form skeleton (the only one already
  modeled on ``ADDING_A_PLATFORM.md``), reply鈫抪ush fallback at 50s TTL,
  loading-animation indicator, source dispatcher.
* PR #14942 (soichiyo)    鈥?Cloudflare-tunnel operating model (docs only).
* PR #14988 (David-0x221Eight) 鈥?text-first scope discipline.
* PR #6676  (liyoungc)    鈥?Push-only mode (used as the ``threshold=0``
  fallback path here).
"""
...
# ---------------------------------------------------------------------------
# Lazy / function-level imports for gateway internals are NOT used here 鈥?
...
# Webhook hardening
WEBHOOK_BODY_MAX_BYTES = 1_048_576  # 1 MiB 鈥?webhooks are tiny JSON
...
# routes on. LINE has no separate "voice" type 鈥?audio messages are recorded
...
# A 1脳1 transparent PNG used as fallback video preview thumbnail when no
# explicit preview is supplied 鈥?LINE requires ``previewImageUrl`` for
...
def strip_markdown_preserving_urls(text: str) -> str:
    ...
    LINE's text bubble has zero Markdown support 鈥?bold, italics, code
    ...
    Source: PR #18153 (leepoweii) 鈥?adapted to keep code-block content
    ...
    # Code blocks first 鈥?keep the inner content, drop the fences.
    ...
    # Markdown links 鈫?"label (url)"
    text = _MD_LINK_RE.sub(lambda m: f"{m.group(1)} ({m.group(2)})", text)

    # Bold/italic markers 鈥?strip.
    text = _MD_BOLD_RE.sub(r"\1", text)
    text = _MD_ITAL_RE.sub(r"\1", text)

    # Headings (#, ##) and bullet markers 鈥?strip the prefix only.
    text = _MD_HEADING_RE.sub("", text)
    text = _MD_BULLET_RE.sub("鈥?", text)
...
def split_for_line(text: str, max_chars: int = LINE_SAFE_BUBBLE_CHARS) -> List[str]:
    ...
    if remaining:
        # Truncate gracefully 鈥?caller already burned its 5-bubble budget.
        if chunks:
            tail = chunks[-1]
...
# ---------------------------------------------------------------------------
# Cache state machine 鈥?slow-LLM postback flow
# ---------------------------------------------------------------------------
...
class RequestCache:
    ...
    PRs #18153 originally combined two TTLs 鈥?one for PENDING (24h) and
...
def _resolve_chat(source: Dict[str, Any]) -> Tuple[str, str]:
    ...
    LINE sources are one of:
      * ``{"type": "user",  "userId":  "U..."}``  鈫?1:1 DM
      * ``{"type": "group", "groupId": "C...", "userId": "U..."}``  鈫?group chat
      * ``{"type": "room",  "roomId":  "R...", "userId": "U..."}``  鈫?multi-user room
...
def _allowed_for_source(
    ...
    *,
    ...
    """Three-list gate 鈥?credit PR #18153."""
    if allow_all:
        return True
    src_type = (source or {}).get("type", "")
    if src_type == "user":
...
def build_postback_button_message(
    ...
    """Template Buttons message 鈥?the slow-LLM postback bubble.
    ...
    LINE limits: ``text`` 鈮?160 chars, ``altText`` 鈮?400 chars.
    """
...
class LineAdapter(BasePlatformAdapter):
    ...
    # LINE has its own message-edit story (none) 鈥?we always send fresh
    ...
    def __init__(self, config, **kwargs):
        ...
        # Public base URL 鈥?required for media sending when bind isn't
        # publicly reachable.
        ...
        self._reply_tokens: Dict[str, Tuple[str, float]] = {}  # chat_id 鈫?(token, expiry)
        ...
        # Media state
        self._media_tokens: Dict[str, Tuple[str, float]] = {}  # token 鈫?(path, expiry)
        self._media_temp_paths: Set[str] = set()
        self._media_ttl = MEDIA_TOKEN_TTL_SECONDS

        # Pending-button slot per chat 鈥?ensures one outstanding postback
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            from encre.gateway.status import acquire_scoped_lock
        ...
        try:
            ...
            self._set_fatal_error(
                "missing_dep",
                "aiohttp is required for the LINE adapter 鈥?install with `pip install aiohttp`",
                retryable=False,
            )
            return False
        ...
        # Public health probe 鈥?useful for tunnel/proxy verification.
    ...
    async def disconnect(self) -> None:
        ...
        if self._lock_key:
            try:
                from encre.gateway.status import release_scoped_lock
    ...
    async def _handle_message_event(self, event: Dict[str, Any]) -> None:
        ...
        # Handle media inbound 鈥?fetch the binary, cache it, and surface a
    ...
    async def _handle_postback_event(self, event: Dict[str, Any]) -> None:
        """User tapped the slow-LLM postback button 鈥?deliver cached payload."""
        ...
        if entry.state is State.READY:
            ...
            # Still working 鈥?re-issue the wait notice.
            try:
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        LINE's chat-info APIs are limited and per-source-type 鈥?instead of
    ...
    # ------------------------------------------------------------------
    # Slow-LLM postback button 鈥?driven by _keep_typing
    # ------------------------------------------------------------------
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``data dir``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            encre_home = Path(get_data_dir()).resolve()
        except Exception:
            encre_home = Path.home().joinpath("data dir").resolve()

        allowed_roots = {
            Path(tempfile.gettempdir()).resolve(),
            Path("/tmp").resolve(),  # 鈫?/private/tmp on macOS
            encre_home,
        }
    ...
    async def send_video(
        ...
        # write a stdlib 1脳1 PNG to /tmp and serve it. PR #8398.
...
def _is_relative_to(child: Path, parent: Path) -> bool:
    """Backport for Path.is_relative_to (Python 3.9+) 鈥?defensive against
...
def is_connected(config) -> bool:
    """Surface in ``encre status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``encre status`` reflect a LINE configuration that lives entirely
...
async def _standalone_send(
    ...
    *,
    ...
    """Out-of-process push delivery for cron jobs running detached from the encre.gateway.
    ...
    ``thread_id`` is accepted for signature parity but ignored 鈥?LINE has
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``encre setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``data dir`` via ``encre.config``.
    """
    print()
    ...
    try:
        from encre.config import get_env_var, set_env_var
    except ImportError:
        print("encre.config not available; set LINE_* vars manually in data dir")
        return
