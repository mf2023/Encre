...
_DOH_TIMEOUT = 4.0  # seconds 鈥?bounded so connect() isn't noticeably delayed
...
# endpoints in the 149.154.160.0/20 block (same seed used by prior implementations).
_SEED_FALLBACK_IPS: list[str] = ["149.154.166.110", "149.154.167.220"]


def _resolve_proxy_url(target_hosts=None) -> str | None:
    # Delegate to shared implementation (env vars + macOS system proxy detection)
    from encre.gateway.platforms.base import resolve_proxy_url
...
async def discover_fallback_ips() -> list[str]:
    ...
    # no timeout of its own 鈥?a wedged OS resolver (broken VPN/DNS) can sit for
