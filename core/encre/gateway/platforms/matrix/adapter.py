...
Environment variables:
    ...
    MATRIX_USER_ID              Full user ID (@bot:server) 鈥?required for password login
...
from encre.gateway.platforms.helpers import ThreadParticipationTracker
...
def _resolve_matrix_bang_command(name: str) -> str | None:
    ...
    """Resolve a ``!command`` token to a dispatchable Encre command token.



    Matrix clients often reserve leading ``/`` for local client commands.

    Encre accepts ``!command`` as a Matrix-friendly alias, but only for
    ...
    *name* only by underscore鈫抙yphen normalization, e.g. ``reload_skills`` 鈫?
    ...
    left as-is 鈥?the gateway dispatcher resolves them to their canonical name.
    ...
    # alias passthrough 鈥?the gateway dispatcher canonicalizes aliases itself.
...
def _normalize_matrix_bang_command(text: str) -> str:
    ...
    """Convert Matrix ``!command`` aliases to normal Encre ``/command`` text."""
...
# Uses get_data_dir() so each profile gets its own Matrix store.

from encre.config import get_data_dir

def _get_encre_dir(*paths):

    from pathlib import Path

    return Path(get_data_dir(), *paths)



_STORE_DIR = _get_encre_dir("platforms/matrix/store", "matrix/store")
    ...
    if proxy_url.split("://")[0].lower().startswith("socks"):
        ...
        try:
            ...
            logger.warning(
                ...
                "aiohttp_socks not installed 鈥?SOCKS proxy %s ignored. "
...
def _check_e2ee_deps() -> bool:
    ...
    store backend (mautrix.crypto.store.asyncpg.PgCryptoStore 鈥?yes, the
...
def check_matrix_requirements() -> bool:
    ...
    missing 鈥?not just mautrix itself.  Previously this short-circuited on
    ...
    except Exception as exc:  # pragma: no cover 鈥?defensive
...
class _CryptoStateStore:
    ...
    async def find_shared_rooms(self, user_id: str) -> list:
        ...
        # Return all joined rooms 鈥?simple but correct for a single-user bot.
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Encre
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # See #12614 鈥?the symptom is "bot joins rooms but never replies".
        ...
        # Mention/thread gating 鈥?parsed once from config.extra or env vars.
        ...
        # errors in some clients.  5s is empirically safe; not user-tunable 鈥?
        ...
        # Proxy support 鈥?resolve once at init, reuse for all HTTP traffic.

        self._proxy_url: str | None = resolve_proxy_url(platform_env_var="MATRIX_PROXY")

        if self._proxy_url:

            logger.info("Matrix: proxy configured 鈥?%s", self._proxy_url)
        ...
        self._text_batch_delay_seconds = float(
            ...
            os.getenv("ENCRE_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")

        )

        self._text_batch_split_delay_seconds = float(

            os.getenv("ENCRE_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
    ...
    def _parse_thread_require_mention(config) -> bool:
        ...
        if configured is not None:
            ...
            # int, float, etc. 鈥?truthiness fallback
    ...
    async def _reverify_keys_after_upload(
        ...
        if not client.device_id or self._device_id_unverified:
            ...
            logger.warning(
                ...
                "Matrix: skipping post-upload key verification 鈥?"
    ...
    async def _verify_device_keys_on_server(self, client: Any, olm: Any) -> bool:
        ...
        if not client.device_id or self._device_id_unverified:
            ...
            logger.warning(
                ...
                "Matrix: skipping device key verification 鈥?"
        ...
        try:
            ...
            logger.error(
                ...
                "Matrix: cannot verify device keys on server: %s 鈥?refusing E2EE",
        ...
        if not our_keys:
            ...
            logger.warning("Matrix: device keys missing from server 鈥?re-uploading")
        ...
        if server_ed25519 != local_ed25519:
            ...
            if olm.account.shared:
                ...
                logger.error(
                    ...
                    "Matrix: server has different identity keys for device %s 鈥?"
            ...
            logger.warning(
                ...
                "Matrix: server has stale keys for device %s 鈥?attempting re-upload",
        ...
        if self._access_token:
            ...
            try:
                    ...
                    try:
                        ...
                        if len(all_devices) == 1:
                            ...
                            logger.warning(
                                ...
                                "Matrix: no devices found for %s 鈥?"
                ...
                logger.error(
                    ...
                    "Matrix: whoami failed 鈥?check MATRIX_ACCESS_TOKEN and MATRIX_HOMESERVER: %s",
            ...
            try:
                ...
                logger.error("Matrix: login failed 鈥?%s", exc)
        ...
        if self._encryption:
            ...
            if not _check_e2ee_deps():
                ...
                if self._e2ee_mode == "optional":
                    ...
                    logger.error(
                        ...
                        "Refusing to connect 鈥?encrypted rooms would silently fail.",
            ...
            if self._encryption:
                ...
                try:
                    ...
                    _acct_id = self._user_id or "encre"
                            ...
                            if output_error == "not_configured":
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        ...
                                        "(non-fatal 鈥?Element will show 'not verified by its owner'): %s",
    ...
    async def send_exec_approval(
        ...
        text = (
            ...
            "鈿狅笍 **Dangerous command requires approval**\n"
    ...
    async def send_model_picker(
        ...
        try:
            ...
            from encre.providers import get_label
        ...
        for emoji, model_id, provider_slug, provider_name in flat_choices:
            ...
            lines.append(f"{emoji} `{model_id}` 鈥?{provider_name}")
    ...
    def format_message(self, content: str) -> str:
        ...
        """Pass-through 鈥?Matrix supports standard Markdown natively."""
    ...
    async def _send_local_file(
        ...
        if not p.exists():
            ...
            text = f"{caption}\n鈿狅笍 Couldn't deliver the attachment." if caption \
                else "鈿狅笍 Couldn't deliver the attachment."
            ...
            try:
                ...
                if _sync_msg and isinstance(_sync_msg, str):
                    ...
                    if "m_unknown_token" in _lower or "unknown_token" in _lower:
                        ...
                        logger.error(
                            ...
                            "Matrix: permanent auth error from sync: %s 鈥?stopping",
                ...
                if (
                    ...
                    logger.error(
                        ...
                        "Matrix: permanent auth error: %s 鈥?stopping sync", exc

                    )

                    return

                logger.warning("Matrix: sync error: %s 鈥?retrying in 5s", exc)
    ...
    def _is_self_sender(self, sender: str) -> bool:
        ...
        lowercasing 鈥?some homeservers normalize the localpart case
        ...
        True defensively 鈥?an unidentified bot dropping its own events
    ...
    def _is_system_or_bridge_sender(sender: str) -> bool:
        ...
        authorization 鈥?and every outbound message relayed by the bridge
        ...
        Matches:
            ...
            ``@_something:server``   鈥?appservice namespace convention

            ``@:server``             鈥?malformed / empty localpart

            ``:server``              鈥?malformed, no leading ``@``
    ...
    async def _on_room_message(self, event: Any) -> None:
        ...
        logger.debug(
            ...
            "Matrix: callback fired 鈥?event %s from %s in %s",
        ...
        # user_id hasn't been resolved yet 鈥?see _is_self_sender docstring
        ...
        if event_ts and event_ts < self._startup_ts - _STARTUP_GRACE_SECONDS:
            ...
            # real time 鈥?every live event then looks "older than startup".
            ...
            #    room can deliver events spanning hours/days 鈥?those skews
            ...
            if not self._clock_skew_warned and (
                ...
                if 5 < skew < 86400:
                    ...
                    if self._late_grace_drops == 0:
                        ...
                        # Consistent offset 鈫?likely real clock skew.

                        self._late_grace_drops += 1

                    else:

                        # Varied skew 鈫?likely backfill, restart sampling.
                    ...
                    if self._late_grace_drops >= 3:
                        ...
                        logger.warning(
                            ...
                            "鈮?%.0fs). The host system clock is likely set "
        ...
        # Get msgtype 鈥?either from content object or raw dict.
    ...
    async def _resolve_message_context(
        ...
        # m.mentions.user_ids (MSC3952 / Matrix v1.7) 鈥?authoritative mention signal.
        ...
        if not is_dm:
            ...
            # allowed_rooms check (whitelist 鈥?must pass before other gating).
            ...
            if self._allowed_rooms and room_id not in self._allowed_rooms:
                ...
                logger.debug(
                    ...
                    "Matrix: ignoring message %s in %s 鈥?room not in "
            ...
            if self._require_mention and not is_free_room and not in_bot_thread:
                ...
                if not is_mentioned and not is_command:
                    ...
                    logger.debug(
                        ...
                        "Matrix: ignoring message %s in %s 鈥?no @mention "
            ...
            # where multiple bots all participate in the same thread.
                ...
                if not is_mentioned:
                    ...
                    logger.debug(
                        ...
                        "Matrix: ignoring message %s in thread %s 鈥?"
    ...
    async def _on_invite(self, event: Any) -> None:
        ...
        logger.info(
            ...
            "Matrix: invited to %s 鈥?joining (is_direct=%s)",
    ...
    async def _record_dm_room(self, room_id: str, inviter: str) -> None:
        ...
        try:
            ...
            pass  # m.direct doesn't exist yet 鈥?start fresh
    ...
    def _is_bot_mentioned(
        ...
        field with the bot's user-id, we trust it 鈥?even when the visible
        ...
        # m.mentions.user_ids 鈥?authoritative per MSC3952 / Matrix v1.7.
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Encre Agent" become "Agent".
    ...
    def _markdown_to_html(self, text: str) -> str:
        ...
        links, blockquotes, lists, and horizontal rules 鈥?everything the
...
# the matrix_cfg YAML鈫抏nv block in gateway/config.py, the _setup_matrix wizard

# + _PLATFORMS["matrix"] static dict in encre/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    succeed when cron runs separately from the encre.gateway. Converts markdown to
    ...
    try:
        ...
        txn_id = f"encre_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    ...
    """Configure Matrix credentials. Replaces encre/setup.py::_setup_matrix
    ...
    if token:
        ...
        user_id = prompt("User ID (@bot:server 鈥?optional, will be auto-detected)")
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        try:
            ...
            if _missing_before:
                ...
                try:
                    ...
                    print_warning(
                        ...
                        "Install failed 鈥?run manually: pip install "
            ...
            try:
                ...
                from encre.tools_config import _pip_install
                ...
                if result.returncode == 0:
                    ...
                    print_warning(
                        ...
                        f"Install failed 鈥?run manually: uv pip install "
        ...
        if allowed_users:
            ...
            print_info("鈿狅笍  No allowlist set - anyone who can message the bot can use it!")



        print_info("馃摤 Home Room: where Encre delivers cron job results and notifications.")
...
def _apply_yaml_config(yaml_cfg: dict, matrix_cfg: dict) -> dict | None:
    ...
    take precedence over YAML. Returns None 鈥?everything flows through env.
...
def _is_connected(config) -> bool:
    ...
    configured. Read via encre.gateway.get_env_value so setup-status
...
def _check_requirements(): return MATRIX_AVAILABLE
