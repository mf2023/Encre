...
"""
ntfy platform adapter (Encre plugin).
...
replies via HTTP POST. No external SDK 鈥?only httpx, which is already
...
``plugins/platforms/ntfy/``. The Encre plugin loader scans the
...
the registry 鈥?no edits to core files required.
    ...
    platforms:
      ...
      ntfy:
        ...
        extra:
          ...
          topic: "encre-in"              # subscribe topic (incoming)

          publish_topic: "encre-out"     # optional 鈥?defaults to topic

          token: "..."                    # optional Bearer / Basic auth token

          markdown: true                  # optional 鈥?enable markdown (default: false)
...
config.yaml ``extra``):
    ...
    NTFY_ALLOW_ALL_USERS       Allow any topic 鈥?dev only
...
authorization. Each topic is treated as a single trusted channel 鈥?
...
_ECHO_TAG = "encre-agent"  # tag added to outgoing messages for echo-loop prevention
...
def _build_auth_header(token: str) -> Dict[str, str]:
    ...
    Tokens are stripped of surrounding whitespace 鈥?pasted tokens often
...
class NtfyAdapter(BasePlatformAdapter):
    ...
    publishes replies via HTTP POST. No external SDK 鈥?only httpx.
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            logger.info("[%s] Connected 鈥?subscribing to %s/%s", self.name, self._server, self._topic)
        ...
        async with self._http_client.stream(
            ...
            if response.status_code == 401:
                ...
                logger.error(
                    ...
                    "[%s] Authentication failed (401) 鈥?stopping reconnect loop. Check NTFY_TOKEN.",
            ...
            if response.status_code == 404:
                ...
                logger.error(
                    ...
                    "[%s] Topic not found (404): %s 鈥?stopping reconnect loop.",
    ...
    async def _on_message(self, event: Dict[str, Any]) -> None:
        ...
        # publisher-controlled and must NOT be used for authorization 鈥?any
...
async def _standalone_send(
    ...
    *,
    ...
    ``encre cron`` running standalone). Without this hook,
    ...
    only 鈥?ntfy has no thread or attachment primitive. Markdown is
