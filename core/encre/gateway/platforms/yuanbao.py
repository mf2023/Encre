...
from encre.gateway.platforms.yuanbao_media import (
...
from encre.gateway.platforms.yuanbao_proto import (
    CMD_TYPE,
    _fields_to_dict,
    _get_string,
    _get_varint,
...
_APP_VERSION = "0.0.0"
_BOT_VERSION = "0.0.0"
_ENCRE_INSTANCE_ID = str(ENCRE_INSTANCE_ID)  # single source: yuanbao_proto.ENCRE_INSTANCE_ID
...
# teardown fast 鈥?a responsive server completes the handshake in well under a
...
# Close codes that indicate permanent errors 鈥?do NOT reconnect.
NO_RECONNECT_CLOSE_CODES = {4012, 4013, 4014, 4018, 4019, 4021}

# Heartbeat timeout threshold 鈥?N consecutive missed pongs trigger reconnect.
...
SLOW_RESPONSE_MESSAGE = "This task is complex and is being processed. Please wait..."
...
#   [file: report.pdf 鈫?/opt/data/.../report.pdf]
...
class MarkdownProcessor:
    ...
    def split_at_paragraph_boundary(
        ...
        sentence_end_re = re.compile(r'[銆傦紒锛?!?]\n')
        best_pos = -1
    ...
    def chunk_markdown_text(
        ...
        # Phase 3: Post-processing 鈥?split still-oversized chunks at paragraph boundaries
        result: list[str] = []
    ...
    def infer_block_separator(cls, prev_chunk: str, next_chunk: str) -> str:
        ...
        - Previous chunk ends with code fence or next chunk starts with fence 鈫?single newline '\\n'
        - Previous chunk ends with table row and next chunk starts with table row 鈫?single newline '\\n' (continued table)
        - Otherwise 鈫?double newline '\\n\\n' (paragraph separator)
...
class SignManager:
    ...
    # key: app_key 鈫?{"token", "bot_id", "expire_ts", ...}
    _cache: dict[str, dict[str, Any]] = {}

    # Per-app_key refresh locks 鈥?prevents concurrent duplicate sign-token
        ...
        async with httpx.AsyncClient(timeout=cls.HTTP_TIMEOUT_S) as client:
            for attempt in range(cls.MAX_RETRIES + 1):
                ...
                headers = {
                    ...
                    "X-Instance-Id": _ENCRE_INSTANCE_ID,
...
class InboundPipeline:
    ...
    def use(self, name_or_mw, handler=None, when=None) -> "InboundPipeline":
        ...
        Accepts either:
          - ``pipeline.use(SomeMiddleware())``  鈥?OOP style
          - ``pipeline.use("name", some_fn)``   鈥?functional style
        """
        ...
        async def next_fn() -> None:
            ...
            while index < len(chain):
                ...
                try:
                    ...
                    raise
                return
            # End of chain 鈥?nothing more to do
...
class DecodeMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if not data_list:
            return  # Stop pipeline 鈥?nothing to decode
...
class RecallGuardMiddleware(InboundMiddleware):
    ...
    Branch A: message in transcript (observed, not yet consumed) 鈫?redact content
    Branch B: message not in transcript 鈫?append system note
    Branch C: message currently being processed 鈫?silent interrupt + delayed redact
    """
    ...
    def _interrupt_for_recall(cls, adapter, session_key: str, recalled_id: str,
        ...
        recall_text = (
            f"[CRITICAL 鈥?MESSAGE RECALLED] The user message that triggered "
            ...
            f"tool results 鈥?the original request is void. "
        ...
        # May overwrite a user message pending in the same ~200ms window 鈥?acceptable.
        ...
        # interrupt 鈥?schedule a delayed redaction to clean it up.
    ...
    def _schedule_content_redact(cls, adapter, session_key: str, recalled_text: str,
        ...
        async def _redact() -> None:
            ...
            try:
                ...
                return
            # Poll until the recalled content appears in transcript 鈥?the
    ...
    def _patch_transcript(cls, adapter, recalled_id: str, group_code: str,
        ...
        # for any message that was observed with one 鈥?Branch A1 (exact id
        ...
        # platform id on the row 鈥?e.g. agent-processed @bot messages
        ...
        # Branch B: not found in transcript 鈫?append system note
...
class AccessPolicy:
    ...
    def is_dm_allowed(self, sender_id: str) -> bool:
        """Strict DM authorization 鈥?pairing does not imply access."""
...
class AutoSetHomeMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if not adapter._auto_sethome_done and adapter._sender_may_designate_home(ctx):
            ...
            if ctx.chat_type == "dm":
                adapter._auto_sethome_done = True  # DM seen 鈥?no further upgrades needed
            if _should_set:
                try:
    ...
    def _extract_text(cls, msg_body: list) -> str:
        ...
        for elem in msg_body:
            ...
            if elem_type == "TIMTextElem":
                ...
                # Unknown element type 鈥?include type as placeholder
...
class OwnerCommandMiddleware(InboundMiddleware):
    ...
    def _detect_owner_command(
        ...
        *,
        ...
        # privileged 鈥?leaking them to non-owners lets any group member approve
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if matched_cmd and not is_owner:
            # Non-owner tried an owner-only command 鈥?reject and stop
            logger.info(
            ...
            adapter._track_task(asyncio.create_task(
                adapter.send(ctx.chat_id, f"鈿狅笍 {matched_cmd} is only available to the creator in private chat mode"),
...
class GroupAtGuardMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if ctx.chat_type == "group" and not ctx.owner_command and not self._is_at_bot(ctx.msg_body, adapter._bot_id):
            ...
            return  # Stop pipeline 鈥?message observed but not dispatched
        await next_fn()
...
class ForwardedRecordsParseMiddleware(InboundMiddleware):
    ...
    the transcript at observe time 鈥?there is no run-time summary fallback
    here.

    On any failure the middleware leaves ``ctx.raw_text`` untouched
    (graceful degradation, design 搂2.8).
    """
    ...
    def _media_marker(
        ...
        # media_id is directly usable as a ybres RID (design 搂2.10.9);
        ...
        if media_type == "url":
            # Link share (e.g. WeChat article) 鈥?keep URL for the agent.
    ...
    # Per-record combined-text cap; record count is NOT capped (design 搂2.10.3).
    ...
    def _walk_forward_msgs(
        ...
        entries in textual order 鈥?the order PatchAnchorsMiddleware relies on
        (design 搂2.10.6). Headers / footers are the caller's job.
        """
        for msg in (forward_data.get("msg") if isinstance(forward_data, dict) else None) or []:
                    ...
                    if mc_type == 1:  # TEXT
                        ...
                        for media in mc.get("multimedia", []) or []:
                            if isinstance(media, dict):
                                ...
                                if ref is not None:
                                    refs.append(ref)
                    elif mc_type == 3:  # nested FORWARD_MSG (design 搂2.10.10)
                        parts.append("[Nested chat history]")
                    else:
                        if plain_text:
            ...
            if len(rendered) > cls.FORWARD_MSG_TEXT_MAX_CHARS:
                rendered = rendered[: cls.FORWARD_MSG_TEXT_MAX_CHARS] + "鈥?truncated)"
    ...
    def build_forward_text(
        ...
        Body lines are ``sender:body`` with full ``[kind|ybres:RID]`` media
        markers preserved. When ``is_dispatch`` is true, refs are appended to
        ``ctx.media_refs`` for downstream resolution and a ``User's note:
        {ctx.raw_text}`` footer is added; observed callers skip both since
        no later middleware runs.
        """
        nickname = ctx.sender_nickname or "User"
        lines = [f"The current user's nickname is {nickname}", "Below is the user's chat history"]
        ...
        if is_dispatch and ctx.raw_text.strip():
            text += f"\n\nUser's note:{ctx.raw_text.strip()}"
        return text
...
class MediaResolveMiddleware(InboundMiddleware):
    ...
    async def _resolve_ybres_refs(
        ...
        *,
        ...
        so cold-start turns with many anchors don't pay ``N 脳 (RPC + download)``
    ...
    async def _collect_observed_media(
        ...
        # Walk the most recent LOOKBACK messages newest鈫抩ldest so that when we
        ...
        # Final ``order`` is reversed back to chronological (old鈫抧ew) before
        ...
        # Restore chronological order (oldest鈫抧ewest) for downstream resolution.
        order.reverse()
    ...
    def _collect_quote_local_media(ctx: InboundContext) -> Tuple[List[str], List[str]]:
        ...
        ``[image: /path]`` / ``[file: name 鈫?/path]``. Unresolved anchors are an
        original-turn resolution failure and belong to that turn's handling, not
        this quote fallback 鈥?so no re-download happens here.
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        # 2) Second source 鈥?quoted media takes priority; otherwise fall back
        ...
        if ctx.reply_to_message_id is not None:
            if ctx.quote_media_refs:
                ...
                # from the adapter msg cache. Patched on its original turn 鈥?no
...
class ConnectionManager:
    ...
    async def open(self) -> bool:
        """Open WebSocket connection: sign-token 鈫?WS connect 鈫?AUTH_BIND 鈫?start loops.
    ...
    async def _handle_frame(self, raw: bytes) -> None:
        ...
        # Fire-and-forget heartbeat ACKs 鈥?server always responds but callers don't
        ...
        if cmd_type == CMD_TYPE["Push"]:
            ...
            # Genuine inbound message 鈥?dispatch to AI
            if data:
                logger.info(
    ...
    def _flush_inbound_buffer(self, key: str) -> None:
        """Flush the debounce buffer for a given key 鈥?execute the pipeline."""
    ...
    async def _reconnect_with_backoff(self) -> bool:
        """Reconnect with exponential backoff (1s, 2s, 4s, 鈥?up to 60s)."""
    ...
    async def _cleanup_ws(self) -> None:
        ...
        if ws is not None:
            try:
                ...
                logger.debug(
                    "[%s] WS close handshake exceeded %.1fs 鈥?dropping connection",
...
class MediaSendHandler(ABC):
    ...
    The shared flow (check ws 鈫?cancel notifier 鈫?validate 鈫?COS upload
    鈫?lock 鈫?dispatch) is handled by the base handle() template method.
    """
...
class ImageUrlHandler(MediaSendHandler):
    """Strategy: send image from a URL (download 鈫?COS 鈫?TIMImageElem)."""
...
class ImageFileHandler(MediaSendHandler):
    """Strategy: send image from a local file path (read 鈫?COS 鈫?TIMImageElem)."""
...
class FileUrlHandler(MediaSendHandler):
    """Strategy: send file from a URL (download 鈫?COS 鈫?TIMFileElem)."""
...
class DocumentHandler(MediaSendHandler):
    """Strategy: send local file/document (read 鈫?COS 鈫?TIMFileElem)."""
...
class StickerHandler(MediaSendHandler):
    ...
    def build_msg_body(self, upload_result, **kwargs):
        from encre.gateway.platforms.yuanbao_sticker import (
            get_sticker_by_name,
            get_random_sticker,
            build_face_msg_body,
...
class GroupQueryService:
    ...
    async def query_group_info_raw(self, group_code: str) -> Optional[dict]:
        ...
        from encre.gateway.platforms.yuanbao_proto import decode_conn_msg as _decode
...
class OutboundManager:
    ...
    Composes:
      - MessageSender   鈥?core text/media sending
      - HeartbeatManager 鈥?reply heartbeat (RUNNING / FINISH) lifecycle
      - SlowResponseNotifier 鈥?delayed 'please wait' notifications
...
class YuanbaoAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs: Any) -> None:
        ...
        # Inbound dispatch tasks 鈥?tracked so disconnect() can cancel them
        self._inbound_tasks: set[asyncio.Task] = set()

        # Set of background tasks 鈥?prevent GC from collecting fire-and-forget tasks
        ...
        # Group chat sequential dispatch queue (session_key 鈫?asyncio.Queue).
        ...
        # Bounded cache of msg_id 鈫?attributed content for recent messages.
        ...
        # upgrade 鈥?the first DM will override it with direct:xxx.
        # ------------------------------------------------------------------
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        chat_id conventions:
          "group:<group_code>"  鈫?group chat
          "direct:<account>"   鈫?C2C / direct message (default)
...
# 鈹€鈹€ Platform registration 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
