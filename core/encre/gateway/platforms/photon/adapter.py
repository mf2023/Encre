...
(see ``sidecar/index.mjs``) that runs the ``spectrum-ts`` SDK 鈥?the SDK is
...
if TYPE_CHECKING:
    ...
    except ImportError:  # pragma: no cover - httpx is already an Encre dep
...
# Dedup parameters 鈥?the gRPC stream is at-least-once, and a sidecar
...
# iMessage is a personal channel 鈥?suppressing rapid repeats reduces
...
# messages are ignored unless they match one of these patterns 鈥?same
...
# Module-level helpers 鈥?also used by check_fn / standalone send
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        ...
        # spectrum-ts not installed yet 鈥?`encre photon setup` will

        # install it.  check_fn still returns False so the gateway

        # surfaces the missing-deps state in `encre setup` / status.
...
def _sidecar_deps_stale() -> bool:
    ...
    `encre update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``encre photon install-sidecar``: ``npm ci`` for an exact,
    ...
    Best-effort 鈥?a failure here just leaves the (stale) deps in place and the
    ...
    try:
        ...
        # photon connect forever 鈥?give up, leave the stale deps in place, and
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # ``project_id`` here is the project's spectrumProjectId 鈥?the value
        ...
        # these 鈥?a tapback on a human鈫攈uman message is not addressed to us.
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Encre defaults).
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._autostart_sidecar:
            ...
            logger.warning(
                ...
                "[photon] sidecar autostart disabled 鈥?inbound + outbound will fail"
        ...
        logger.info(
            ...
            "[photon] connected 鈥?sidecar on %s:%d, streaming inbound over gRPC",
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Encre-Sidecar-Token": self._sidecar_token}
    ...
    def _is_duplicate(self, msg_id: str) -> bool:
        ...
        # the dict without limit 鈥?not just the expired-only prune.
    ...
    async def _dispatch_inbound(self, event: Dict[str, Any]) -> None:
        ...
        """Normalize a sidecar inbound event and dispatch it to the encre.gateway.
        ...
        # iMessage spaces carry their type directly 鈥?no id string-sniffing.
        ...
        if ctype == "reaction":
            ...
            # Route only tapbacks on messages WE sent 鈥?those are implicitly

            # addressed to the bot (feishu precedent: synthetic text event).

            # Reactions on human鈫攈uman messages are not for us. Checked before
        ...
        # Anything past here is a real (reactable) message 鈥?remember it as
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        # Checkout-agnostic: any Encre checkout's sidecar entry point.
    ...
    def _pid_alive(pid: int) -> bool:
        ...
        try:
            ...
            os.kill(pid, 0)  # windows-footgun: ok 鈥?only called from _reap_stale_sidecar which win32-guards early
    ...
    async def _reap_stale_sidecar(self) -> None:
        ...
        know, so it can't be told to ``/shutdown`` 鈥?and every replacement
        ...
        try:
            ...
            return  # nothing listening 鈥?the normal case
        ...
        if not stale:
            ...
            raise RuntimeError(
                ...
                f"(pids: {foreign or 'unknown'}, not a Photon sidecar) 鈥?"
        ...
        for pid in stale:
            ...
            if self._pid_alive(pid):
                ...
                try:
                    ...
                    os.kill(pid, signal.SIGKILL)  # windows-footgun: ok 鈥?unreachable on win32 (early return above)
        ...
        if foreign:
            ...
            raise RuntimeError(
                ...
                f"processes (pids: {foreign}) 鈥?free it or set "
    ...
    async def _start_sidecar(self) -> None:
        ...
        if not (_SIDECAR_DIR / "node_modules").exists():
            ...
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `encre photon setup`)"

            )

        # A `encre update` that bumps the spectrum-ts pin rewrites
        ...
        # gateway death of ANY kind 鈥?including SIGKILL, where disconnect()

        # never runs 鈥?can't leave it orphaned on the port.
        ...
        # Wait for /healthz to come up 鈥?give it up to 15s on cold start.
    ...
    async def _supervise_sidecar(self, proc: subprocess.Popen) -> None:
        ...
        if self._inbound_running:
            ...
            logger.error(
                ...
                "[photon] sidecar exited unexpectedly (code %s) 鈥?triggering reconnect",
    ...
    async def send_image(
        ...
        try:
            ...
            # Couldn't fetch the URL 鈥?fall back to sending it as text.
    ...
    # Same lifecycle-hook pattern as Telegram/Discord: 馃憖 while processing,

    # swapped for 馃憤/馃憥 on completion. Opt-in via PHOTON_REACTIONS 鈥?iMessage
    ...
    # A DM space is addressable two ways 鈥?the chat GUID (`any;-;+1555...`)
    ...
    async def add_reaction(
        ...
        maps 鉂わ笍馃憤馃憥馃槀鈥硷笍鉂?to native tapbacks; anything else uses Apple's
        ...
        if not target:
            ...
            return {
                ...
                "error": "no message to react to 鈥?pass message_id (no "
    ...
    async def remove_reaction(
        ...
        if not target:
            ...
            return {
                ...
                "error": "no message to unreact 鈥?pass message_id",
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        ...
        """Tapback 馃憖 on the triggering message while the agent works."""
    ...
    async def on_processing_complete(
        ...
        """Swap the 馃憖 progress tapback for a 馃憤/馃憥 result.
    ...
    def format_message(self, content: str) -> str:
        ...
        # Markdown is passed through verbatim 鈥?the sidecar sends it with the
    ...
    async def _send_with_retry(
        ...
        text under ``PHOTON_MARKDOWN=false`` 鈥?either way the gateway's
    ...
    async def _sidecar_call(self, path: str, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # _http_client directly 鈥?it always runs on the gateway's loop.
...
# MIME 鈫?file-extension maps for caching inbound attachment bytes. These mirror
    ...
    try:
        ...
        if mime.startswith("image/"):
            ...
            try:
                ...
                # Bytes don't look like a supported image (e.g. HEIC magic) 鈥?
        ...
        # Video, application/*, and everything else 鈫?document cache.
...
async def _standalone_send(
    ...
    *,
    ...
    thread_id: Optional[str] = None,  # noqa: ARG001 鈥?Spectrum has no threads yet

    media_files: Optional[list] = None,

    force_document: bool = False,  # noqa: ARG001 鈥?iMessage auto-detects file kind
    ...
    headers = {"X-Encre-Sidecar-Token": token}
...
# Plugin entry point    # Register CLI subcommands 鈥?`encre photon ...`
