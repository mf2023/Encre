...
This module is pure Python 鈥?it intentionally does not depend on
...
``spectrumProjectId`` field), but the dashboard unified them 鈥?every
...
Cloud with ``(id, projectSecret)`` 鈥?the same ``id`` used in Dashboard API

paths 鈥?which we persist as ``PHOTON_PROJECT_ID`` for the runtime.



Credential storage mirrors every other Encre channel:



    * runtime SDK creds  -> ``data dir/.env``  (``PHOTON_PROJECT_ID`` =

      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``

    * management metadata -> ``data dir/auth.json`` under
...
# endpoint 鈥?an unregistered client_id is rejected with
...
# registers Encre as its own client_id.
...
# Default name of the project Encre provisions for the operator.

DEFAULT_PROJECT_NAME = "Encre Agent"



# Polling defaults per RFC 8628.  Photon overrides via `interval` /

# `expires_in` in the device-code response 鈥?those win.
...
# auth.json helpers 鈥?share the file with the rest of encre.



def _auth_json_path() -> Path:

    """Resolve ``data dir/auth.json`` honouring the active Encre profile."""

    try:

        from encre.config import get_data_dir

        return Path(get_data_dir()) / "auth.json"

    except Exception:

        return Path(os.path.expanduser("data dir")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``data dir/.env`` is loaded into the gateway's
...
def store_project_credentials(
    ...
    *,
    ...
    The runtime SDK creds land in ``data dir/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    ...
    """Write the SDK creds to ``data dir/.env`` (canonical runtime store).
    ...
    caller 鈥?same CodeQL-clean-flow rationale as the rest of this module.

    """

    try:

        from encre.config import save_env_value

    except ImportError:

        logger.warning("photon: encre.config unavailable 鈥?skipping .env write")
    ...
    *,
    ...
    while time.time() < deadline:
        ...
        if resp.status_code == 429:
            ...
            # RFC 8628 搂3.5 鈥?treat 429 as slow_down.
...
def get_session(token: str) -> Dict[str, Any]:
    ...
    """GET ``/api/auth/get-session`` 鈥?confirm the token + fetch the user."""
...
def list_projects(token: str) -> List[Dict[str, Any]]:
    ...
    """GET ``/api/projects`` 鈥?return the caller's projects."""
...
def regenerate_project_secret(token: str, project_id: str) -> str:
    ...
    """POST ``/api/projects/{id}/regenerate-secret`` 鈫?the new project secret.
...
def list_users(project_id: str, project_secret: str) -> List[Dict[str, Any]]:
    ...
    """GET Spectrum Cloud ``/projects/{id}/users/`` 鈫?``SpectrumUser[]``."""
...
def register_user_if_absent(
    ...
    *,
    ...
    Returns ``(user, created)`` 鈥?``created`` is False when a user with the
...
def user_assigned_line(user: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    column) 鈥?i.e. the number to text to reach the agent, as opposed to the
...
def list_lines(token: str, project_id: str) -> List[Dict[str, Any]]:
    ...
    """GET ``/api/projects/{id}/lines`` 鈫?``[{id, platform, phoneNumber, status}]``."""
...
# Credential status (display-only 鈥?never emits raw secret material)
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    function (``"鉁?stored"`` / ``"鉁?missing"`` / a non-secret id); the
    ...
    labels["device_token"] = (
        ...
        "鉁?stored" if load_photon_token()

        else "鉁?missing (run `encre photon setup`)"
    ...
    labels["project_id"] = sid if sid else "鉁?missing"

    labels["project_key"] = "鉁?stored" if sec else "鉁?missing"

    phone, assigned = load_user_numbers()

    labels["phone_number"] = (

        phone if phone else "鉁?missing (run `encre photon setup --phone ...`)"

    )

    labels["assigned_phone_number"] = (

        assigned if assigned else "鉁?missing (run `encre photon setup`)"
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_secret() -> str:
        ...
        return "鉁?stored" if sec else "鉁?missing"
    ...
    def _present_phone() -> str:
        ...
        return phone or "鉁?missing (run `encre photon setup --phone ...`)"
    ...
    def _present_assigned_phone() -> str:
        ...
        return assigned or "鉁?missing (run `encre photon setup`)"
