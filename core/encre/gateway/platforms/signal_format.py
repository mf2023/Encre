...
message 鈥?whether sent through the live Signal adapter or through a standalone
send path 鈥?emits identical ``bodyRanges`` output and therefore looks the same
...
def markdown_to_signal(text: str) -> tuple[str, list[str]]:
    ...
    code units 鈥?that is the unit the Signal wire protocol uses for string
    ...
    def _normalize_bullet_markers(source: str) -> str:
        ...
        substitute a real ``鈥` bullet so the list reads naturally. Fenced code
        ...
        for idx, part in enumerate(parts):
            ...
            parts[idx] = re.sub(r"(?m)^([ \t]{0,3})[-*+]\s+", r"\1鈥?", part)
        return "".join(parts)
