...
"""
SimpleX Chat platform adapter (Encre plugin).

Connects to a ``simplex-chat`` daemon running in WebSocket mode. Inbound
messages arrive over a persistent WebSocket connection; outbound messages use
the same WebSocket with JSON text commands. The daemon models each conversation
as ephemeral, pairwise message queues identified by numeric contact/group IDs
rather than long-lived server accounts.

This adapter ships as an Encre platform plugin under
``plugins/platforms/simplex/``. The Encre plugin loader scans the directory at
startup, calls ``register(ctx)``, and the platform becomes available to
``gateway/run.py`` and ``tools/send_message_tool`` through the registry 鈥?no
edits to core files are required.
...
ENCRE_SIMPLEX_TEXT_BATCH_DELAY
                               Quiet-period seconds (default: 0.8) used to
                               concatenate rapid-fire inbound text messages
                               into a single MessageEvent 鈥?same pattern as
                               Telegram's text batching.

The ``websockets`` Python package is imported lazily 鈥?the plugin is
discoverable and ``encre setup`` can describe it even when websockets is not
installed. ``check_requirements()`` returns False until the package is present,
...
# Imported at module top because they're stdlib-only inside Encre 鈥?no
...
def _is_image_ext(ext: str) -> bool:
    """Return True if *ext* is an image format SimpleX can render inline.

    Args:
        ext: A file extension, with or without the leading dot, in any case.

    Returns:
        True when *ext* is one of the inline-renderable image formats.
    """
    return ext.lower() in {".jpg", ".jpeg", ".png", ".gif", ".webp"}


def _is_audio_ext(ext: str) -> bool:
    """Return True if *ext* is an audio format treated as a voice/file note.

    Args:
        ext: A file extension, with or without the leading dot, in any case.

    Returns:
        True when *ext* is one of the recognised audio formats.
    """
...
class SimplexAdapter(BasePlatformAdapter):
    """SimpleX Chat adapter backed by the simplex-chat daemon WebSocket API.

    Responsibilities:
        * Maintain a persistent WebSocket connection to the daemon, including
          automatic reconnect with exponential backoff plus jitter.
        * Translate inbound daemon events (``newChatItems``, ``rcvFile*``,
          ``contactRequest`` 鈥? into Encre ``MessageEvent`` objects and feed
          them to :meth:`BasePlatformAdapter.handle_message`.
        * Send text, images, documents, video and voice notes using the
          daemon's structured ``/_send`` JSON command form.
        * Batch rapid-fire inbound text into a single event (mirroring
          Telegram) so quick multi-line pastes read as one message.
        * Auto-accept contact requests and gate group traffic against an
          allowlist.

    Key instance attributes:
        ws_url: The daemon WebSocket URL.
        auto_accept: Whether incoming contact requests are auto-accepted.
        group_allow_from: Set of allowed group IDs (``"*"`` means any group).
        _ws / _ws_task / _health_task: Live connection and background tasks.
        _pending_corr_ids / _pending_responses: Echo filtering and command
            correlation bookkeeping.
        _pending_text_batches / _pending_text_batch_tasks: Text batching state.

    Instantiated by the ``adapter_factory`` passed to ``ctx.register_platform()``
    in :func:`register`.
    """
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        """Construct the adapter and read its configuration.

        Configuration is taken from ``config.extra`` first and then from the
        environment, so env vars override seeded config. Connection details,
        auto-accept behaviour, the group allowlist and text-batching delay are
        resolved here; background tasks are started later in :meth:`connect`.

        Args:
            config: The platform configuration object supplied by the registry.
            kwargs: Extra keyword arguments forwarded by the registry factory.
        """
        ...
        # Contact-request auto-accept (on by default 鈥?matches the way most
        ...
        # are ignored entirely (safer default 鈥?a bot in a group otherwise
        ...
        # Text message batching 鈥?concatenate rapid-fire messages into one
        # event before dispatching, mirroring Telegram's batching.
        self._text_batch_delay = float(
            os.getenv("ENCRE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        Performs a lazy import of ``websockets``, verifies the daemon URL is
        configured, does a quick open/close connectivity probe, then launches
        the listener and health-monitor background tasks.

        Args:
            is_reconnect: Hint that this is a reconnection attempt; currently
                used only for logging/semantics, the connect path is identical.

        Returns:
            True when the listener and health tasks were started; False when
            ``websockets`` is missing or the daemon is unreachable.
        """
        try:
        ...
        # Quick connectivity check 鈥?try to open and immediately close
        try:
    ...
    async def disconnect(self) -> None:
        """Stop the WebSocket listener and clean up all background state.

        Cancels the listener and health tasks, closes the live socket, and
        cancels any pending text-batch flush timers and outstanding command
        futures so the event loop can shut down without dangling work.
        """
        self._running = False
    ...
    async def _handle_event(self, event: dict) -> None:
        ...
        if resp_type == "rcvFileDescrReady":
            ...
            if file_id is not None:
                logger.debug(
                    "SimpleX: rcvFileDescrReady for fileId=%s 鈥?sending /freceive",
                    file_id,
                )
                await self._send_fire_and_forget(f"/freceive {file_id}")
            return

        # New messages 鈥?simplex-chat sends "newChatItems" with an array
        ...
        # Singular variant 鈥?some daemon versions emit this
        ...
        # File transfer completion 鈥?deliver any deferred chat item
    ...
    def _make_corr_id(self) -> str:
        ...
        ever invoking ``_handle_chat_item``. The set is bounded 鈥?when
    ...
    async def _send_ws(self, payload: dict) -> None:
        ...
        caller never has to handle reconnection 鈥?the ``_ws_listener``
    ...
    async def _send_command(
        ...
        """Send a command and await the correlated daemon response.

        A correlation ID is minted and registered in ``_pending_responses`` so
        the listener can resolve the matching future when the daemon replies.

        Args:
            command: The simplex-chat command string to send.
            timeout: Maximum seconds to wait for the correlated response.

        Returns:
            The daemon's response dict, or ``None`` on timeout / not-connected /
            send failure (a warning is logged in those cases).
        """
        ws = self._ws
        if not ws:
        ...
        try:
            ...
            logger.warning("SimpleX: command failed: %s 鈥?%s", command[:50], e)
            self._pending_responses.pop(corr_id, None)
            return None
    ...
    async def send(
        ...
        Args:
            chat_id: Target conversation ID. Group IDs are prefixed ``group:``;
                everything else is treated as a direct contact ID.
            content: The text body. ``MEDIA:<path>`` tags are expanded into
                native voice/document attachments and then stripped from text.
            reply_to: Reserved for signature parity with other adapters; SimpleX
                has no first-class reply API, so it is ignored.
            metadata: Optional arbitrary metadata map (currently unused).

        Returns:
            A :class:`SendResult` describing success or the first attachment
            failure encountered.
        """
    ...
    def _prepare_image(file_path: str) -> tuple[str, str]:
        ...
        Args:
            file_path: Path to the source image on the local filesystem.

        Returns:
            A ``(png_path, thumb_data_uri)`` tuple where ``png_path`` is the
            (possibly converted) PNG file to upload and ``thumb_data_uri`` is a
            ``data:`` URI of the inline preview, or ``""`` when conversion is
            unavailable.
        """
        import subprocess
        import tempfile
    ...
    async def send_image(
        ...
        **kwargs,
        ...
        Args:
            chat_id: Target conversation ID (``group:`` prefix for groups).
            image_url: Source image, either ``file://<path>`` or an
                ``http(s)://`` URL that will be downloaded and cached.
            caption: Optional text caption shown beneath the image.
            kwargs: Forwarded for signature parity with the base adapter.

        Returns:
            A :class:`SendResult` reporting success or the failure reason.
        """
        ...
        if image_url.startswith("file://"):
            ...
            try:
                from encre.gateway.platforms.base import cache_image_from_url
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        Args:
            chat_id: Target conversation ID (``group:`` prefix for groups).
            image_path: Filesystem path of the image to send.
            caption: Optional text caption.
            reply_to: Reserved for parity with other adapters; ignored.
            kwargs: Forwarded to :meth:`send_image`.
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        SimpleX has no dedicated video message type, so the video is delivered
        as a generic file attachment.

        Args:
            chat_id: Target conversation ID (``group:`` prefix for groups).
            video_path: Filesystem path of the video to send.
            caption: Optional text caption.
            reply_to: Reserved for parity with other adapters; ignored.
            kwargs: Forwarded to :meth:`send_document`.
    ...
    async def send_document(
        ...
        **kwargs,
        ...
        Args:
            chat_id: Target conversation ID (``group:`` prefix for groups).
            file_path: Filesystem path of the file to send.
            caption: Optional text caption shown with the file.
            filename: Optional explicit filename; defaults to the path's name.
            kwargs: Forwarded for signature parity with the base adapter.
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        Args:
            chat_id: Target conversation ID (``group:`` prefix for groups).
            audio_path: Filesystem path of the audio file to send.
            caption: Optional text caption shown with the voice note.
            reply_to: Reserved for parity with other adapters; ignored.
            duration: Duration of the clip in seconds, for the player UI.
            kwargs: Forwarded for signature parity with the base adapter.
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        SimpleX's protocol exposes no typing-indicator API, so this method
        exists only to satisfy the base adapter's interface.

        Args:
            chat_id: Target conversation ID (ignored).
            metadata: Optional metadata map (ignored).
        """

    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        """Return basic chat info for a conversation.

        Args:
            chat_id: Target conversation ID (``group:`` prefix for groups).

        Returns:
            A dict with ``chat_id``, ``type`` (``"group"`` or ``"dm"``) and a
            best-effort ``name``.
        """
        if chat_id.startswith("group:"):
...
def _env_enablement() -> Optional[dict]:
    ...
    The special ``home_channel`` key is handled by the core hook 鈥?it
...
async def _standalone_send(
    ...
    *,
    ...
    Args:
        pconfig: The platform configuration carrying the daemon URL.
        chat_id: Target conversation ID (``group:`` prefix for groups).
        message: The text body to deliver.
        thread_id: Accepted for signature parity; not used by SimpleX.
        media_files: Accepted for parity; only the text body is delivered,
            because file transfers need the daemon's filesystem-backed flow.
        force_document: Accepted for parity; not used here.

    Returns:
        A dict with ``{"success": True, "platform": "simplex", "chat_id": ...}``
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``encre setup gateway`` 鈫?SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``data dir/.env`` via
    ``encre.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "encre.config not available; set SIMPLEX_* vars manually in "
            "data dir/.env"
        )
        return
    ...
    def _prompt(var: str, prompt: str, *, secret: bool = False) -> None:
        ...
        try:
            if secret:
                from encre.secret_prompt import masked_secret_prompt
...
def _build_adapter(config):
    """Registry factory that constructs a :class:`SimplexAdapter`.

    Args:
        config: The platform configuration object supplied by the registry.

    Returns:
        A fully constructed :class:`SimplexAdapter` instance.
    """
