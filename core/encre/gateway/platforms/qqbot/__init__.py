...
Package structure:
    - ``adapter`` 鈥?:class:`QQAdapter` implements the full message lifecycle
      (WebSocket connect, event dispatch, REST send, media upload, STT).
    - ``constants`` 鈥?API URLs, timeouts, message/media type codes, limits.
    - ``utils`` 鈥?User-Agent builder, HTTP header factory, config coercion.
    - ``crypto`` 鈥?AES-256-GCM key generation and credential decryption
      for the QR-code scan-to-configure flow.
    - ``onboard`` 鈥?Synchronous QR-code registration: create task 鈫?display QR
      鈫?poll for scan 鈫?decrypt bot credentials.
    - ``chunked_upload`` 鈥?Three-step chunked upload (prepare 鈫?PUT parts 鈫?      complete) for files between 10 MB and ~100 MB.
    - ``keyboards`` 鈥?Inline keyboard dataclasses, button builders, approval
...
# config coercion helper (accepts strings/lists/tuples/sets 鈫?list[str]).
