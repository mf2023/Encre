...
QQBot shared utilities 鈥?User-Agent builder, HTTP helpers, config coercion.

This module provides three categories of helpers:

1. :func:`build_user_agent` 鈥?constructs a descriptive User-Agent string
   for QQ Bot API requests. Format: ``QQBotAdapter/<ver> (Python/<ver>; <os>; Encre/<ver>)``
2. :func:`get_api_headers` 鈥?returns standard HTTP headers dict including
   Content-Type, Accept, and dynamic User-Agent.
3. :func:`coerce_list` 鈥?normalizes config values into trimmed string lists.
   Accepts comma-separated strings, lists, tuples, sets, or single values.

Dependencies:
    - :const:`QQBOT_VERSION` from ``constants`` 鈥?adapter version string.
...
def get_api_headers() -> Dict[str, str]:
    ...
    The ``Accept: application/json`` header is required by q.qq.com 鈥?    without it, the server returns a JavaScript anti-bot challenge page
...
def coerce_list(value: Any) -> List[str]:
    ...
    Formats supported:
        - None 鈫?empty list []
        - Comma-separated string "a, b, c" 鈫?["a", "b", "c"]
        - List ["a", "b"] 鈫?["a", "b"]
        - Tuple ("a", "b") 鈫?["a", "b"]
        - Set {"a"} 鈫?["a"]
        - Single value 42 鈫?["42"]
