...
entry-point qr_register() that handles the full flow (create task 鈫?display QR code 鈫?poll 鈫?decrypt credentials).

Flow overview::

    1. Generate random AES-256 key (generate_bind_key)
    2. POST /lite/create_bind_task with key 鈫?get task_id
    3. Display QR code URL to user
    4. User scans QR with QQ app 鈫?server encrypts client_secret with key
    5. Poll /lite/poll_bind_result until status=COMPLETED
    6. Decrypt bot_encrypt_secret with AES-256-GCM 鈫?get client_secret
    7. Return {app_id, client_secret, user_openid}

APIs called:
    - ``create_bind_task`` 鈥?creates a bind task and returns task_id + encrypted response.
    - ``poll_bind_result`` 鈥?polls for scan completion; returns decrypted credentials.
...
# ---------------------------------------------------------------------------
# BindStatus 鈥?result codes from poll_bind_result
# ---------------------------------------------------------------------------
...
class BindStatus(IntEnum):
    ...
    Attributes:
        ...
        COMPLETED: User has confirmed 鈥?credentials are ready.
        EXPIRED: QR code has expired 鈥?need to create a new task.
    """
...
# ---------------------------------------------------------------------------
# QR rendering 鈥?terminal display helper
# ---------------------------------------------------------------------------

import qrcode as _qrcode_mod  # optional 鈥?QR rendering falls back to URL if missing
...
def qr_register(timeout_seconds: int = 600) -> Optional[dict]:
    ...
    the complete lifecycle: create bind task 鈫?display QR code 鈫?poll for
    scan completion 鈫?decrypt credentials.

    Flow::

        for attempt in range(_MAX_REFRESHES + 1):
            1. Create bind task 鈫?(task_id, aes_key)
            2. Build and display QR code URL
            3. Poll poll_bind_result until:
               - COMPLETED 鈫?decrypt and return credentials
               - EXPIRED 鈫?break to next iteration (refresh QR)
               - deadline exceeded 鈫?return None
        ...
        while time.monotonic() < deadline:
            ...
            if status == BindStatus.EXPIRED:
                if refresh_count >= _MAX_REFRESHES:
                    logger.warning("[QQBot onboard] QR code expired %d times 鈥?giving up", _MAX_REFRESHES)
                    return None
