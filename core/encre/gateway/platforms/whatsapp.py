...
find_node_executable = lambda: None
get_encre_dir = lambda: get_data_dir()
with_encre_node_path = lambda p: p
...
def _listener_pids_on_port(port: int) -> list:
    """PIDs of processes *listening* on ``port`` (POSIX) 鈥?never clients.
    ...
    that port number 鈥?e.g. a browser with a tab open on a local dev server
    ...
    try:
        ...
        pass  # lsof not installed 鈥?fall through to ss
...
def _bridge_pid_is_ours(pid: int, session_path: Path, expected_start) -> bool:
    ...
    process 鈥?observed in the wild landing on a desktop browser's main process,
    ...
    the pidfile (definitive), and 鈥?for legacy pidfiles with no baseline 鈥?the
    ...
    """
    from encre.gateway.status import _pid_exists
    if not _pid_exists(pid):
        return False
    if expected_start is not None:
        from encre.gateway.status import get_process_start_time
    ...
    from encre.gateway.status import _read_process_cmdline
...
def _kill_stale_bridge_by_pidfile(session_path: Path) -> None:
    ...
    process becomes orphaned 鈥?this helper finds and kills it.
...
from encre.gateway.platforms.whatsapp_common import WhatsAppBehaviorMixin
from encre.gateway.whatsapp_identity import to_whatsapp_jid
...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Encre media cache directory.
    ...
    symlinks) and require it to live under one of the real cache roots 鈥?this
    covers both the canonical ``cache/<kind>`` layout and the legacy
    ``<kind>_cache`` layout that ``get_encre_dir`` may return.
    """
    try:
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Encre-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    - dm_policy: "open" | "allowlist" | "disabled" | "pairing" 鈥?how DMs are handled (default: "pairing")
    - allow_from: List of sender IDs allowed in DMs (when dm_policy="allowlist")
    - group_policy: "open" | "allowlist" | "disabled" | "pairing" 鈥?which groups are processed (default: "pairing")
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if WhatsAppAdapter._DEFAULT_BRIDGE_DIR is None:
            from encre.gateway.platforms.whatsapp_common import resolve_whatsapp_bridge_dir
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_encre_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
        ...
        # notification before the normal "鉁?whatsapp disconnected" fires.
        ...
        # (e.g. forwarded batches, paste-splits) 鈥?without debounce each
        ...
        # ``encre.gateway.platforms.whatsapp.extra.text_batch_delay_seconds`` /
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed 鈥?install Node.js and re-run `encre gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `encre whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired 鈥?pair from the dashboard or run `encre whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `encre update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".encre-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Encre-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `encre update`, so without this check it
            ...
            # with_encre_node_path() copies os.environ when called with no arg.
            bridge_env = with_encre_node_path()
            if self._reply_prefix is not None:
                bridge_env["WHATSAPP_REPLY_PREFIX"] = self._reply_prefix
            # Pass the profile-aware cache directories so the bridge writes
            # media where the Python side reads it.  Without these the bridge
            # hardcodes data dir/image,audio,document_cache, which diverges
            # under data dir overrides, profiles, and the new cache/ layout.
            from encre.gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["ENCRE_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["ENCRE_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["ENCRE_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    # Still not connected 鈥?warn but proceed (bridge may
                    # auto-reconnect later, e.g. after a code 515 restart).
                    print(f"[{self.name}] 鈿?WhatsApp not connected after 30s")
                    print(f"[{self.name}]   Bridge log: {self._bridge_log}")
                    print(f"[{self.name}]   If session expired, re-pair: encre whatsapp")
    ...
    async def send_image(
        ...
        ``metadata`` is accepted to honor the base-class contract 鈥?the
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        """Send a video natively via bridge 鈥?plays inline in WhatsApp."""
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        try:
            ...
            # Must wrap in `async with` 鈥?a bare `await session.post(...)`
    ...
    # 鈹€鈹€ Text debounce batching 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
        ...
        try:
            ...
            for url in raw_urls:
                ...
                if msg_type == MessageType.PHOTO and url.startswith(("http://", "https://")):
                    ...
                    # Local file path 鈥?bridge already downloaded the image
                    ...
                    # Local file path 鈥?bridge already downloaded the audio
                    if _is_allowed_bridge_path(url):
                        cached_urls.append(url)
                        media_types.append(bridge_mime or ("audio/ogg" if msg_type == MessageType.VOICE else "audio/mpeg"))
                    ...
                    # Local file path 鈥?bridge already downloaded the document
                    if _is_allowed_bridge_path(url):
                        cached_urls.append(url)
...
# the whatsapp_cfg YAML鈫抏nv block + _PLATFORM_CONNECTED_CHECKERS entry in
# gateway/config.py, the _setup_whatsapp wizard + _PLATFORMS["whatsapp"] static
# dict in encre/cli.py, and the _send_whatsapp dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    succeed when cron runs separately from the encre.gateway. Replaces the legacy
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # bubble, and ogg/opus as a voice note 鈥?not a file/document.
            for media_path, is_voice in media:
                if not os.path.exists(media_path):
                    ...
                    # file is missing 鈥?deliver the caption as a plain message
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in encre/cli.py and the
    static _PLATFORMS["whatsapp"] dict. CLI helpers are lazy-imported so the
    plugin's module-load surface stays minimal.
    """
    def _ge(k, d=None): return os.environ.get(k, d)
    def _se(k, v): os.environ[k] = str(v)
    get_env_value = _ge
    save_env_value = _se
...
def _is_connected(config) -> bool:
    ...
    Auth itself is handled by the external Node.js bridge 鈥?we can't verify the
    bridge token here 鈥?so the opt-in flag is the connection signal. The legacy
    ...
    would make WhatsApp always show as "configured" in ``encre setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via encre/cli.py get_env_value (not os.getenv) so setup-status
    # callers that patch get_env_value 鈥?and the gateway connected-platforms
    # check 鈥?observe the same value. Matches the discord/slack plugin pattern.
    val = (os.environ.get("WHATSAPP_ENABLED") or "").strip().lower()
