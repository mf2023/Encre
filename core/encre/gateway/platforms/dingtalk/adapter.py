...
# gracefully 鈥?same as a missing one 鈥?rather than crash the whole adapter
...
class _SafeLogFormatFilter(logging.Filter):
    ...
    ``logger.exception('unknown exception', e)`` 鈥?the message has no
...
def _install_safe_log_filter() -> None:
    ...
    Idempotent 鈥?safe to call from every ``connect()``.  The filter lives on
...
# DingTalk message type 鈫?runtime content type
...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    def REQUIRES_EDIT_FINALIZE(self) -> bool:  # noqa: N802
        ...
        are configured 鈥?webhook-only DingTalk doesn't need it.
        """
        return bool(self._card_template_id and self._card_sdk)
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Chats for which we've already fired the Done reaction 鈥?prevents
        ...
        # allows streaming_update on a finalized card 鈥?it flips back to
        # streaming).  We track those reopened cards so the next `send()` can
        # auto-close them as siblings 鈥?otherwise tool-progress cards get
    ...
    def _should_process_message(self, message: "ChatbotMessage", text: str, is_group: bool, chat_id: str) -> bool:
        ...
        When ``allowed_chats`` is non-empty, it acts as a hard gate 鈥?messages
    ...
    def _fire_done_reaction(self, chat_id: str) -> None:
        """Swap 馃Thinking 鈫?馃コDone on the original user message.

        Idempotent per chat_id 鈥?safe to call from segment-break flushes
        ...
        async def _swap() -> None:
            await self._send_emotion(
                msg_id, conversation_id, "馃Thinking", recall=True,
            )
            await self._send_emotion(
                msg_id, conversation_id, "馃コDone", recall=False,
            )
    ...
    async def _on_message(
        ...
        # gets its own Thinking鈫扗one cycle.
        if chat_id:
    ...
    def _extract_text(message: "ChatbotMessage") -> str:
        ...
        Handles both legacy and current dingtalk-stream SDK payload shapes:
          ...
          * >= 0.20: ``message.text`` is a ``TextContent`` dataclass whose
            ``__str__`` returns ``"TextContent(content=...)"`` 鈥?never fall
        ...
        # LLM see the raw text 鈥?it handles "@bot hello" cleanly.
        return content
                        ...
                        if dl_code:
                            ...
                            if mapped == "image":
                                ...
                                if msg_type == MessageType.TEXT:
                                    ...
                                    # native voice note 鈥?route through STT.
    ...
    async def send(
        ...
        # to an inbound user message 鈥?only `base.py:_send_with_retry` sets
        ...
        #      updates don't flicker closed鈫抯treaming鈫抍losed repeatedly).
        ...
        if self._card_template_id and current_message and self._card_sdk:
            ...
            # before creating a new one (handles tool-progress 鈫?final-
            ...
            if result and result.success:
                if is_final_reply:
                    # Final reply: card closed, swap Thinking 鈫?Done.
        ...
        # in the text body 鈥?no platform-specific markdown wrapper).
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"encre_{uuid.uuid4().hex[:12]}"
        ...
        *,
        ...
        try:
            ...
            if finalize:
                ...
                # Non-final edit reopens the card into streaming state 鈥?
...
class _IncomingHandler(
    ...
    async def process(self, message: "CallbackMessage"):
        ...
        method returns the ACK immediately 鈥?blocking here would prevent the
        ...
        try:
            ...
            # Thinking reaction 鈥?fire-and-forget, tracked
            if msg_id and conversation_id:
                self._adapter._spawn_bg(
                    self._adapter._send_emotion(
                        msg_id, conversation_id, "馃Thinking", recall=False,
                    )
                )
...
# the dingtalk_cfg YAML鈫抏nv block + _PLATFORM_CONNECTED_CHECKERS entry in
# gateway/config.py, the _setup_dingtalk wizard + _PLATFORMS["dingtalk"] static
# dict in encre/cli.py, and the _send_dingtalk dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    succeed when cron runs separately from the encre.gateway. The live adapter uses
...
def interactive_setup() -> None:
    """Configure DingTalk 鈥?QR scan (recommended) or manual credential entry.

    Replaces encre/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in encre/cli.py. CLI helpers are
    ...
    prompt_choice = lambda x, y: None
    prompt = lambda x='': input(x); prompt_yes_no = lambda x: input(x).strip().lower() in ('y','yes'); print_header = print; print_success = print; print_warning = print
...
def _apply_yaml_config(yaml_cfg: dict, dingtalk_cfg: dict) -> dict | None:
    ...
    Returns None 鈥?everything flows through env.
    """
    ...
    if allowed is None:
        ...
        # allowlist at encre.gateway.platforms.dingtalk.extra.allowed_users; the
        # adapter reads it from PlatformConfig.extra, but gateway
        # authorization (_is_user_authorized in gateway/authz_mixin.py)
        # only consults DINGTALK_ALLOWED_USERS 鈥?without this bridge a
        # nested-only allowlist passes the adapter and is then denied at
        # the encre.gateway. Check this block's own extra first (the dispatch
...
def _check_requirements() -> bool:
    return DINGTALK_STREAM_AVAILABLE
