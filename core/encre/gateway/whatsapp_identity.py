...
- :func:`normalize_whatsapp_identifier` 鈥?strip JID/LID/device/plus syntax
  down to the bare numeric identifier.
- :func:`to_whatsapp_jid` 鈥?build a bridge-safe outbound JID from a bare phone
  number or pass-through an already-qualified JID unchanged.
- :func:`canonical_whatsapp_identifier` 鈥?walk the bridge's
  ``lid-mapping-*.json`` files and return a stable canonical identity
  across phone/LID variants.
- :func:`expand_whatsapp_aliases` 鈥?return the full alias set for an
...
def normalize_whatsapp_identifier(value: str) -> str:
    ...
    Useful for plugins that want to match sender IDs against user-supplied
    config (phone numbers in ``config.yaml``) without worrying about which
    variant the bridge happens to deliver.

    Args:
        value: The raw identifier string as emitted by the bridge.

    Returns:
        The bare numeric identifier, or an empty string when ``value`` is
        empty/whitespace.
    """
    return (
        str(value or "")
...
# A target that is "just a phone number" 鈥?optional leading ``+`` then digits
...
def to_whatsapp_jid(value: str) -> str:
    ...
    Baileys' ``jidDecode`` crashes on a bare phone number 鈥?it expects a
    fully-qualified JID such as ``50766715226@s.whatsapp.net``. This helper is
    the inverse of :func:`normalize_whatsapp_identifier`: instead of stripping a
    JID down to its numeric core for comparison, it *builds* the JID a send must
    use.

    Behaviour:

    - ``"+50766715226"`` / ``"50766715226"`` 鈫?``"50766715226@s.whatsapp.net"``
    - ``"50766715226@s.whatsapp.net"`` 鈫?unchanged
    - ``"group-id@g.us"`` / ``"130631430344750@lid"`` 鈫?unchanged
    - ``"user:device@s.whatsapp.net"`` style colon-before-``@`` 鈫?``@`` form
    - anything that isn't a recognizable bare phone 鈫?returned unchanged so
      the bridge can surface a meaningful error rather than us mangling it.

    Returns ``""`` for an empty/whitespace input.

    Args:
        value: The outbound target, either a bare phone number or a JID.

    Returns:
        A bridge-safe JID string, or ``""`` when input is empty.
    """
    if not value:
        return ""
    ...
    # legacy Baileys shape whose ``:device`` part is not addressable 鈥?collapse
    ...
    # Already a fully-qualified JID 鈥?leave it alone.
    if "@" in normalized:
        return normalized
...
def expand_whatsapp_aliases(identifier: str) -> Set[str]:
    ...
    bridge's ``whatsapp/session/lid-mapping-*.json`` files under the Encre data dir,
    starting from ``identifier``. The result always includes the normalized
    input itself, so callers can safely ``in`` check against the return value
    without a separate fallback branch.

    Returns an empty set if ``identifier`` normalizes to empty.

    Args:
        identifier: The starting WhatsApp identifier (any variant).

    Returns:
        The full set of reachable alias identifiers, including the input.
    """
    normalized = normalize_whatsapp_identifier(identifier)
    if not normalized:
        return set()

    session_dir = get_encre_dir("platforms/whatsapp/session", "whatsapp/session")
...
def canonical_whatsapp_identifier(identifier: str) -> str:
    ...
    applies to a DM ``chat_id`` *and* to the ``participant_id`` of a member
    inside a group chat 鈥?both represent a user identity, and the bridge may
    flip between the two for the same human.
    ...
    Args:
        identifier: The raw WhatsApp identifier (phone JID, LID, or bare
            number) to canonicalise.

    Returns:
        The stable canonical identifier, or ``""`` when input is empty.
    """
    normalized = normalize_whatsapp_identifier(identifier)
    if not normalized:
        return ""
