...
"""Encre protocol layer: multi-session event routing.

Hosts :class:`EventRouter`, the concurrency-safe bridge between channels,
the SessionManager and the agent runtime.  The router exposes
:meth:`EventRouter.submit` / :meth:`EventRouter.submit_stream`, which
channels call to run prompts, and guarantees each connection gets an
...
from encre.model_selection import apply_target_models
...
logger = logging.getLogger("encre.protocol.session_router")
...
class EventRouter:
    ...
    async def submit_stream(
        ...
        **Must** be called inside an ``iclaw_context()`` block so the
        ...
        apply_target_models(cfg, target_model_ids)
        if session_id:
