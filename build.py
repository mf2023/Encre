...
def _generate_registry(locale_list: list[str]) -> Path:
    """Generate a filtered locales/registry.ts containing only the requested locales.

    Returns the path to the generated file (same location as the source template).
    """
    registry_dir = DESKTOP / "renderer" / "src" / "locales"
    registry_path = registry_dir / "registry.ts"

    # Build import block
    lines: list[str] = []
    lines.append('/**')
    lines.append(' * Copyright © 2025-2026 Wenze Wei. All Rights Reserved.')
    lines.append(' *')
    lines.append(' * AUTO-GENERATED by build.py — DO NOT EDIT BY HAND.')
    lines.append(' * Re-generated on every build with the requested locale subset.')
    lines.append(' */')
    lines.append('')
    lines.append('import type { LocaleMessages } from "../features/i18n.js";')
    for loc in locale_list:
        module_name = loc.replace("-", "")  # zh-Hant → zhHant in import identifier
        lines.append(f'import {{ {module_name} }} from "./{loc}.js";')

    lines.append('')
    lines.append('export const LOCALE_REGISTRY = {')
    for loc in locale_list:
        module_name = loc.replace("-", "")
        lines.append(f'  {loc}: {module_name},')
    lines.append('} as const satisfies Record<string, LocaleMessages>;')
    lines.append('')
    lines.append(f'export const ALL_LOCALES: readonly string[] = {locale_list!r};')
    lines.append('')

    registry_path.write_text('\n'.join(lines), encoding='utf-8')
    print(f"  [registry] generated {len(locale_list)} locale(s): {locale_list}")
...
def build_frontend(locale_list: list[str]) -> None:
    ...
    Generates a filtered locales/registry.ts before invoking the esbuild step so
    unused locale modules are omitted from the bundle.
    """
    print("\n=== Building frontend (locales: {}) ===".format(locale_list))
    run("cd desktop && npm ci")
    _generate_registry(locale_list)
